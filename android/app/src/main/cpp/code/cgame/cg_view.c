...
qboolean CG_IsThirdPersonFollowMode(vrFollowMode_t followMode)
{
    if (followMode == VRFM_QUERY) {
        return cg.snap->ps.pm_flags & PMF_FOLLOW && (vr->follow_mode == VRFM_THIRDPERSON_1 || vr->follow_mode == VRFM_THIRDPERSON_2);
    }
...
{
    return ((cg.predictedPlayerState.stats[STAT_HEALTH] <= 0) && (cg.predictedPlayerState.pm_type != PM_INTERMISSION));
}
...
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView, qboolean demoPlayback)
{
    ...
    trap_Cvar_Set("vr_thirdPersonSpectator", (CG_IsDeathCam() || cg.demoPlayback || CG_IsThirdPersonFollowMode(VRFM_QUERY) ? "1" : "0"));
    ...
    cg.renderingThirdPerson = cg.predictedPlayerState.pm_type == PM_SPECTATOR || cg.demoPlayback || CG_IsThirdPersonFollowMode(VRFM_QUERY) || cg_thirdPerson.integer;
