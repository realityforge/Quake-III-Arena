...
void NormalizeAngles(vec3_t angles)
{
    ...
    while (angles[YAW] >= 180)
        angles[YAW] -= 360;
    while (angles[YAW] < -180)
        angles[YAW] += 360;
    while (angles[ROLL] >= 180)
        angles[ROLL] -= 360;
    while (angles[ROLL] < -180)
        angles[ROLL] += 360;
}
...
void VR_HapticEvent(const char* event, int position, int flags, int intensity, float angle, float yHeight)
{
    ...
    if (strcmp(event, "pickup_shield") == 0 || strcmp(event, "pickup_weapon") == 0 || strstr(event, "pickup_item") != NULL) {
    ...
    } else if (strcmp(event, "chainsaw_fire") == 0 || strcmp(event, "RTCWQuest:fire_tesla") == 0) {
    ...
    } else if (strcmp(event, "rocket_fire") == 0 || strcmp(event, "RTCWQuest:fire_sniper") == 0 || strcmp(event, "bfg_fire") == 0 || strcmp(event, "handgrenade_fire") == 0) {
        ...
        for (size_t i = 0; i < sizeof(actionsToEnumerate) / sizeof(actionsToEnumerate[0]); ++i) {
            ...
            if (countOutput < 32) {
                ...
                for (uint32_t a = 0; a < countOutput; ++a) {
                    ...
                    nameGetInfo.whichComponents = XR_INPUT_SOURCE_LOCALIZED_NAME_USER_PATH_BIT | XR_INPUT_SOURCE_LOCALIZED_NAME_INTERACTION_PROFILE_BIT | XR_INPUT_SOURCE_LOCALIZED_NAME_COMPONENT_BIT;
...
static void IN_VRController(qboolean isRightController, XrPosef pose)
{
    ...
    if (vr.virtual_screen || cl.snap.ps.pm_type == PM_INTERMISSION) {
        ...
        vr.weapon_zoomed = vr_weaponScope->integer && vr.weapon_stabilised && (cl.snap.ps.weapon == WP_RAILGUN) && (VectorLength(vr.weaponoffset) < 0.24f) && cl.snap.ps.stats[STAT_HEALTH] > 0;
...
void IN_VRInputFrame(void)
{
    ...
    if (vr_extralatencymode != NULL && vr_extralatencymode->integer) {
