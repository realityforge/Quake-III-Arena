...
int FS_FindVM(void** startSearch, char* found, int foundlen, const char* name, int enableDll)
{
    ...
    if (enableDll) {
        ...
        Com_sprintf(dllName, sizeof(dllName), "%s" ARCH_STRING "_%s" DLL_EXT, name, nativeVMBase);
    }
...
static void FS_CheckPak0(void)
{
    ...
    for (path = fs_searchpaths; path; path = path->next) {
        ...
        if (!Q_stricmpn(pakBasename, "pak0", MAX_OSPATH) && curpack->checksum == DEMO_PAK0_CHECKSUM) // is this the demo?
        {
