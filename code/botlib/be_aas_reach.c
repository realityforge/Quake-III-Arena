...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs) {
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    //
    // botimport.Print(PRT_MESSAGE, "found a trigger_push with velocity %f %f
...
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs,
  ...
  if (areanum) {
    ...
    if (!trace.startsolid) {
      ...
      // a point is in an area and that starting an AAS_TraceClientBBox from
      // that point will return trace.startsolid qtrue
#if 0
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num,
  ...
  if (water_foundreach) {
    ...
    if (aasworld.areasettings[AAS_PointAreaNum(testpoint)].areaflags &
        AREA_LIQUID) {
      // don't create ridiculous water jump reachabilities from areas very far
    ...
    if (ground_bestdist > 0 && ground_bestdist < aassettings.phys_maxbarrier) {
      ...
      if (!water_foundreach || (ground_bestdist - water_bestdist < 16)) {
        ...
        if (!AAS_AreaCrouch(area1num) && !AAS_AreaCrouch(area2num)) {
          ...
          lreach->traveltime =
              aassettings.rs_barrierjump; // AAS_BarrierJumpTravelTime();
          lreach->next = areareachability[area1num];
          areareachability[area1num] = lreach;
      ...
      if (!aassettings.rs_maxfallheight ||
        ...
        if (!trace.startsolid && trace.fraction >= 1.0) {
          ...
          if (AAS_PointAreaNum(trace.endpos) == area2num) {
            ...
            numareas =
                AAS_TraceAreas(start, end, areas, NULL, ARRAY_LEN(areas));
  ...
  if (bestdist > 4 && bestdist < maxjumpdistance) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue,
                                velocity, cmdmove, 3, 30, 0.1f, stopevent, 0,
                                qfalse);
      ...
      teststart[2] += 1;
      numareas =
          AAS_TraceAreas(move.endpos, teststart, areas, NULL, ARRAY_LEN(areas));
    ...
    if ((traveltype & TRAVELTYPE_MASK) == TRAVEL_WALKOFFLEDGE &&
      ...
      lreach->traveltime =
          aassettings.rs_startjump + VectorDistance(bestend, beststart) * 240 /
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
      ...
      else*/
      {
        ...
        VectorClear(cmdmove);
        AAS_PredictClientMovement(&move, -1, destorigin, PRESENCE_NORMAL,
...
aas_lreachability_t *AAS_FindFaceReachabilities(vec3_t *facepoints,
  ...
  vec3_t beststart = {0}, beststart2 = {0}, bestend = {0}, bestend2 = {0}, tmp,
         hordir, testpoint;
...
absmins[2]); botimport.Print(PRT_MESSAGE, "absmaxs = %f %f %f\n", absmaxs[0],
absmaxs[1], absmaxs[2]); #endif REACH_DEBUG VectorAdd(absmins, absmaxs, origin);
    VectorScale (origin, 0.5, origin);
        ...
        if (ret && speed < 150) {
          ...
          // if (hordist < 1.6 * facecenter[2] - areastart[2])
          {
            ...
            AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL,
                                      qfalse, velocity, cmdmove, 30, 30, 0.1f,
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
  ...
  for (i = 0; i < area2->numfaces; i++) {
    ...
    v = aasworld.vertexes
            [aasworld.edges[abs(aasworld.edgeindex[face2->firstedge])].v[0]];
      ...
      for (j = 0; j < area->numfaces; j++) {
        ...
        for (l = 0; l < face2->numedges; l++) {
          ...
          if (abs(edge1num) == abs(edge2num)) {
            ...
            // if not going through a cluster portal
            numareas =
                AAS_TraceAreas(mid, testend, areas, NULL, ARRAY_LEN(areas));
            for (p = 0; p < numareas; p++)
              if (AAS_AreaClusterPortal(areas[p]))
                break;
            if (p < numareas)
              break;
            // if a maximum fall height is set and the bot would fall down
            // further
            if (aassettings.rs_maxfallheight &&
