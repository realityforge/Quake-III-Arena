...
//						presencetype	: presence type to start
//with 						velocity		: velocity to start with
//						cmdmove			: client command
//movement 						cmdframes		: number of frame cmdmove is valid 						maxframes
//: maximum number of predicted frames
//						frametime		: duration of one
//predicted frame 						stopevent		: events that stop the prediction
//						stopareanum		: stop as soon as
//entered this area
...
int AAS_ClientMovementPrediction(struct aas_clientmove_s *move, int entnum,
  ...
  for (n = 0; n < maxframes; n++) {
    ...
    if (n < cmdframes) {
      ...
      velchange = -phys_maxacceleration; newvel = frame_test_vel[i] + velchange;
              //
    ...
    do {
      ...
      //
      if (stopevent & (SE_ENTERAREA | SE_TOUCHJUMPPAD | SE_TOUCHTELEPORTER |
...
//						end		: end position of
//jump 						*speed	: returned speed for jump
