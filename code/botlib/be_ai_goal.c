  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    if (spawnflags & 1) {
      ...
      if (!(AAS_PointContents(origin) & CONTENTS_WATER)) {
        ...
        trace = AAS_Trace(origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs,
                          end, -1, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
        ...
        if (trace.fraction >= 1) {
          ...
          // botimport.Print(PRT_MESSAGE, "item %s reachable from jumppad area
          // %d\r\n", ic->iteminfo[i].classname, goalareanum);
          if (!goalareanum)
            continue;
        } // end if
    ...
    if (goalareanum) {
      ...
      li->goalareanum = AAS_BestReachableArea(
          origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, li->goalorigin);
      if (!li->goalareanum) {
        botimport.Print(PRT_MESSAGE,
...
int BotChooseLTGItem(int goalstate, vec3_t origin, int *inventory,
  ...
  for (li = levelitems; li; li = li->next) {
    ...
    if (!li->goalareanum)
      continue;
    // FIXME: is this a good thing? added this for items that never spawned into
    // the game (f.i. CTF flags in obelisk)
