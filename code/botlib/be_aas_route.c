...
void AAS_CreateAllRoutingCache(void) {
  ...
  for (i = 1; i < aasworld.numareas; i++) {
    ...
    for (j = 1; j < aasworld.numareas; j++) {
      ...
      // TFL_DEFAULT); Log_Write("traveltime from %d to %d is %d", i, j, t);
    } // end for
  }   // end for
...
void AAS_WriteRouteCache(void) {
  ...
  aasworld.decompressedvis ); size = AAS_CompressVis( aasworld.decompressedvis,
  aasworld.numareas, aasworld.decompressedvis ); botimport.FS_Write(&size,
  sizeof(int), fp); botimport.FS_Write(aasworld.decompressedvis, size, fp);
  }
  */
  //
...
int AAS_AreaRouteToGoalArea(int areanum, vec3_t origin, int goalareanum,
  ...
  // check !AAS_AreaReachability(areanum) with custom developer-only debug
  // message
  if (areanum <= 0 || areanum >= aasworld.numareas) {
    if (botDeveloper) {
      botimport.Print(PRT_ERROR,
  ...
  if (!aasworld.areasettings[areanum].numreachableareas ||
      !aasworld.areasettings[goalareanum].numreachableareas) {
    return qfalse;
  } // end if
