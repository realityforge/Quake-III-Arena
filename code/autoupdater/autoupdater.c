...
#define AUTOUPDATE_URL                                                         \
  AUTOUPDATE_BASEURL "/" AUTOUPDATE_PACKAGE "/" AUTOUPDATE_PLATFORM            \
                     "/" AUTOUPDATE_ARCH "/"
#endif
...
static void restoreRollbacks(void) {
  ...
  for (item = manifest; item != NULL; item = item->next) {
    if (item->rollback) {
      ...
      snprintf(rollbackPath, sizeof(rollbackPath), "updates/rollbacks/%d",
               item->rollback);
...
#define APPENDCMDLINE(str)                                                     \
  ...
  if (CreateProcessA(exe, cmdline, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL,
...
static void prepHttpLib(void) {
  hInternet = InternetOpenA(AUTOUPDATE_USER_AGENT, INTERNET_OPEN_TYPE_PRECONFIG,
                            NULL, NULL, 0);
  if (!hInternet) {
...
static int runHttpDownload(const char *from, FILE *to) {
  ...
  HINTERNET hUrl = InternetOpenUrlA(
      ...
      INTERNET_FLAG_HYPERLINK | INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTP |
          ...
          INTERNET_FLAG_NO_UI | INTERNET_FLAG_RESYNCHRONIZE |
          INTERNET_FLAG_RELOAD | INTERNET_FLAG_SECURE,
      0);

  if (!hUrl) {
    infof("InternetOpenUrl failed. err=%d", (int)GetLastError());
  } else if (!HttpQueryInfo(hUrl,
                            HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER,
                            &httpcode, &dwordlen, &zero)) {
...
static int runHttpDownload(const char *from, FILE *to) {
  ...
  curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER,
                   1L); /* require valid SSL cert. */
  curl_easy_setopt(
      curl, CURLOPT_SSL_VERIFYHOST,
      2L); /* require SSL cert with same hostname as we connected to. */
...
static void parseArgv(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i += 2) {
    if (strcmp(argv[i], "--waitpid") == 0) {
      ...
      infof("We will wait for process " PIDFMT " if necessary",
            (PIDFMTCAST)options.waitforprocess);
...
static void verifySignature(const char *fname, const char *sigfname,
                            const char *keyfname) {
  rsa_key key;
  ...
  if ((rc = rsa_verify_hash(sig, siglen, hash, hashlen, sha256_hash_index,
                            SALT_LEN, &status, &key)) != CRYPT_OK) {
...
/* unix replaces the process with execl(), but Windows needs to wait for the
 * parent to terminate. */
#ifdef _WIN32
  DWORD ppid = 0;
  ...
  if (options.waitforprocess) {
    char pidstr[64];
    snprintf(pidstr, sizeof(pidstr), PIDFMT,
             (PIDFMTCAST)options.waitforprocess);
    launchProcess(options.updateself, options.updateself, "--waitpid", pidstr,
                  NULL);
  } else {
...
static void hashFile(const char *fname, unsigned char *sha256) {
  ...
  if ((rc = hash_file(sha256_hash_index, fname, sha256, &hashlen)) !=
      CRYPT_OK) {
    ...
    if (strcasecmp(item->fname, fname) == 0) {
      if (fileNeedsUpdate(item)) {
        ...
        if (options.waitforprocess) {
          ...
          snprintf(pidstr, sizeof(pidstr), PIDFMT,
                   (PIDFMTCAST)options.waitforprocess);
          launchProcess(to, to, "--updateself", argv0, "--waitpid", pidstr,
                        NULL);
        } else {
...
static void applyUpdates(void) {
  ...
  for (item = manifest; item != NULL; item = item->next) {
    ...
    if (io != NULL) {
      ...
      snprintf(rollbackPath, sizeof(rollbackPath), "updates/rollbacks/%d",
               rollbackIndex);
      infof("Moving file for rollback: '%s' -> '%s'", item->fname,
            rollbackPath);
      remove(rollbackPath);
...
static void waitToApplyUpdates(void) {
  if (options.waitforprocess) {
    infof("Waiting for pid " PIDFMT " to die...",
          (PIDFMTCAST)options.waitforprocess);
    {
#ifdef _WIN32
