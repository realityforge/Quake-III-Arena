...
qboolean SNDDMA_Init(void) {
  ...
  // !!! FIXME: pulseaudio capture records audio the entire time the program is
  // running. https://bugzilla.libsdl.org/show_bug.cgi?id=4087
  if (Q_stricmp(SDL_GetCurrentAudioDriver(), "pulseaudio") == 0) {
    Com_Printf("SDL audio capture support disabled for pulseaudio "
               "(https://bugzilla.libsdl.org/show_bug.cgi?id=4087)\n");
  } else if (!s_sdlCapture->integer) {
    Com_Printf("SDL audio capture support disabled by user ('+set s_sdlCapture "
               "1' to enable)\n");
  }
#if USE_MUMBLE
    ...
    /* !!! FIXME: list available devices and let cvar specify one, like OpenAL
     * does */
    SDL_AudioSpec spec;
    SDL_zero(spec);
    spec.freq = 48000;
