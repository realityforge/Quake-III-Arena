...
#define GLE(ret, name, ...)                                                    \
  ...
  if (fixedFunction) {
    if (QGL_VERSION_ATLEAST(1, 2)) {
      ...
      QGL_ES_1_1_PROCS;
      QGL_ES_1_1_FIXED_FUNCTION_PROCS;
      // error so this doesn't segfault due to NULL desktop GL functions being
      // used
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder,
  ...
  for (i = 0; i < 16; i++) {
    ...
    if ((SDL_window =
             SDL_CreateWindow(CLIENT_WINDOW_TITLE, x, y, glConfig.vidWidth,
                              glConfig.vidHeight, flags)) == NULL) {
  ...
  if (fixedFunction) {
    ...
    if (SDL_GL_ExtensionSupported("GL_EXT_compiled_vertex_array")) {
      if (r_ext_compiled_vertex_array->value) {
        ...
        qglUnlockArraysEXT =
            (void(APIENTRY *)(void))SDL_GL_GetProcAddress("glUnlockArraysEXT");
...
void GLimp_EndFrame(void) {
  ...
  if (r_fullscreen->modified) {
    ...
    if (needToToggle) {
      sdlToggled =
          SDL_SetWindowFullscreen(SDL_window, r_fullscreen->integer) >= 0;
