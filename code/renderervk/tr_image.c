...
void GL_TextureMode(const char* string)
{
    ...
    if (glConfig.hardwareType == GLHW_3DFX_2D3D && gl_filter_max == GL_LINEAR && gl_filter_min == GL_LINEAR_MIPMAP_LINEAR) {
...
byte* resample_image_data(const image_t* image, byte* data, const int data_size, int* bytes_per_pixel)
{
    ...
    case VK_FORMAT_B4G4R4A4_UNORM_PACK16:
        ...
        for (i = 0; i < data_size; i += 4, p++) {
            ...
            byte b = data[i + 2];
            byte a = data[i + 3];
            *p = (uint32_t)((a / 255.0) * 15.0 + 0.5) | ((uint32_t)((r / 255.0) * 15.0 + 0.5) << 4) | ((uint32_t)((g / 255.0) * 15.0 + 0.5) << 8) | ((uint32_t)((b / 255.0) * 15.0 + 0.5) << 12);
        }
        *bytes_per_pixel = 2;
    ...
    case VK_FORMAT_A1R5G5B5_UNORM_PACK16:
        ...
        for (i = 0; i < data_size; i += 4, p++) {
            ...
            byte g = data[i + 1];
            byte b = data[i + 2];
            *p = (uint32_t)((b / 255.0) * 31.0 + 0.5) | ((uint32_t)((g / 255.0) * 31.0 + 0.5) << 5) | ((uint32_t)((r / 255.0) * 31.0 + 0.5) << 10) | (1 << 15);
        }
        *bytes_per_pixel = 2;
        return buffer; // must be freed after upload!
...
#ifdef USE_VK_PBR
    tr.emptyImage = R_CreateImage("*empty", NULL, (byte*)data, 2, 2, IMGFLAG_NONE);
#endif
