...
#ifdef _WIN32
  {
    /* We don't need the Unix pipe() tapdance here because Windows lets children
     * wait on parent processes. */
    ...
    sprintf(cmdline, "" AUTOUPDATER_BIN " --waitpid %u",
            (unsigned int)GetCurrentProcessId());

    if (CreateProcessA(AUTOUPDATER_BIN, cmdline, NULL, NULL, FALSE,
...
#ifdef __APPLE__
        if (chdir("../..") == -1) { /* put this at base of app bundle so paths
                                       make sense later. */
          _exit(1);                 /* oh well. */
        }
#endif
