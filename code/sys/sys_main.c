...
void *Sys_LoadDll(const char *name, qboolean useSystemLib) {
  ...
  if (!Sys_DllExtension(name)) {
    Com_Printf(
        "Refusing to attempt to load library \"%s\": Extension not allowed.\n",
        name);
    return NULL;
  }
  ...
  if (!dllhandle) {
    ...
    if (len < sizeof(libPath)) {
      ...
      Com_Printf("Skipping trying to load \"%s\" from \"%s\", file name is too "
                 "long.\n",
                 name, topDir);
    }
    ...
    if (!dllhandle) {
      ...
      if (FS_FilenameCompare(topDir, basePath)) {
        len = Com_sprintf(libPath, sizeof(libPath), "%s%c%s", basePath,
                          PATH_SEP, name);
        if (len < sizeof(libPath)) {
          Com_Printf("Trying to load \"%s\" from \"%s\"...\n", name, basePath);
          dllhandle = Sys_LoadLibrary(libPath);
        } else {
          Com_Printf("Skipping trying to load \"%s\" from \"%s\", file name is "
                     "too long.\n",
                     name, basePath);
        }
      }
