...
void ProjectDlightTexture_altivec(void) {
  ...
  vector unsigned char vSel =
      VECCONST_UINT8(0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00,
  ...
  for (l = 0; l < backEnd.refdef.num_dlights; l++) {
    ...
    for (i = 0; i < tess.numVertexes; i++, texCoords += 2, colors += 4) {
      ...
      colorChar = vec_sel(colorChar, vSel,
...
void LerpMeshVertexes_altivec(md3Surface_t *surf, float backlerp) {
  ...
  if (backlerp == 0) {
    ...
    for (vertNum = 0; vertNum < numVerts;
      ...
      newNormalsFloatVec = vec_perm(newNormalsFloatVec, newNormalsFloatVec,
                                    newNormalsStorePermute);
