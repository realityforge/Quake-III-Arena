  ...
  while (1) {
    ...
    //
      ...
      if (!Q_stricmp(token, "$whiteimage")) {
        ...
        if (!tr.worldDeluxeMapping) {
          ri.Printf(PRINT_WARNING,
                    "WARNING: shader '%s' wants a deluxe map in a map compiled "
                    "without them\n",
                    shader.name);
          return qfalse;
        }
    ...
    //
      ...
      if (totalImages > MAX_IMAGE_ANIMATIONS) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: ignoring excess images for 'animMap' (found %d, "
                  "max is %d) in shader '%s'\n",
      ...
      if (stage->bundle[0].videoMapHandle != -1) {
        ...
        ri.Printf(PRINT_WARNING,
                  "WARNING: could not load '%s' for 'videoMap' keyword in "
                  "shader '%s'\n",
                  token, shader.name);
      }
    }
    ...
    //
      ...
      if (!Q_stricmp(token, "diffuseMap")) {
        ...
        VectorSet4(stage->normalScale, r_baseNormalX->value,
    ...
    //
      ...
      if (token[0] == 0) {
        ri.Printf(PRINT_WARNING,
                  "WARNING: missing parameter for specular reflectance in "
                  "shader '%s'\n",
                  shader.name);
        continue;
      }
      ...
      if (r_pbr->integer) {
        ...
        stage->specularScale[0] = stage->specularScale[1] =
    ...
    //
      ...
      if (r_pbr->integer)
        ...
        // Assumes max exponent of 8190 and min of 0, must change here if
        // altered in lightall_fp.glsl
    ...
    //
      ...
      if (token[0] == 0) {
        if (r_pbr->integer) {
          ...
          stage->specularScale[1] = stage->specularScale[2] =
  ...
  if (strcmp(token, "-")) {
    for (i = 0; i < 6; i++) {
      ...
      shader.sky.outerbox[i] = R_FindImageFile(
          (char *)pathname, IMGTYPE_COLORALPHA, imgFlags | IMGFLAG_CLAMPTOEDGE);
  ...
  while (1) {
    ...
    // sun parms
      ...
      if (isGL2Sun) {
        ...
        // parse twice, since older shaders may include mapLightScale before
        // sunShadowScale
        token = COM_ParseExt(text, qfalse);
        if (token[0])
          tr.sunShadowScale = atof(token);
    ...
    // fogParms
      ...
      if (r_greyscale->integer) {
        ...
        shader.fogParms.color[0] =
            LERP(shader.fogParms.color[0], luminance, r_greyscale->value);
        shader.fogParms.color[1] =
            LERP(shader.fogParms.color[1], luminance, r_greyscale->value);
        shader.fogParms.color[2] =
            LERP(shader.fogParms.color[2], luminance, r_greyscale->value);
      }
...
static void
CollapseStagesToLightall(shaderStage_t *diffuse, shaderStage_t *normal,
                         shaderStage_t *specular, shaderStage_t *lightmap,
                         qboolean useLightVector, qboolean useLightVertex,
                         qboolean parallax, qboolean tcgen) {
  int defs = 0;

  // ri.Printf(PRINT_ALL, "shader %s has diffuse %s", shader.name,
  // diffuse->bundle[0].image[0]->imgName);
  ...
  if (r_normalMapping->integer) {
    image_t *diffuseImg;
    if (normal) {
      // ri.Printf(PRINT_ALL, ", normalmap %s",
      // normal->bundle[0].image[0]->imgName);
      ...
      imgFlags_t normalFlags =
          (diffuseImg->flags & ~IMGFLAG_GENNORMALMAP) | IMGFLAG_NOLIGHTSCALE;
      ...
      normalImg =
          R_FindImageFile(normalName, IMGTYPE_NORMALHEIGHT, normalFlags);
  ...
  if (r_specularMapping->integer) {
    ...
    if (specular) {
      ...
      specularImg =
          R_FindImageFile(specularName, IMGTYPE_COLORALPHA, specularFlags);
...
static int CollapseStagesToGLSL(void) {
  ...
  if (!skip) {
    ...
    if (stages[0].active && stages[0].bundle[0].tcGen == TCGEN_LIGHTMAP &&
        stages[1].active) {
      int blendBits =
          stages[1].stateBits & (GLS_DSTBLEND_BITS | GLS_SRCBLEND_BITS);
  ...
  if (!skip) {
    ...
    for (i = 0; i < MAX_SHADER_STAGES; i++) {
      ...
      if (pStage->bundle[0].tcGen == TCGEN_LIGHTMAP) {
        int blendBits =
            pStage->stateBits & (GLS_DSTBLEND_BITS | GLS_SRCBLEND_BITS);
      ...
      for (j = i + 1; j < MAX_SHADER_STAGES; j++) {
        ...
        case ST_COLORMAP:
          if (pStage2->bundle[0].tcGen == TCGEN_LIGHTMAP) {
            ...
            // Only add lightmap to blendfunc filter stage if it's the first
            // time lightmap is used otherwise it will cause the shader to be
            // darkened by the lightmap multiple times.
            if (!usedLightmap ||
  ...
  // convert any remaining lightmap stages to a lighting pass with a white
  // texture only do this with r_sunlightMode non-zero, as it's only for correct
  // shadows.
...
static void InitShader(const char *name, int lightmapIndex) {
  ...
  for (i = 0; i < MAX_SHADER_STAGES; i++) {
    ...
    if (r_pbr->integer) {
      ...
      stages[i].specularScale[0] = stages[i].specularScale[1] =
...
static void CreateExternalShaders(void) {
  ...
  // HACK: if sunflare is missing, make one using the flare image or dlight
  // image
