...
void RB_ToneMap(FBO_t *hdrFbo, ivec4_t hdrBox, FBO_t *ldrFbo, ivec4_t ldrBox,
                int autoExposure) {
  ...
  if (autoExposure) {
    if (lastFrameCount == 0 || tr.frameCount < lastFrameCount ||
      ...
      FBO_Blit(hdrFbo, hdrBox, NULL, tr.textureScratchFbo[0], dstBox,
               &tr.calclevels4xShader[0], NULL, 0);
      ...
      while (size > 1) {
        ...
        // FBO_Blit(targetFbo, srcBox, NULL, tr.textureScratchFbo[nextScratch],
        // dstBox, &tr.calclevels4xShader[1], NULL, 0);
    ...
    FBO_Blit(tr.targetLevelsFbo, srcBox, NULL, tr.calcLevelsFbo, NULL, NULL,
             color, GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA);
  }

  // tonemap
  color[0] = color[1] = color[2] =
      pow(2, r_cameraExposure->value -
                 autoExposure); // exp2(r_cameraExposure->value);
  color[3] = 1.0f;
...
void RB_BokehBlur(FBO_t *src, ivec4_t srcBox, FBO_t *dst, ivec4_t dstBox,
  ...
  if (glRefConfig.framebufferObject) {
    ...
    if (blur > 1.0f) {
      ...
      // FBO_Blit(tr.quarterFbo[0], NULL, NULL, tr.textureScratchFbo[0], NULL,
      // NULL, NULL, 0);
      FBO_FastBlit(tr.quarterFbo[0], NULL, tr.textureScratchFbo[0], NULL,
                   GL_COLOR_BUFFER_BIT, GL_LINEAR);
    }
#endif
    ...
    // ok blur, but can see some pixelization
      ...
      for (i = 0; i < 2; i++) {
        ...
        if (i != 0)
          FBO_Blit(tr.textureScratchFbo[0], NULL, blurTexScale,
                   tr.textureScratchFbo[1], NULL, &tr.bokehShader, color,
                   GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE);
        else
          FBO_Blit(tr.textureScratchFbo[0], NULL, blurTexScale,
                   tr.textureScratchFbo[1], NULL, &tr.bokehShader, color, 0);
      }
      ...
      for (i = 0; i < 2; i++) {
        ...
        FBO_Blit(tr.quarterFbo[0], NULL, blurTexScale, tr.quarterFbo[1], NULL,
...
static void RB_RadialBlur(FBO_t *srcFbo, FBO_t *dstFbo, int passes,
                          float stretch, float x, float y, float w, float h,
...
static qboolean RB_UpdateSunFlareVis(void) {
  ...
  qglGetQueryObjectuiv(tr.sunFlareQuery[tr.sunFlareQueryIndex], GL_QUERY_RESULT,
                       &sampleCount);
...
void RB_SunRays(FBO_t *srcFbo, ivec4_t srcBox, FBO_t *dstFbo, ivec4_t dstBox) {
  ...
  // project sun point
  // Mat4Multiply(backEnd.viewParms.projectionMatrix,
  // backEnd.viewParms.world.modelMatrix, mvp);
  ...
  // radial blur passes, ping-ponging between the two quarter-size buffers
  {
    ...
    for (i = 0; i < 2; ++i) {
      RB_RadialBlur(tr.quarterFbo[i & 1], tr.quarterFbo[(~i) & 1], 5, stretch,
                    0.f, 0.f, tr.quarterFbo[0]->width, tr.quarterFbo[0]->height,
                    pos[0], pos[1], 1.125f);
      stretch += stretch_add;
    }
  }
...
static void RB_BlurAxis(FBO_t *srcFbo, FBO_t *dstFbo, float strength,
                        qboolean horizontal) {
  float dx, dy;
  float xmul, ymul;
  float weights[3] = {
...
void RB_GaussianBlur(float blur) {
...

  {
    ...
    // first, downsample the framebuffer
    FBO_FastBlit(NULL, NULL, tr.quarterFbo[0], NULL, GL_COLOR_BUFFER_BIT,
                 GL_LINEAR);
    FBO_FastBlit(tr.quarterFbo[0], NULL, tr.textureScratchFbo[0], NULL,
                 GL_COLOR_BUFFER_BIT, GL_LINEAR);
    ...
    FBO_BlitFromTexture(tr.whiteImage, NULL, NULL, tr.textureScratchFbo[0],
                        NULL, NULL, color, GLS_DEPTHTEST_DISABLE);
