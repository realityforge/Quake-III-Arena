...
void Vao_SetVertexPointers(vao_t *vao) {
  ...
  for (attribIndex = 0; attribIndex < ATTR_INDEX_COUNT; attribIndex++) {
    ...
    if (vAtb->enabled) {
      ...
      // Vao_SetVertexPointers is only called during init when using VAOs, and
      // vertex attribs start disabled anyway
      if (!glRefConfig.vertexArrayObject &&
          (glState.vertexAttribsEnabled & attribBit))
...
vao_t *R_CreateVao2(const char *name, int numVertexes, srfVert_t *verts,
                    int numIndexes, glIndex_t *indexes) {
  vao_t *vao;
  int i;
...
void R_BindVao(vao_t *vao) {
  ...
  if (glState.currentVao != vao) {
    ...
    if (glRefConfig.vertexArrayObject) {
      ...
      // Intel Graphics doesn't save GL_ELEMENT_ARRAY_BUFFER binding with VAO
      // binding.
...
void R_InitVaos(void) {
  ...
  tess.vao = R_CreateVao("tessVertexArray_VAO", NULL, vertexesSize, NULL,
                         indexesSize, VAO_USAGE_DYNAMIC);
  ...
  tess.vao->attribs[ATTR_INDEX_LIGHTDIRECTION].stride =
...
void R_VaoList_f(void) {
  ...
  for (i = 0; i < tr.numVaos; i++) {
    ...
    ri.Printf(PRINT_ALL, "%d.%02d MB %s\n", vao->vertexesSize / (1024 * 1024),
              (vao->vertexesSize % (1024 * 1024)) * 100 / (1024 * 1024),
              vao->name);
  ...
  for (i = 0; i < tr.numVaos; i++) {
    ...
    ri.Printf(PRINT_ALL, "%d.%02d MB %s\n", vao->indexesSize / (1024 * 1024),
              (vao->indexesSize % (1024 * 1024)) * 100 / (1024 * 1024),
              vao->name);
...
void RB_UpdateTessVao(unsigned int attribBits) {
  ...
  if (tess.numVertexes > 0 && tess.numVertexes <= SHADER_MAX_VERTEXES &&
    ...
    qglBufferData(GL_ELEMENT_ARRAY_BUFFER, tess.vao->indexesSize, NULL,
                  GL_DYNAMIC_DRAW);
...
// assuming each vert is referenced 4 times, need 16 bytes (4 glIndex_t) per
// vert
...
void VaoCache_Commit(void) {
  ...
  for (; batchLength < vc.batchLengths + vc.numBatches; batchLength++) {
    if (*batchLength == vcq.numSurfaces) {
      ...
      for (surf = vcq.surfaces; surf < end; surf++, indexSet2++) {
        if (surf->indexes != indexSet2->data ||
            (surf->numIndexes * sizeof(glIndex_t)) != indexSet2->size)
          break;
      }
  ...
  // FIXME: keep track of the vertexes so we don't have to reupload them every
  // time
  else {
    ...
    *batchLength = vcq.numSurfaces;
    ...
    for (surf = vcq.surfaces; surf < end; surf++) {
      ...
      int i, indexOffset =
                 (vc.vertexOffset + vcq.vertexCommitSize) / sizeof(srfVert_t);
    ...
    // vcq.vertexCommitSize, vc.vertexOffset, vcq.indexCommitSize,
    // vc.indexOffset, (int)(batchLength - vc.batchLengths));

    if (vcq.vertexCommitSize) {
      qglBindBuffer(GL_ARRAY_BUFFER, vc.vao->vertexesVBO);
      qglBufferSubData(GL_ARRAY_BUFFER, vc.vertexOffset, vcq.vertexCommitSize,
                       vcq.vertexes);
    ...
    if (vcq.indexCommitSize) {
      ...
      qglBufferSubData(GL_ELEMENT_ARRAY_BUFFER, vc.indexOffset,
                       vcq.indexCommitSize, vcq.indexes);
...
void VaoCache_Init(void) {
  vc.vao = R_CreateVao("VaoCache", NULL, VAOCACHE_VERTEX_BUFFER_SIZE, NULL,
                       VAOCACHE_INDEX_BUFFER_SIZE, VAO_USAGE_DYNAMIC);
  ...
  vc.vao->attribs[ATTR_INDEX_LIGHTDIRECTION].offset =
      offsetof(srfVert_t, lightdir);
...
void VaoCache_CheckAdd(qboolean *endSurface, qboolean *recycleVertexBuffer,
  ...
  if (vc.vao->indexesSize <
      vc.indexOffset + vcq.indexCommitSize + indexesSize) {
  ...
  if (VAOCACHE_QUEUE_MAX_VERTEXES * sizeof(srfVert_t) <
      vcq.vertexCommitSize + vertexesSize) {
    // ri.Printf(PRINT_ALL, "out of queued vertexes\n");
    *endSurface = qtrue;
  }

  if (VAOCACHE_QUEUE_MAX_INDEXES * sizeof(glIndex_t) <
      vcq.indexCommitSize + indexesSize) {
...
void VaoCache_RecycleIndexBuffer(void) {
  qglBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vc.vao->indexesIBO);
  qglBufferData(GL_ELEMENT_ARRAY_BUFFER, vc.vao->indexesSize, NULL,
                GL_DYNAMIC_DRAW);
  vc.indexOffset = 0;
  vc.numSurfaces = 0;
  vc.numBatches = 0;
