...
qboolean R_LoadIQM(model_t *mod, void *buffer, int filesize,
  ...
  if (header->num_meshes) {
    ...
    if (vertexArrayFormat[IQM_POSITION] == -1 ||
      ...
      ri.Printf(PRINT_WARNING,
                "R_LoadIQM: %s is missing IQM_POSITION, IQM_NORMAL, and/or "
                "IQM_TEXCOORD array.\n",
                mod_name);
      return qfalse;
    }
    ...
    if (header->num_joints) {
      if (vertexArrayFormat[IQM_BLENDINDEXES] == -1 ||
        ...
        ri.Printf(PRINT_WARNING,
                  "R_LoadIQM: %s is missing IQM_BLENDINDEXES and/or "
                  "IQM_BLENDWEIGHTS array.\n",
                  mod_name);
        return qfalse;
      }
    } else {
      ...
      if (header->num_joints) {
        for (j = 0; j < mesh->num_vertexes; j++) {
          ...
          for (k = 0; k < j; k++) {
            ...
            if (*(int *)&blendIndexes[4 * influence] !=
                *(int *)&blendIndexes[4 * vtx]) {
              continue;
            }

            if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
              if (blendWeights.f[4 * influence + 0] ==
                      blendWeights.f[4 * vtx + 0] &&
                  blendWeights.f[4 * influence + 1] ==
                      blendWeights.f[4 * vtx + 1] &&
                  blendWeights.f[4 * influence + 2] ==
                      blendWeights.f[4 * vtx + 2] &&
                  blendWeights.f[4 * influence + 3] ==
                      blendWeights.f[4 * vtx + 3]) {
                break;
              }
            } else {
              if (*(int *)&blendWeights.b[4 * influence] ==
  ...
  if (header->num_poses != header->num_joints && header->num_poses != 0) {
    ri.Printf(PRINT_WARNING,
              "R_LoadIQM: %s has %d poses and %d joints, must have the same "
              "number or 0 poses\n",
  ...
  if (header->num_meshes) {
    ...
    if (allocateInfluences) {
      ...
      if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_UBYTE) {
        ...
        dataPtr +=
            allocateInfluences * 4 * sizeof(byte); // influenceBlendWeights
      } else if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
        iqmData->influenceBlendWeights.f = (float *)dataPtr;
        dataPtr +=
            allocateInfluences * 4 * sizeof(float); // influenceBlendWeights
      }
    }
  }
  if (header->num_joints) {
  ...
  if (header->num_poses) {
    ...
    dataPtr += header->num_poses * header->num_frames * 12 *
               sizeof(float); // pose mats
  }
  if (header->ofs_bounds) {
  ...
  if (header->num_meshes) {
    ...
    for (i = 0; i < header->num_meshes; i++, mesh++, surface++) {
      ...
      Q_strlwr(
          surface
              ->name); // lowercase the surface name so skin compares are faster
      surface->shader =
      ...
      for (i = 0; i < header->num_meshes; i++, surface++) {
        ...
        for (j = 0; j < surface->num_vertexes; j++) {
          ...
          for (k = 0; k < surface->num_influences; k++) {
            ...
            if (*(int *)&iqmData->influenceBlendIndexes[4 * influence] !=
                *(int *)&blendIndexes[4 * vtx]) {
              continue;
            }

            if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
              if (iqmData->influenceBlendWeights.f[4 * influence + 0] ==
                      blendWeights.f[4 * vtx + 0] &&
                  iqmData->influenceBlendWeights.f[4 * influence + 1] ==
                      blendWeights.f[4 * vtx + 1] &&
                  iqmData->influenceBlendWeights.f[4 * influence + 2] ==
                      blendWeights.f[4 * vtx + 2] &&
                  iqmData->influenceBlendWeights.f[4 * influence + 3] ==
                      blendWeights.f[4 * vtx + 3]) {
                break;
              }
            } else {
          ...
          if (k == surface->num_influences) {
            ...
            iqmData->influenceBlendIndexes[4 * influence + 0] =
                blendIndexes[4 * vtx + 0];
            iqmData->influenceBlendIndexes[4 * influence + 1] =
                blendIndexes[4 * vtx + 1];
            iqmData->influenceBlendIndexes[4 * influence + 2] =
                blendIndexes[4 * vtx + 2];
            iqmData->influenceBlendIndexes[4 * influence + 3] =
                blendIndexes[4 * vtx + 3];

            if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
              iqmData->influenceBlendWeights.f[4 * influence + 0] =
                  blendWeights.f[4 * vtx + 0];
              iqmData->influenceBlendWeights.f[4 * influence + 1] =
                  blendWeights.f[4 * vtx + 1];
              iqmData->influenceBlendWeights.f[4 * influence + 2] =
              ...
              iqmData->influenceBlendWeights.b[4 * influence + 0] =
                  blendWeights.b[4 * vtx + 0];
              iqmData->influenceBlendWeights.b[4 * influence + 1] =
                  blendWeights.b[4 * vtx + 1];
              iqmData->influenceBlendWeights.b[4 * influence + 2] =
                  blendWeights.b[4 * vtx + 2];
              iqmData->influenceBlendWeights.b[4 * influence + 3] =
                  blendWeights.b[4 * vtx + 3];
            }
  ...
  if (iqmData->num_surfaces &&
    ...
    iqmData->vaoSurfaces = ri.Hunk_Alloc(
        sizeof(*iqmData->vaoSurfaces) * iqmData->numVaoSurfaces, h_low);
    ...
    for (i = 0; i < iqmData->num_surfaces; i++, vaoSurf++, surf++) {
      ...
      for (j = 0; j < surf->num_vertexes; j++) {
        ...
        // normal
        R_VaoPackNormal((int16_t *)(data + dataOfs),
                        &iqmData->normals[vtx * 3]);
        dataOfs += sizeof(int16_t) * 4;

        // tangent
        R_VaoPackTangent((int16_t *)(data + dataOfs),
      ...
      if (iqmData->num_joints) {
        ...
        if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
          ...
          vaoSurf->vao->attribs[ATTR_INDEX_BONE_WEIGHTS].type =
              GL_UNSIGNED_BYTE;
  ...
  if (data->num_poses > 0) {
    ...
    for (i = 0; i < surf->num_influences; i++) {
      ...
      for (numWeights = 0; numWeights < 4; numWeights++) {
        if (data->blendWeightsType == IQM_FLOAT)
          blendWeights[numWeights] =
              data->influenceBlendWeights.f[4 * influence + numWeights];
        else
          blendWeights[numWeights] =
              (float)data->influenceBlendWeights.b[4 * influence + numWeights] /
              255.0f;
      ...
      if (numWeights == 0) {
        ...
        vtxMat[0] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 0];
        vtxMat[1] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 1];
        vtxMat[2] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 2];
        vtxMat[3] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 3];
        vtxMat[4] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 4];
        vtxMat[5] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 5];
        vtxMat[6] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 6];
        vtxMat[7] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 7];
        vtxMat[8] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 8];
        vtxMat[9] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 9];
        vtxMat[10] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 10];
        vtxMat[11] =
            blendWeights[0] *
            poseMats[12 * data->influenceBlendIndexes[4 * influence + 0] + 11];

        for (j = 1; j < numWeights; j++) {
          vtxMat[0] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 0];
          vtxMat[1] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 1];
          vtxMat[2] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 2];
          vtxMat[3] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 3];
          vtxMat[4] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 4];
          vtxMat[5] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 5];
          vtxMat[6] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 6];
          vtxMat[7] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 7];
          vtxMat[8] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 8];
          vtxMat[9] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] + 9];
          vtxMat[10] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] +
                       10];
          vtxMat[11] +=
              blendWeights[j] *
              poseMats[12 * data->influenceBlendIndexes[4 * influence + j] +
    ...
    for (i = 0; i < surf->num_vertexes; i++, xyz += 3, normal += 3,
        tangent += 4, texCoords += 2, outXYZ++, outNormal += 4, outTangent += 4,
        outTexCoord++) {
      int influence =
      ...
      (*outXYZ)[0] = vtxMat[0] * xyz[0] + vtxMat[1] * xyz[1] +
                     vtxMat[2] * xyz[2] + vtxMat[3];
      (*outXYZ)[1] = vtxMat[4] * xyz[0] + vtxMat[5] * xyz[1] +
                     vtxMat[6] * xyz[2] + vtxMat[7];
      (*outXYZ)[2] = vtxMat[8] * xyz[0] + vtxMat[9] * xyz[1] +
                     vtxMat[10] * xyz[2] + vtxMat[11];
