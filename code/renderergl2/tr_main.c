...
void R_CalcTexDirs(vec3_t sdir, vec3_t tdir, const vec3_t v1, const vec3_t v2,
                   const vec3_t v3, const vec2_t w1, const vec2_t w2,
                   const vec2_t w3) {
...
=============
*/
vec_t R_CalcTangentSpace(vec3_t tangent, vec3_t bitangent, const vec3_t normal,
                         const vec3_t sdir, const vec3_t tdir) {
  vec3_t n_cross_t;
...
qboolean R_CalcTangentVectors(srfVert_t *dv[3]) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    s = dv[i]->st[0] + 10.0f;
    t = dv[i]->st[1];
    bary[0] = ((dv[1]->st[0] - s) * (dv[2]->st[1] - t) -
               (dv[2]->st[0] - s) * (dv[1]->st[1] - t)) /
              bb;
    bary[1] = ((dv[2]->st[0] - s) * (dv[0]->st[1] - t) -
               (dv[0]->st[0] - s) * (dv[2]->st[1] - t)) /
              bb;
    bary[2] = ((dv[0]->st[0] - s) * (dv[1]->st[1] - t) -
               (dv[1]->st[0] - s) * (dv[0]->st[1] - t)) /
              bb;
    ...
    bitangent[0] = bary[0] * dv[0]->xyz[0] + bary[1] * dv[1]->xyz[0] +
                   bary[2] * dv[2]->xyz[0];
    bitangent[1] = bary[0] * dv[0]->xyz[1] + bary[1] * dv[1]->xyz[1] +
                   bary[2] * dv[2]->xyz[1];
    bitangent[2] = bary[0] * dv[0]->xyz[2] + bary[1] * dv[1]->xyz[2] +
                   bary[2] * dv[2]->xyz[2];
    ...
    //%     stv[ i ][ 0 ], stv[ i ][ 1 ], stv[ i ][ 2 ], ttv[ i ][ 0 ], ttv[ i
    //][ 1 ], ttv[ i ][ 2 ] );
  }
...
*/
int R_CullPointAndRadiusEx(const vec3_t pt, float radius,
                           const cplane_t *frustum, int numPlanes) {
  int i;
  float dist;
...
int R_CullPointAndRadius(const vec3_t pt, float radius) {
  return R_CullPointAndRadiusEx(pt, radius, tr.viewParms.frustum,
                                (tr.viewParms.flags & VPF_FARPLANEFRUSTUM) ? 5
                                                                           : 4);
}
...
void R_SetupFrustum(viewParms_t *dest, float xmin, float xmax, float ymax,
                    float zProj, float zFar, float stereoSep) {
  vec3_t ofsorigin;
  ...
  if (stereoSep == 0 && xmin == -xmax) {
    ...
    // In stereo rendering, due to the modification of the projection matrix,
    // dest->or.origin is not the actual origin that we're rendering so offset
    // the tip of the view pyramid.
...
void R_SetupProjectionZ(viewParms_t *dest) {
  ...
  if (dest->isPortal) {
    ...
    // Lengyel, Eric. "Modifying the Projection Matrix to Perform Oblique
    // Near-plane Clipping". Terathon Software 3D Graphics Library, 2004.
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, int fogIndex,
                   int dlightMap, int pshadowMap, int cubemap) {
  int index;
...
void R_SortDrawSurfs(drawSurf_t *drawSurfs, int numDrawSurfs) {
  ...
  // - tr.refdef.drawSurfs), numDrawSurfs);
...
void R_GenerateDrawSurfs(void) {
  ...
  // we know the size of the clipping volume. Now set the rest of the projection
  // matrix.
...
void R_RenderDlightCubemaps(const refdef_t *fd) {
  ...
  for (i = 0; i < tr.refdef.num_dlights; i++) {
    ...
    shadowParms.viewportY =
        glConfig.vidHeight - (tr.refdef.y + PSHADOW_MAP_SIZE);
...
void R_RenderPshadowMaps(const refdef_t *fd) {
    ...
    for (j = i + 1; j < tr.refdef.num_pshadows; j++) {
      ...
      if (touch) {
        ...
        BoundingSphereOfSpheres(ps1->viewOrigin, ps1->viewRadius,
                                ps2->viewOrigin, ps2->viewRadius, newOrigin,
                                &newRadius);
  ...
  for (i = 0; i < tr.refdef.num_pshadows; i++) {
    ...
    shadow->cullPlane.dist =
        DotProduct(shadow->cullPlane.normal, shadow->lightOrigin);
  ...
  for (i = 0; i < tr.refdef.num_pshadows; i++) {
...

    {
...

      {
        ...
        VectorScale(dest->or.axis[1], 1.0f, dest->frustum[0].normal);
        VectorMA(dest->or.origin, -shadow->viewRadius, dest->frustum[0].normal,
                 pop);
        dest->frustum[0].dist = DotProduct(pop, dest->frustum[0].normal);

        VectorScale(dest->or.axis[1], -1.0f, dest->frustum[1].normal);
        VectorMA(dest->or.origin, -shadow->viewRadius, dest->frustum[1].normal,
                 pop);
        dest->frustum[1].dist = DotProduct(pop, dest->frustum[1].normal);

        VectorScale(dest->or.axis[2], 1.0f, dest->frustum[2].normal);
        VectorMA(dest->or.origin, -shadow->viewRadius, dest->frustum[2].normal,
                 pop);
        dest->frustum[2].dist = DotProduct(pop, dest->frustum[2].normal);

        VectorScale(dest->or.axis[2], -1.0f, dest->frustum[3].normal);
        VectorMA(dest->or.origin, -shadow->viewRadius, dest->frustum[3].normal,
                 pop);
        dest->frustum[3].dist = DotProduct(pop, dest->frustum[3].normal);
...
void R_RenderSunShadowMaps(const refdef_t *fd, int level) {
  ...
  if (r_forceSun->integer == 2) {
    ...
    {
      lightCol[0] = lightCol[1] = lightCol[2] =
          CLAMP(sin(angle) * 2.0f, 0.0f, 1.0f) * 2.0f;
      lightCol[3] = 1.0f;
    } else {
      lightCol[0] = lightCol[1] = lightCol[2] =
          CLAMP(sin(angle) * 2.0f * 0.1f, 0.0f, 0.1f);
      lightCol[3] = 1.0f;
    }
  ...
  if (level == 3 || lightViewIndependentOfCameraView) {
    ...
    // Level 0 tries to use a diamond texture orientation relative to camera
    // view Use halfway between camera view forward and left for light view up
  ...
  if (fabsf(DotProduct(lightViewAxis[2], lightViewAxis[0])) > 0.9f) {
    if (level == 3 || lightViewIndependentOfCameraView) {
      ...
      // Level 0 tries to use a diamond texture orientation relative to camera
      // view Use halfway between camera view forward and up for light view up
  ...
  // Create bounds for light projection using slice of view projection
  {
    ...
    if (lightViewIndependentOfCameraView) {
      ...
      cascadeBound =
          MAX(cascadeBound, lightviewBounds[1][2] - lightviewBounds[0][2]);
      worldUnitsPerTexel = cascadeBound / tr.sunShadowFbo[level]->width;
      invWorldUnitsPerTexel = 1.0f / worldUnitsPerTexel;

      VectorScale(lightviewBounds[0], invWorldUnitsPerTexel,
                  lightviewBounds[0]);
      ...
      VectorScale(lightviewBounds[1], invWorldUnitsPerTexel,
                  lightviewBounds[1]);
    ...
    // lightviewBounds[0][0], lightviewBounds[1][0]); ri.Printf(PRINT_ALL, "xmin
    // %f xmax %f ymin %f ymax %f\n", lightviewBounds[0][1],
    // lightviewBounds[1][1], -lightviewBounds[1][2], -lightviewBounds[0][2]);
  }

  {
    ...
    if (glRefConfig.framebufferObject) {
      ...
      shadowParms.viewportX = tr.refdef.x;
      shadowParms.viewportY =
          glConfig.vidHeight - (tr.refdef.y + tr.sunShadowFbo[level]->height);
    }
...
void R_RenderCubemapSide(int cubemapIndex, int cubemapSide, qboolean subscene) {
  ...
  if (!subscene) {
    ...
    // fix involves changing r_FBufScale to fit smaller cubemap image size, or
    // rendering cubemap to framebuffer first
    if (0) //(glRefConfig.framebufferObject && r_sunlightMode->integer &&
...

  {
    ...
    if (scale < 1.0001f && cubemapSide == 0) {
      ri.Printf(PRINT_ALL,
                "cubemap %d %s (%f, %f, %f) is outside the lightgrid or inside "
                "a wall!\n",
                cubemapIndex, tr.cubemaps[cubemapIndex].name,
                tr.refdef.vieworg[0], tr.refdef.vieworg[1],
                tr.refdef.vieworg[2]);
    }
  }
