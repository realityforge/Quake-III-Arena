...
void RB_BeginDrawingView(void) {
  ...
  if (glRefConfig.framebufferObject) {
    ...
    // FIXME: HUGE HACK: render to the screen fbo if we've already postprocessed
    // the frame and aren't drawing more world drawing more world check is in
    // case of double renders, such as skyportals
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    ...
    // separate entities merged into a single batch, like smoke and blood puff
    // sprites
    if (shader != NULL &&
    ...
    if (entityNum != oldEntityNum) {
      ...
      if (entityNum != REFENTITYNUM_WORLD) {
        ...
        // FIXME: e.shaderTime must be passed as int to avoid fp-precision loss
        // issues
        backEnd.refdef.floatTime =
            originalTime - (double)backEnd.currentEntity->e.shaderTime;
...
void RE_StretchRaw(int x, int y, int w, int h, int cols, int rows,
  ...
  GLSL_SetUniformMat4(&tr.textureColorShader, UNIFORM_MODELVIEWPROJECTIONMATRIX,
                      glState.modelviewProjection);
...
void RE_UploadCinematic(int w, int h, int cols, int rows, const byte *data,
  ...
  if (!tr.scratchImage[client]) {
    ri.Printf(PRINT_WARNING,
              "RE_UploadCinematic: scratch images not initialized\n");
    return;
  }
  ...
  if (glRefConfig.framebufferObject &&
    ...
    if (!isShadowView) {
      if (tr.msaaResolveFbo) {
        ...
        // If we're rendering directly to the screen, copy the depth to a
        // texture This is incredibly slow on Intel Graphics, so just skip it on
        // there
        if (!glRefConfig.intelGraphics)
          qglCopyTextureSubImage2DEXT(tr.renderDepthImage->texnum,
                                      GL_TEXTURE_2D, 0, 0, 0, 0, 0,
                                      glConfig.vidWidth, glConfig.vidHeight);
      }

      if (tr.hdrDepthFbo) {
        // need the depth in a texture we can do GL_LINEAR sampling on, so copy
        // it to an HDR image
        vec4_t srcTexCoords;

        VectorSet4(srcTexCoords, 0.0f, 0.0f, 1.0f, 1.0f);

        FBO_BlitFromTexture(tr.renderDepthImage, srcTexCoords, NULL,
                            tr.hdrDepthFbo, NULL, NULL, NULL, 0);
      }
      ...
      if (r_sunlightMode->integer &&
        ...
        box[2] =
            box[0] + backEnd.viewParms.viewportWidth / (float)glConfig.vidWidth;
        box[3] = box[1] +
                            ...
                            backEnd.refdef.vieworg);
        {
          ...
          GLSL_SetUniformVec3(&tr.shadowmaskShader, UNIFORM_VIEWFORWARD,
                              viewVector);
          VectorScale(backEnd.refdef.viewaxis[1], xmax, viewVector);
          GLSL_SetUniformVec3(&tr.shadowmaskShader, UNIFORM_VIEWLEFT,
                              viewVector);
        ...
        if (r_shadowBlur->integer) {
          ...
          GLSL_SetUniformVec4(&tr.depthBlurShader[0], UNIFORM_VIEWINFO,
                              viewInfo);
          ...
          GLSL_SetUniformVec4(&tr.depthBlurShader[1], UNIFORM_VIEWINFO,
                              viewInfo);
      ...
      if (r_ssao->integer) {
        ...
        viewInfo[2] =
            1.0f / ((float)(tr.quarterImage[0]->width) *
                    tan(backEnd.viewParms.fovX * M_PI / 360.0f) * 2.0f);
        viewInfo[3] =
            1.0f / ((float)(tr.quarterImage[0]->height) *
                    tan(backEnd.viewParms.fovY * M_PI / 360.0f) * 2.0f);
  ...
  if (glRefConfig.framebufferObject) {
    if (!backEnd.framePostProcessed) {
      if (tr.msaaResolveFbo && r_hdr->integer) {
        // Resolving an RGB16F MSAA FBO to the screen messes with the
        // brightness, so resolve to an RGB16F FBO first
        FBO_FastBlit(tr.renderFbo, NULL, tr.msaaResolveFbo, NULL,
                     GL_COLOR_BUFFER_BIT, GL_NEAREST);
        FBO_FastBlit(tr.msaaResolveFbo, NULL, NULL, NULL, GL_COLOR_BUFFER_BIT,
                     GL_NEAREST);
      } else if (tr.renderFbo) {
        FBO_FastBlit(tr.renderFbo, NULL, NULL, NULL, GL_COLOR_BUFFER_BIT,
                     GL_NEAREST);
      }
    }
  }
  ...
  if (cmd->map != -1) {
    if (cmd->cubeSide != -1) {
      if (tr.shadowCubemaps[cmd->map]) {
        qglCopyTextureSubImage2DEXT(
            tr.shadowCubemaps[cmd->map]->texnum,
            GL_TEXTURE_CUBE_MAP_POSITIVE_X + cmd->cubeSide, 0, 0, 0,
            backEnd.refdef.x,
      ...
      if (tr.pshadowMaps[cmd->map]) {
        qglCopyTextureSubImage2DEXT(tr.pshadowMaps[cmd->map]->texnum,
                                    ...
                                    glConfig.vidHeight -
                                        (backEnd.refdef.y + PSHADOW_MAP_SIZE),
                                    PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
      }
    }
  }
...
const void *RB_PostProcess(const void *data) {
  ...
  if (tr.msaaResolveFbo) {
    ...
    // Can't resolve just part of the MSAA FBO, so multiple views will suffer a
  ...
  if (srcFbo) {
    if (r_hdr->integer && (r_toneMap->integer || r_forceToneMap->integer)) {
      ...
      FBO_FastBlit(srcFbo, srcBox, NULL, dstBox, GL_COLOR_BUFFER_BIT,
                   GL_NEAREST);
    } else {
      vec4_t color;

      color[0] = color[1] = color[2] =
          pow(2, r_cameraExposure->value); // exp2(r_cameraExposure->value);
      color[3] = 1.0f;
  ...
  if (0 && r_sunlightMode->integer) {
    ...
    FBO_BlitFromTexture(tr.sunShadowDepthImage[0], NULL, NULL, NULL, dstBox,
                        NULL, NULL, 0);
    VectorSet4(dstBox, 128, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[1], NULL, NULL, NULL, dstBox,
                        NULL, NULL, 0);
    VectorSet4(dstBox, 256, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[2], NULL, NULL, NULL, dstBox,
                        NULL, NULL, 0);
    VectorSet4(dstBox, 384, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.sunShadowDepthImage[3], NULL, NULL, NULL, dstBox,
                        NULL, NULL, 0);
  }
  ...
  if (0 && r_shadows->integer == 4) {
    ...
    FBO_BlitFromTexture(tr.pshadowMaps[0], NULL, NULL, NULL, dstBox, NULL, NULL,
                        0);
    VectorSet4(dstBox, 512 + 128, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[1], NULL, NULL, NULL, dstBox, NULL, NULL,
                        0);
    VectorSet4(dstBox, 512 + 256, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[2], NULL, NULL, NULL, dstBox, NULL, NULL,
                        0);
    VectorSet4(dstBox, 512 + 384, glConfig.vidHeight - 128, 128, 128);
    FBO_BlitFromTexture(tr.pshadowMaps[3], NULL, NULL, NULL, dstBox, NULL, NULL,
                        0);
  }
  ...
  if (0) {
    ...
    FBO_BlitFromTexture(tr.renderDepthImage, NULL, NULL, NULL, dstBox, NULL,
                        NULL, 0);
    VectorSet4(dstBox, 512, glConfig.vidHeight - 256, 256, 256);
    FBO_BlitFromTexture(tr.screenShadowImage, NULL, NULL, NULL, dstBox, NULL,
                        NULL, 0);
  }
  ...
  if (0) {
    ivec4_t dstBox;
    VectorSet4(dstBox, 256, glConfig.vidHeight - 256, 256, 256);
    FBO_BlitFromTexture(tr.sunRaysImage, NULL, NULL, NULL, dstBox, NULL, NULL,
                        0);
  }
...
void R_SaveDDS(const char *filename, byte *pic, int width, int height,
               int depth);
...
const void *RB_ExportCubemaps(const void *data) {
  ...
  if (cmd) {
    ...
    for (i = 0; i < tr.numCubemaps; i++) {
      ...
      for (j = 0; j < 6; j++) {
        FBO_AttachImage(tr.renderCubeFbo, cubemap->image,
