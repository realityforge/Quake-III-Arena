...
void R_ImageList_f(void) {
  ...
  for (i = 0; i < tr.numImages; i++) {
    ...
    ri.Printf(PRINT_ALL, "%4i: %4ix%4i %s %4i%s %s\n", i, image->uploadWidth,
              image->uploadHeight, format, displaySize, sizeSuffix,
              image->imgName);
...
static void RGBAtoNormal(const byte *in, byte *out, int width, int height,
...
static void DoFCBI(byte *in, byte *out, int width, int height, int component) {
  ...
  for (y = 3; y < height - 3; y += 2) {
    ...
    for (; x < width - 3; x += 2) {
      ...
      // SAMPLE2(sc, x-3, y-1); SAMPLE2(sd, x-1, y-1); SAMPLE2(se, x+1, y-1);
      // SAMPLE2(sf, x+3, y-1); SAMPLE2(sg, x-3, y+1); SAMPLE2(sh, x-1, y+1);
      // SAMPLE2(si, x+1, y+1); SAMPLE2(sj, x+3, y+1);
  ...
  for (y = 2; y < height - 3; y++) {
    ...
    for (; x < width - 3; x += 2) {
      ...
      if (hd > 100 || vd > 100 || abs(hp - vp) > 200) {
        ...
        // hdd = abs(sc[i] + sd[i] + se[i] - 3 * (sf[i] + sg[i]) + sh[i] + si[i]
        // + sj[i]); vdd = abs(sa[i] + sf[i] + sk[i] - 3 * (sd[i] + si[i]) +
        // sb[i] + sg[i] + sl[i]);
...
static void FillInNormalizedZ(const byte *in, byte *out, int width,
                              int height) {
  int x, y;
  ...
  for (y = 0; y < height; y += WORKBLOCK_SIZE) {
    for (x = 0; x < width; x += WORKBLOCK_SIZE) {
      ...
      for (y2 = 0; y2 < workheight; y2++) {
        inbyte =
            outdata +
            ((y2 + WORKBLOCK_BORDER) * fullworkwidth + WORKBLOCK_BORDER) * 4;
...
================
*/
void R_LightScaleTexture(byte *in, int inwidth, int inheight,
                         qboolean only_gamma) {
  if (only_gamma) {
...
static void R_MipMapsRGB(byte *in, int inWidth, int inHeight) {
  ...
  if (inWidth == 1 || inHeight == 1) {
    for (x = (inWidth * inHeight) >> 1; x; x--) {
      for (c = 3; c; c--, in++) {
        total =
            (downmipSrgbLookup[*(in)] + downmipSrgbLookup[*(in + 4)]) * 2.0f;
...
static void R_MipMapNormalHeight(const byte *in, byte *out, int width,
                                 int height, qboolean swizzle) {
  int i, j;
  int row;
  int sx = swizzle ? 3 : 0;
  int sa = swizzle ? 0 : 3;
  ...
  for (i = 0; i < height; i++, in += row) {
    for (j = 0; j < width; j++, out += 4, in += 8) {
      ...
      out[sa] =
          MAX(MAX(in[sa], in[sa + 4]), MAX(in[sa + row], in[sa + row + 4]));
    }
  }
}
...
===============
*/
static qboolean RawImage_ScaleToPower2(byte **data, int *inout_width,
                                       int *inout_height, imgType_t type,
                                       imgFlags_t flags,
  ...
  if (picmip && data && resampledBuffer && r_imageUpsample->integer &&
    ...
    *resampledBuffer = ri.Hunk_AllocateTempMemory(finalwidth * finalheight * 4);
    ...
    // scaled_width, scaled_height, endTime - startTime);
...
static GLenum RawImage_GetFormat(const byte *data, int numPixels,
                                 GLenum picFormat, qboolean lightMap,
                                 imgType_t type, imgFlags_t flags) {
  int samples = 3;
...
static void RawImage_UploadToRgtc2Texture(GLuint texture, int miplevel, int x,
    ...
    for (ix = 0; ix < width; ix += 4) {
      ...
      for (component = 0; component < 2; component++) {
        ...
        for (oy = 0; oy < oh; oy++)
          for (ox = 0; ox < ow; ox++)
            workingData[oy * 4 + ox] =
                data[((iy + oy) * width + ix + ox) * 4 + component];
  ...
  qglCompressedTextureSubImage2DEXT(texture, GL_TEXTURE_2D, miplevel, x, y,
                                    width, height, GL_COMPRESSED_RG_RGTC2, size,
                                    compressedData);
...
static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y,
                                   ...
                                   GLenum picFormat, int numMips,
                                   GLenum internalFormat, imgType_t type,
...
static void Upload32(byte *data, int x, int y, int width, int height,
                     GLenum picFormat, int numMips, image_t *image,
                     qboolean scaled) {
  int i, c;
  byte *scan;
  ...
  if (cubemap) {
    for (i = 0; i < 6; i++) {
      ...
      RawImage_UploadTexture(image->texnum, data, x, y, width, height,
                             GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, picFormat,
                             numMips, internalFormat, type, flags, qfalse);
      for (c = numMips; c; c--) {
    ...
    RawImage_UploadTexture(image->texnum, data, x, y, width, height,
                           GL_TEXTURE_2D, picFormat, numMips, internalFormat,
                           type, flags, qfalse);
  }
...
================
*/
image_t *R_CreateImage2(const char *name, byte *pic, int width, int height,
                        GLenum picFormat, int numMips, imgType_t type,
    ...
    if (cubemap) {
      ...
      for (i = 0; i < 6; i++)
        qglTextureImage2DEXT(image->texnum, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                             miplevel, internalFormat, mipWidth, mipHeight, 0,
                             dataFormat, GL_UNSIGNED_BYTE, NULL);
    } else {
      qglTextureImage2DEXT(image->texnum, GL_TEXTURE_2D, miplevel,
                           internalFormat, mipWidth, mipHeight, 0, dataFormat,
  ...
  if (textureFilterAnisotropic && !cubemap)
    qglTextureParameteriEXT(
        image->texnum, textureTarget, GL_TEXTURE_MAX_ANISOTROPY_EXT,
        mipmap
  ...
  case GL_DEPTH_COMPONENT32_ARB:
    ...
    qglTextureParameterfEXT(image->texnum, textureTarget, GL_DEPTH_TEXTURE_MODE,
                            GL_LUMINANCE);
...
void R_UpdateSubImage(image_t *image, byte *pic, int x, int y, int width,
...
void R_LoadImage(const char *name, byte **pic, int *width, int *height,
  ...
  *numMips = 0;
  ...
  // If compressed textures are enabled, try loading a DDS first, it'll load
  // fastest
        ...
        for (y = 1; y < height - 1; y++) {
          ...
          for (x = 1; x < width - 1; x++) {
            ...
            result = *(picbyte - (width + 1) * 4) + *(picbyte - width * 4) +
                     *(picbyte - (width - 1) * 4) + *(picbyte - 1 * 4) +
                     *(picbyte) + *(picbyte + 1 * 4) +
                     *(picbyte + (width - 1) * 4) + *(picbyte + width * 4) +
                     *(picbyte + (width + 1) * 4);
...
void R_CreateBuiltinImages(void) {
  ...
  for (x = 0; x < 32; x++) {
    ...
    tr.scratchImage[x] = R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE,
                                       DEFAULT_SIZE, IMGTYPE_COLORALPHA,
  ...
  if (glRefConfig.framebufferObject) {
    ...
    tr.renderImage =
        R_CreateImage("_render", NULL, width, height, IMGTYPE_COLORALPHA,
                      IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);

    if (r_shadowBlur->integer)
      tr.screenScratchImage = R_CreateImage(
          "screenScratch", NULL, width, height, IMGTYPE_COLORALPHA,
          IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, rgbFormat);

    if (r_shadowBlur->integer || r_ssao->integer)
      tr.hdrDepthImage =
          R_CreateImage("*hdrDepth", NULL, width, height, IMGTYPE_COLORALPHA,
...

    {
      ...
      tr.calcLevelsImage = R_CreateImage(
          "*calcLevels", p, 1, 1, IMGTYPE_COLORALPHA,
          IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
      tr.targetLevelsImage = R_CreateImage(
          "*targetLevels", p, 1, 1, IMGTYPE_COLORALPHA,
          IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
      tr.fixedLevelsImage = R_CreateImage(
          "*fixedLevels", p, 1, 1, IMGTYPE_COLORALPHA,
          IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
    }
    ...
    for (x = 0; x < 2; x++) {
      tr.quarterImage[x] = R_CreateImage(
          va("*quarter%d", x), NULL, width / 2, height / 2, IMGTYPE_COLORALPHA,
          IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
    }
    ...
    for (x = 0; x < MAX_DRAWN_PSHADOWS; x++) {
      tr.pshadowMaps[x] = R_CreateImage(
          va("*shadowmap%i", x), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE,
          IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE,
          GL_DEPTH_COMPONENT24);
    ...
    if (r_sunlightMode->integer) {
      for (x = 0; x < 4; x++) {
        tr.sunShadowDepthImage[x] = R_CreateImage(
            ...
            IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24);
        qglTextureParameterfEXT(tr.sunShadowDepthImage[x]->texnum,
    ...
    if (r_cubeMapping->integer) {
      tr.renderCubeImage =
          R_CreateImage("*renderCube", NULL, r_cubemapSize->integer,
                        r_cubemapSize->integer, IMGTYPE_COLORALPHA,
                        IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE |
                            IMGFLAG_MIPMAP | IMGFLAG_CUBEMAP,
                        rgbFormat);
    }
  }
}
...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  if (totalSurfaces > MAX_SKIN_SURFACES) {
    ri.Printf(PRINT_WARNING,
              "WARNING: Ignoring excess surfaces (found %d, max is %d) in skin "
              "'%s'!\n",
  ...
  skin->surfaces =
      ri.Hunk_Alloc(skin->numSurfaces * sizeof(skinSurface_t), h_low);
