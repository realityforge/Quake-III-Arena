...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts,
  ...
  if (!hShader) {
    // This isn't a useful warning, and an hShader of zero isn't a null shader,
    // it's the default shader. ri.Printf( PRINT_WARNING, "WARNING:
...
void RE_AddRefEntityToScene(const refEntity_t *ent) {
  ...
  backEndData->entities[r_numentities].mirrored =
      (DotProduct(ent->axis[2], cross) < 0.f);
  ...
  if ((tr.refdef.rdflags & RDF_NOWORLDMODEL) || !(r_depthPrepass->value)) {
    ...
    if (r_sunlightMode->integer == 1) {
      tr.refdef.sunAmbCol[0] = tr.refdef.sunAmbCol[1] = tr.refdef.sunAmbCol[2] =
          r_forceSun->integer ? r_forceSunAmbientScale->value
                              : tr.sunShadowScale;
    } else {
      if (r_forceSun->integer)
        ...
        VectorScale(tr.sunLight, scale * tr.sunShadowScale,
                    tr.refdef.sunAmbCol);
    }
  }
