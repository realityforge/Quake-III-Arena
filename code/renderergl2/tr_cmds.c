...
void R_PerformanceCounters(void) {
  ...
  if (r_speeds->integer == 1) {
    ri.Printf(PRINT_ALL,
              "%i/%i/%i shaders/batches/surfs %i leafs %i verts %i/%i tris "
              "%.2f mtex %.2f dc\n",
              backEnd.pc.c_shaders, backEnd.pc.c_surfBatches,
              backEnd.pc.c_surfaces, tr.pc.c_leafs, backEnd.pc.c_vertexes,
...
void *R_GetCommandBufferReserved(int bytes, int reservedBytes) {
  ...
  if (cmdList->used + bytes + sizeof(int) + reservedBytes >
      MAX_RENDER_COMMANDS) {
...
void *R_GetCommandBuffer(int bytes) {
  return R_GetCommandBufferReserved(
      bytes, PAD(sizeof(swapBuffersCommand_t), sizeof(void *)));
}
