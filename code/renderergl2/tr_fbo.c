...
qboolean R_CheckFBO(const FBO_t *fbo) {
  GLenum code =
      qglCheckNamedFramebufferStatusEXT(fbo->frameBuffer, GL_FRAMEBUFFER);
  ...
  case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
    ri.Printf(PRINT_WARNING,
              "R_CheckFBO: (%s) Framebuffer incomplete multisample\n",
              fbo->name);
    break;
...
void FBO_Init(void) {
  ...
  if (multisample && glRefConfig.framebufferMultisample) {
    ...
    tr.msaaResolveFbo = FBO_Create("_msaaResolve", tr.renderDepthImage->width,
                                   tr.renderDepthImage->height);
    FBO_AttachImage(tr.msaaResolveFbo, tr.renderImage, GL_COLOR_ATTACHMENT0, 0);
    FBO_AttachImage(tr.msaaResolveFbo, tr.renderDepthImage, GL_DEPTH_ATTACHMENT,
                    0);
  ...
  if (tr.screenScratchImage) {
    tr.screenScratchFbo =
        FBO_Create("screenScratch", tr.screenScratchImage->width,
                   tr.screenScratchImage->height);
  ...
  if (MAX_DRAWN_PSHADOWS && tr.pshadowMaps[0]) {
    for (i = 0; i < MAX_DRAWN_PSHADOWS; i++) {
      tr.pshadowFbos[i] =
          FBO_Create(va("_shadowmap%d", i), tr.pshadowMaps[i]->width,
                     tr.pshadowMaps[i]->height);
      // FIXME: this next line wastes 16mb with 16x512x512 sun shadow maps, skip
      // if OpenGL 4.3+ or ARB_framebuffer_no_attachments
  ...
  if (tr.sunShadowDepthImage[0]) {
    for (i = 0; i < 4; i++) {
      tr.sunShadowFbo[i] =
          FBO_Create("_sunshadowmap", tr.sunShadowDepthImage[i]->width,
                     tr.sunShadowDepthImage[i]->height);
      // FIXME: this next line wastes 16mb with 4x1024x1024 sun shadow maps,
      // skip if OpenGL 4.3+ or ARB_framebuffer_no_attachments This at least
      // gets sun shadows working on older GPUs (Intel)
  ...
  if (tr.screenShadowImage) {
    tr.screenShadowFbo =
        FBO_Create("_screenshadow", tr.screenShadowImage->width,
  ...
  if (tr.textureScratchImage[0]) {
    for (i = 0; i < 2; i++) {
      tr.textureScratchFbo[i] = FBO_Create(va("_texturescratch%d", i),
                                           tr.textureScratchImage[i]->width,
  ...
  if (tr.targetLevelsImage) {
    tr.targetLevelsFbo =
        FBO_Create("_targetlevels", tr.targetLevelsImage->width,
  ...
  if (tr.quarterImage[0]) {
    for (i = 0; i < 2; i++) {
      tr.quarterFbo[i] =
          FBO_Create(va("_quarter%d", i), tr.quarterImage[i]->width,
...
void FBO_Blit(FBO_t *src, ivec4_t inSrcBox, vec2_t srcTexScale, FBO_t *dst,
              ivec4_t dstBox, struct shaderProgram_s *shaderProgram,
              vec4_t color, int blend) {
...
void FBO_FastBlit(FBO_t *src, ivec4_t srcBox, FBO_t *dst, ivec4_t dstBox,
  ...
  qglBlitFramebuffer(srcBoxFinal[0], srcBoxFinal[1], srcBoxFinal[2],
                     srcBoxFinal[3], dstBoxFinal[0], dstBoxFinal[1],
