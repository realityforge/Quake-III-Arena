...
static int R_DlightSurface(msurface_t *surf, int dlightBits) {
  ...
  if (surf->cullinfo.type & CULLINFO_PLANE) {
    for (i = 0; i < tr.refdef.num_dlights; i++) {
      ...
      d = DotProduct(dl->origin, surf->cullinfo.plane.normal) -
          surf->cullinfo.plane.dist;
...
static int R_PshadowSurface(msurface_t *surf, int pshadowBits) {
  ...
  if (surf->cullinfo.type & CULLINFO_PLANE) {
    for (i = 0; i < tr.refdef.num_pshadows; i++) {
      ...
      d = DotProduct(ps->lightOrigin, surf->cullinfo.plane.normal) -
          surf->cullinfo.plane.dist;
  ...
  if (surf->cullinfo.type & CULLINFO_BOX) {
    for (i = 0; i < tr.refdef.num_pshadows; i++) {
      ...
      if (ps->lightOrigin[0] - ps->lightRadius > surf->cullinfo.bounds[1][0] ||
          ...
          BoxOnPlaneSide(surf->cullinfo.bounds[0], surf->cullinfo.bounds[1],
                         &ps->cullPlane) == 2) {
  ...
  if (surf->cullinfo.type & CULLINFO_SPHERE) {
    for (i = 0; i < tr.refdef.num_pshadows; i++) {
      ...
      if (!SpheresIntersect(ps->viewOrigin, ps->viewRadius,
                            surf->cullinfo.localOrigin,
                            surf->cullinfo.radius) ||
          DotProduct(surf->cullinfo.localOrigin, ps->cullPlane.normal) -
                  ps->cullPlane.dist <
              -surf->cullinfo.radius) {
        // pshadow doesn't reach the bounds
        pshadowBits &= ~(1 << i);
...
================
*/
static void R_RecursiveWorldNode(mnode_t *node, uint32_t planeBits,
                                 uint32_t dlightBits, uint32_t pshadowBits) {
    ...
    if (pshadowBits) {
      ...
      for (i = 0; i < tr.refdef.num_pshadows; i++) {
        ...
        if (pshadowBits & (1 << i)) {
          ...
          dist = DotProduct(shadow->lightOrigin, node->plane->normal) -
                 node->plane->dist;
...

  {
    ...
    while (c--) {
      // just mark it as visible, so we don't jump out of the cache derefencing
      // the surface
      surf = *view;
      if (tr.world->surfacesViewCount[surf] != tr.viewCount) {
        tr.world->surfacesViewCount[surf] = tr.viewCount;
