...
static void R_ColorShiftLightingFloats(float in[4], float out[4]) {
  float r, g, b;
  float scale =
      (1 << (r_mapOverBrightBits->integer - tr.overbrightBits)) / 255.0f;
...
static void R_LoadLightmaps(lump_t *l, lump_t *surfs) {
  ...
  if (r_mergeLightmaps->integer) {
    ...
    tr.numLightmaps =
        (numLightmaps + (numLightmapsPerPage - 1)) / numLightmapsPerPage;
  } else {
  ...
  if (r_hdr->integer) {
    // Check for the first hdr lightmap, if it exists, use GL_RGBA16 for
    // textures.
    char filename[MAX_QPATH];

    Com_sprintf(filename, sizeof(filename), "maps/%s/lm_0000.hdr",
  ...
  if (r_mergeLightmaps->integer) {
    ...
    for (i = 0; i < tr.numLightmaps; i++) {
      tr.lightmaps[i] =
          R_CreateImage(va("_fatlightmap%d", i), NULL, width, height,
                        IMGTYPE_COLORALPHA, imgFlags, textureInternalFormat);

      if (tr.worldDeluxeMapping)
        tr.deluxemaps[i] = R_CreateImage(va("_fatdeluxemap%d", i), NULL, width,
  ...
  for (i = 0; i < numLightmaps; i++) {
    ...
    // if (tr.worldLightmapping)
    {
      ...
      if (textureInternalFormat == GL_RGBA16) {
        Com_sprintf(filename, sizeof(filename), "maps/%s/lm_%04d.hdr",
                    s_worldData.baseName, i * (tr.worldDeluxeMapping ? 2 : 1));
      ...
      if (r_mergeLightmaps->integer)
        R_UpdateSubImage(tr.lightmaps[lightmapnum], image, xoff, yoff,
                         tr.lightmapSize, tr.lightmapSize,
                         textureInternalFormat);
      else
        tr.lightmaps[i] = R_CreateImage(
            va("*lightmap%d", i), image, tr.lightmapSize, tr.lightmapSize,
            IMGTYPE_COLORALPHA, imgFlags, textureInternalFormat);
    ...
    if (tr.worldDeluxeMapping) {
      ...
      if (r_mergeLightmaps->integer)
        ...
        tr.deluxemaps[i] =
            R_CreateImage(va("*deluxemap%d", i), image, tr.lightmapSize,
                          tr.lightmapSize, IMGTYPE_DELUXE, imgFlags, 0);
    }
  }
...
static float FatPackU(float input, int lightmapnum) {
  ...
  if (tr.fatLightmapCols > 0) {
    ...
    return (input + (lightmapnum % tr.fatLightmapCols)) /
           (float)(tr.fatLightmapCols);
  }
...
===============
*/
static void ParseFace(dsurface_t *ds, drawVert_t *verts, float *hdrVertColors,
                      msurface_t *surf, int *indexes) {
  int i, j;
  srfBspSurface_t *cv;
  glIndex_t *tri;
  ...
  // get shader value
  surf->shader =
      ShaderForShaderNum(ds->shaderNum, FatLightmap(realLightmapNum));
  ...
  for (i = 0; i < numVerts; i++)
    LoadDrawVertToSrfVert(
        &cv->verts[i], &verts[i], realLightmapNum,
  ...
  if (badTriangles) {
    ri.Printf(PRINT_WARNING,
              "Face has bad triangles, originally shader %s %d tris %d verts, "
              "now %d tris\n",
...
===============
*/
static void ParseMesh(dsurface_t *ds, drawVert_t *verts, float *hdrVertColors,
                      msurface_t *surf) {
  ...
  for (i = 0; i < numPoints; i++)
    LoadDrawVertToSrfVert(
        &points[i], &verts[i], realLightmapNum,
...
static void ParseTriSurf(dsurface_t *ds, drawVert_t *verts,
                         float *hdrVertColors, msurface_t *surf, int *indexes) {
  srfBspSurface_t *cv;
  glIndex_t *tri;
  int i, j;
  int numVerts, numIndexes, badTriangles;
  ...
  if (badTriangles) {
    ri.Printf(PRINT_WARNING,
              "Trisurf has bad triangles, originally shader %s %d tris %d "
              "verts, now %d tris\n",
              surf->shader->name, numIndexes / 3, numVerts,
              numIndexes / 3 - badTriangles);
    cv->numIndexes -= badTriangles * 3;
...
static void R_LoadSurfaces(lump_t *surfs, lump_t *verts, lump_t *indexLump) {
  ...
  s_worldData.surfacesViewCount =
      ri.Hunk_Alloc(count * sizeof(*s_worldData.surfacesViewCount), h_low);
  s_worldData.surfacesDlightBits =
      ri.Hunk_Alloc(count * sizeof(*s_worldData.surfacesDlightBits), h_low);
  s_worldData.surfacesPshadowBits =
      ri.Hunk_Alloc(count * sizeof(*s_worldData.surfacesPshadowBits), h_low);
  ...
  if (r_hdr->integer) {
    ...
    Com_sprintf(filename, sizeof(filename), "maps/%s/vertlight.raw",
                s_worldData.baseName);
    // ri.Printf(PRINT_ALL, "looking for %s\n", filename);
  ...
  // This ensures surfaces are close together to reduce L2 cache misses when
  // using VAOs, which don't actually use the verts and indexes
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump) {
  ...
  for (i = 0; i < count; i++, fogs++) {
    ...
    out->colorInt =
        ColorBytes4(shader->fogParms.color[0], shader->fogParms.color[1],
...
void R_LoadLightGrid(lump_t *l) {
  ...
  if (r_hdr->integer) {
    ...
    Com_sprintf(filename, sizeof(filename), "maps/%s/lightgrid.raw",
                s_worldData.baseName);
    // ri.Printf(PRINT_ALL, "looking for %s\n", filename);
    ...
    if (hdrLightGrid) {
      ...
      w->lightGrid16 =
          ri.Hunk_Alloc(sizeof(w->lightGrid16) * 6 * numGridPoints, h_low);
...
void R_LoadEntities(lump_t *l) {
  ...
  while (1) {
    ...
    if (!Q_stricmp(keyname, "autoExposureMinMax")) {
      sscanf(value, "%f %f", &tr.autoExposureMinMax[0],
...
qboolean R_ParseSpawnVars(char *spawnVarChars, int maxSpawnVarChars,
                          int *numSpawnVars,
...
void R_AssignCubemapsToWorldSurfaces(void) {
  ...
  for (i = 0; i < w->numsurfaces; i++) {
    ...
    if (surf->cullinfo.type & CULLINFO_SPHERE) {
      ...
      surfOrigin[0] =
          (surf->cullinfo.bounds[0][0] + surf->cullinfo.bounds[1][0]) * 0.5f;
      surfOrigin[1] =
          (surf->cullinfo.bounds[0][1] + surf->cullinfo.bounds[1][1]) * 0.5f;
      surfOrigin[2] =
          (surf->cullinfo.bounds[0][2] + surf->cullinfo.bounds[1][2]) * 0.5f;
    } else {
...
void R_LoadCubemaps(void) {
  ...
  for (i = 0; i < tr.numCubemaps; i++) {
    ...
    Com_sprintf(filename, MAX_QPATH, "cubemaps/%s/%03d.dds", tr.world->baseName,
                i);
  ...
  for (i = 0; i < tr.numCubemaps; i++) {
    if (!tr.cubemaps[i].image) {
      tr.cubemaps[i].image = R_CreateImage(
          va("*cubeMap%d", i), NULL, r_cubemapSize->integer,
  ...
  for (k = 0, surface = &s_worldData.surfaces[0];
    ...
    case SF_TRIANGLES:
      for (i = 0; i < bspSurf->numVerts; i++) {
        ...
        R_LightDirForPoint(bspSurf->verts[i].xyz, lightDir, normal,
                           &s_worldData);
...
void RE_LoadWorldMap(const char *name) {
  ...
  if (0) {
    ...
    for (i = 0; i < lightGridSize; i++, data += 8) {
      ...
      // FIXME: magic number for determining if light direction is close enough
      // to sunlight
    ...
    if (0) {
      ...
      for (i = 0; i < w->lightGridBounds[2]; i++) {
        ...
        ri.FS_WriteFile(fileName, buffer,
    ...
    for (i = 0; i < w->numWorldSurfaces; i++) {
      ...
      if (ci->type & CULLINFO_PLANE) {
        if (DotProduct(ci->plane.normal, tr.sunDirection) <= 0.0f) {
          // ri.Printf(PRINT_ALL, "surface %d is not oriented towards
          // sunlight\n", i);
          continue;
        }
      }
      ...
      if (ci->type & CULLINFO_BOX) {
        ...
        /*
        ri.Printf(PRINT_ALL, "surf %d bounds (%f %f %f)-(%f %f %f) ibounds (%d
        %d %d)-(%d %d %d)\n", i, ci->bounds[0][0], ci->bounds[0][1],
        ci->bounds[0][2], ci->bounds[1][0], ci->bounds[1][1], ci->bounds[1][2],
        ...
        */
        ...
        // FIXME: magic number for determining whether object is mostly in
        // sunlight
