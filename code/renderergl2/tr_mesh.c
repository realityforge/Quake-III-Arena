...
int R_ComputeLOD(trRefEntity_t *ent) {
  ...
  if (tr.currentModel->numLods < 2) {
    ...
    if (tr.currentModel->type == MOD_MDR) {
      ...
      // frame = ( md3Frame_t * ) ( ( ( unsigned char * )
      // tr.currentModel->md3[0] ) + tr.currentModel->md3[0]->ofsFrames );
...
void R_AddMD3Surfaces(trRefEntity_t *ent) {
  ...
  for (i = 0; i < model->numSurfaces; i++) {
    ...
    if (!personalModel && r_shadows->integer == 2 && fogNum == 0 &&
      ...
      R_AddDrawSurf((void *)&model->vaoSurfaces[i], tr.shadowShader, 0, qfalse,
                    qfalse, 0);
    }
    ...
    if (r_shadows->integer == 3 && fogNum == 0 &&
      ...
      R_AddDrawSurf((void *)&model->vaoSurfaces[i], tr.projectionShadowShader,
                    0, qfalse, qfalse, 0);
    }

    // don't add third_person objects if not viewing through a portal
    if (!personalModel) {
      R_AddDrawSurf((void *)&model->vaoSurfaces[i], shader, fogNum, qfalse,
                    qfalse, cubemapIndex);
    }
