...
static void RB_SurfaceVertsAndIndexes(int numVerts, srfVert_t *verts,
...
static qboolean RB_SurfaceVaoCached(int numVerts, srfVert_t *verts,
                                    int numIndexes, glIndex_t *indexes,
                                    int dlightBits, int pshadowBits) {
...
#define NUM_BEAM_SEGS 6
  ...
  // FIXME: A lot of this can probably be removed for speed, and refactored into
...
static void DoRailCore(const vec3_t start, const vec3_t end, const vec3_t up,
  ...
  tess.color[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0] * 0.25f * 257.0f;
  tess.color[tess.numVertexes][1] =
      backEnd.currentEntity->e.shaderRGBA[1] * 0.25f * 257.0f;
  tess.color[tess.numVertexes][2] =
      backEnd.currentEntity->e.shaderRGBA[2] * 0.25f * 257.0f;
  tess.numVertexes++;
  ...
  tess.color[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0] * 257;
  tess.color[tess.numVertexes][1] =
      backEnd.currentEntity->e.shaderRGBA[1] * 257;
  tess.color[tess.numVertexes][2] =
      backEnd.currentEntity->e.shaderRGBA[2] * 257;
  tess.numVertexes++;
...
static void LerpMeshVertexes(mdvSurface_t *surf, float backlerp) {
  ...
  if (backlerp == 0) {
    ...
    for (vertNum = 0; vertNum < surf->numVerts; vertNum++) {
      ...
      outNormal[0] = (int16_t)(newVerts->normal[0] * (1.0f - backlerp) +
                               oldVerts->normal[0] * backlerp);
      outNormal[1] = (int16_t)(newVerts->normal[1] * (1.0f - backlerp) +
                               oldVerts->normal[1] * backlerp);
      outNormal[2] = (int16_t)(newVerts->normal[2] * (1.0f - backlerp) +
                               oldVerts->normal[2] * backlerp);
      outNormal[3] = 0;

      outTangent[0] = (int16_t)(newVerts->tangent[0] * (1.0f - backlerp) +
                                oldVerts->tangent[0] * backlerp);
      outTangent[1] = (int16_t)(newVerts->tangent[1] * (1.0f - backlerp) +
                                oldVerts->tangent[1] * backlerp);
      outTangent[2] = (int16_t)(newVerts->tangent[2] * (1.0f - backlerp) +
...
void RB_SurfaceVaoMdvMesh(srfVaoMdvMesh_t *surface) {
  ...
  glState.vertexAttribsInterpolation =
      (refEnt->oldframe == refEnt->frame) ? 0.0f : refEnt->backlerp;
