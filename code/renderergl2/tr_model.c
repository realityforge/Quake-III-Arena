...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, int bufferSize,
                          const char *modName);
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, int bufferSize,
  ...
  mdvModel->frames = frame =
      ri.Hunk_Alloc(sizeof(*frame) * md3Model->numFrames, h_low);
  ...
  mdvModel->tags = tag = ri.Hunk_Alloc(
  ...
  mdvModel->tagNames = tagName =
      ri.Hunk_Alloc(sizeof(*tagName) * (md3Model->numTags), h_low);
  ...
  mdvModel->surfaces = surf =
      ri.Hunk_Alloc(sizeof(*surf) * md3Model->numSurfaces, h_low);
  ...
  for (i = 0; i < md3Model->numSurfaces; i++) {
    ...
    if (md3Surf->numVerts >= SHADER_MAX_VERTEXES) {
      ri.Printf(
          PRINT_WARNING, "R_LoadMD3: %s has more than %i verts on %s (%i).\n",
    ...
    surf->shaderIndexes = shaderIndex =
        ri.Hunk_Alloc(sizeof(*shaderIndex) * md3Surf->numShaders, h_low);
    ...
    surf->indexes = tri =
        ri.Hunk_Alloc(sizeof(*tri) * 3 * md3Surf->numTriangles, h_low);
    ...
    {
      vec3_t *sdirs =
          ri.Malloc(sizeof(*sdirs) * surf->numVerts * mdvModel->numFrames);
      vec3_t *tdirs =
          ri.Malloc(sizeof(*tdirs) * surf->numVerts * mdvModel->numFrames);
      ...
      for (f = 0; f < mdvModel->numFrames; f++) {
        for (j = 0, tri = surf->indexes; j < surf->numIndexes;
             j += 3, tri += 3) {
          vec3_t sdir, tdir;
      ...
      for (j = 0, v = surf->verts; j < (surf->numVerts * mdvModel->numFrames);
        ...
        tangent[3] =
            R_CalcTangentSpace(tangent, NULL, normal, sdirs[j], tdirs[j]);
...

  {
    ...
    mdvModel->vaoSurfaces = ri.Hunk_Alloc(
    ...
    for (i = 0; i < mdvModel->numSurfaces; i++, vaoSurf++, surf++) {
      ...
      if (mdvModel->numFrames > 1) {
        // vertex animation, store texcoords first, then
        ...
        dataSize =
            offset_xyz + surf->numVerts * mdvModel->numFrames * stride_xyz;
      } else {
...
mdvTag_t *R_GetAnimTag(mdrHeader_t *mod, int framenum, const char *tagName,
  ...
  for (i = 0; i < mod->numTags; i++, tag++) {
    if (!strcmp(tag->name, tagName)) {
      ...
      frame =
          (mdrFrame_t *)((byte *)mod + mod->ofsFrames + framenum * frameSize);
...
int R_LerpTag(orientation_t *tag, qhandle_t handle, int startFrame,
  ...
  if (!model->mdv[0]) {
    if (model->type == MOD_MDR) {
      ...
      return R_IQMLerpTag(tag, model->modelData, startFrame, endFrame, frac,
                          tagName);
    } else {
      start = end = NULL;
    }
