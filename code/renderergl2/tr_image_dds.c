...
#define EncodeFourCC(x)                                                        \
  ((((ui32_t)((x)[0]))) | (((ui32_t)((x)[1])) << 8) |                          \
   (((ui32_t)((x)[2])) << 16) | (((ui32_t)((x)[3])) << 24))
...
void R_LoadDDS(const char *filename, byte **pic, int *width, int *height,
  ...
  *pic = NULL;
  ...
  if (ddsHeaderDxt10) {
    ...
    default:
      ri.Printf(PRINT_ALL, "DDS File %s has unsupported DXGI format %d.",
    ...
    if (ddsHeader->pixelFormatFlags & DDSPF_FOURCC) {
      ...
      *picFormat = GL_RGBA8;
      ...
      ri.Printf(PRINT_ALL, "DDS File %s has unsupported RGBA format.",
                filename);
      ri.FS_FreeFile(buffer.v);
      return;
    }
