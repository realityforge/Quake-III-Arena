...
static void DrawSkySide(struct image_s *image, const int mins[2],
  ...
  for (t = 0; t < maxs[1] - mins[1]; t++) {
    for (s = 0; s < maxs[0] - mins[0]; s++) {
      ...
      tess.indexes[tess.numIndexes++] =
          s + t * (maxs[0] - mins[0] + 1) + firstVertex;
      tess.indexes[tess.numIndexes++] =
          s + (t + 1) * (maxs[0] - mins[0] + 1) + firstVertex;
      tess.indexes[tess.numIndexes++] =
          (s + 1) + t * (maxs[0] - mins[0] + 1) + firstVertex;
      ...
      tess.indexes[tess.numIndexes++] =
          s + (t + 1) * (maxs[0] - mins[0] + 1) + firstVertex;
      tess.indexes[tess.numIndexes++] =
          (s + 1) + (t + 1) * (maxs[0] - mins[0] + 1) + firstVertex;
    }
  }
  ...
  */
  ...
  // qglDrawElements(GL_TRIANGLES, tess.numIndexes - tess.firstIndex,
  // GL_INDEX_TYPE, BUFFER_OFFSET(tess.firstIndex * sizeof(glIndex_t)));
...
void RB_DrawSun(float scale, shader_t *shader) {
  ...
  // backEnd.viewParms.or.origin[1], backEnd.viewParms.or.origin[2]);
  {
