...
qboolean CheatsOk(gentity_t *ent) {
  if (!g_cheats.integer) {
    trap_SendServerCommand(
        ent - g_entities, "print \"Cheats are not enabled on this server.\n\"");
    return qfalse;
  }
  if (ent->health <= 0) {
    trap_SendServerCommand(
        ent - g_entities, "print \"You must be alive to use this command.\n\"");
    return qfalse;
  }
  return qtrue;
}
...
static const char *gameNames[] = {
    "Free For All",     "Tournament",   "Single Player", "Team Deathmatch",
...
void Cmd_CallVote_f(gentity_t *ent) {
  ...
  if (!Q_stricmp(arg1, "map_restart")) {
    ...
    trap_SendServerCommand(
        ...
        "g_gametype <n>, kick <player>, clientkick <clientnum>, g_doWarmup, "
        "timelimit <time>, fraglimit <frags>.\n\"");
    return;
  }
  ...
  if (!Q_stricmp(arg1, "g_gametype")) {
    ...
    i = ClientNumberFromString(ent, arg2, !Q_stricmp(arg1, "clientkick"),
                               !Q_stricmp(arg1, "kick"));
    if (i == -1) {
      return;
    }
...
void Cmd_Stats_f(gentity_t *ent) {
  /*
          ...
          //trap_SendServerCommand( ent-g_entities, va("print \"visited %d of %d
     areas\n\"", n, max)); trap_SendServerCommand( ent-g_entities, va("print
     \"%d%% level coverage\n\"", n * 100 / max));
  */
}
