...
int G_SelectRandomBotInfo(int team) {
  ...
  // don't add duplicate bots to the server if there are less bots than bot
  // types
  ...
  if (Q_stricmp(name, "random") == 0) {
    ...
    if (botinfoNum < 0) {
      G_Printf(S_COLOR_RED
               "Error: Cannot add random bot, no bot info available.\n");
...
void Svcmd_BotList_f(void) {
  ...
  for (i = 0; i < g_numBots; i++) {
    ...
    Q_strncpyz(funname, Info_ValueForKey(g_botInfos[i], "funname"),
               sizeof(funname));
    if (!*funname) {
      strcpy(funname, "");
    }
