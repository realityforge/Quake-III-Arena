  ...
  */
  ...
  if (ent->r.svFlags & SVF_BOT) {
    s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%"
           ...
           c2, client->pers.maxHealth, client->sess.wins, client->sess.losses,
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
  ...
  //	if ( !client->areabits )
  //		client->areabits = G_Alloc( (trap_AAS_PointReachabilityAreaIndex(
  //NULL ) + 7) / 8 );
...
void ClientSpawn(gentity_t *ent) {
  ...
  if (!level.intermissiontime) {
    if (ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
      ...
      // select the highest weapon number available, after any spawn given items
      // have fired
      client->ps.weapon = 1;
