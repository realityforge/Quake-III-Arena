...
static const char *master_items[] = {"Local",   "Internet",  "Master1",
                                     "Master2", "Master3",   "Master4",
                                     "Master5", "Favorites", NULL};

static const char *servertype_items[] = {
    "All",        "Free For All",     "Team Deathmatch",
    "Tournament", "Capture the Flag", NULL};

static const char *sortkey_items[] = {"Server Name",       "Map Name",
                                      "Open Player Spots", "Game Type",
                                      "Ping Time",         NULL};
...
const char *punkbuster_msg[] = {"PunkBuster will be", "disabled the next time",
                                "Quake III Arena", "is started.", NULL};
...
static servernode_t g_globalserverlist[UI_MAX_MASTER_SERVERS]
                                      [MAX_GLOBALSERVERS];
...
static void ArenaServers_Remove(void) {
  ...
  for (i = 0; i < g_arenaservers.numfavoriteaddresses; i++) {
    if (!Q_stricmp(g_arenaservers.favoriteaddresses[i],
...
static void ArenaServers_Insert(char *adrstr, char *info, int pingtime) {
  ...
  */
  ...
  if (servernodeptr->nettype < 0 ||
      servernodeptr->nettype >= ARRAY_LEN(netnames) - 1) {
    servernodeptr->nettype = 0;
  }
...
int ArenaServers_SetType(int type) {
  ...
  if (type >= UIAS_GLOBAL1 && type <= UIAS_GLOBAL5) {
    ...
    if (type == g_servertype ||
        type == ((g_servertype + 1) % UIAS_NUM_SOURCES)) {
      direction = 1;
    } else {
      direction = -1;
    }
...
static void Punkbuster_ConfirmEnable(qboolean result) {
  ...
  g_arenaservers.punkbuster.curvalue =
      Com_Clamp(0, 1, trap_Cvar_VariableValue("cl_punkbuster"));
}
...
static void ArenaServers_Event(void *ptr, int event) {
  ...
  case ID_PUNKBUSTER:
    if (g_arenaservers.punkbuster.curvalue) {
      UI_ConfirmMenu_Style("Enable Punkbuster?",
                           UI_CENTER | UI_INVERSE | UI_SMALLFONT, 0,
                           Punkbuster_ConfirmEnable);
    } else {
      UI_ConfirmMenu_Style("Disable Punkbuster?",
                           UI_CENTER | UI_INVERSE | UI_SMALLFONT, 0,
...
static void ArenaServers_MenuInit(void) {
  ...
  g_arenaservers.master.curvalue = g_servertype =
      Com_Clamp(0, UIAS_NUM_SOURCES - 1, ui_browserMaster.integer);
