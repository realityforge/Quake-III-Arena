...
static void SCR_PerformanceCounters()
{
	...
	x = 0, y = 240;
	SCR_AdjustFrom640(&x, &y, 0, 0);
	SCR_DrawString(x, y, cw, ch, va("FE: %.2fms (99th: %.2fms)", usecFE.median / 1000.0f, usecFE.percentile99 / 1000.0f), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Leaf/Area:  %i %i", pcFE[RF_LEAF_CLUSTER], pcFE[RF_LEAF_AREA]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("FrusLeafs:  %i", pcFE[RF_LEAFS]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("MD3 Cull S: %i %i %i", pcFE[RF_MD3_CULL_S_IN], pcFE[RF_MD3_CULL_S_CLIP], pcFE[RF_MD3_CULL_S_OUT]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("MD3 Cull B: %i %i %i", pcFE[RF_MD3_CULL_B_IN], pcFE[RF_MD3_CULL_B_CLIP], pcFE[RF_MD3_CULL_B_OUT]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Bez Cull S: %i %i %i", pcFE[RF_BEZ_CULL_S_IN], pcFE[RF_BEZ_CULL_S_CLIP], pcFE[RF_BEZ_CULL_S_OUT]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Bez Cull B: %i %i %i", pcFE[RF_BEZ_CULL_B_IN], pcFE[RF_BEZ_CULL_B_CLIP], pcFE[RF_BEZ_CULL_B_OUT]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Light Cull: %i %i", pcFE[RF_LIGHT_CULL_IN], pcFE[RF_LIGHT_CULL_OUT]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Lit Leafs:  %i", pcFE[RF_LIT_LEAFS]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Lit Surfs:  %i %i", pcFE[RF_LIT_SURFS], pcFE[RF_LIT_CULLS]), qfalse);
	y += ch;

	x = 240, y = 240;
	SCR_AdjustFrom640(&x, &y, 0, 0);
	SCR_DrawString(x, y, cw, ch, va("3D:  %.2fms (99th: %.2fms)", usec3D.median / 1000.0f, usec3D.percentile99 / 1000.0f), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("End: %.2fms (99th: %.2fms)", usecBS.median / 1000.0f, usecBS.percentile99 / 1000.0f), qfalse);
	y += ch;
	if (usecGPU.sampleCount > 0) {
		SCR_DrawString(x, y, cw, ch, va("GPU: %.2fms (99th: %.2fms)", usecGPU.median / 1000.0f, usecGPU.percentile99 / 1000.0f), qfalse);
		y += ch;
	}
	SCR_DrawString(x, y, cw, ch, va("Base Verts: %i", pc3D[RB_VERTICES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Base Tris:  %i", pc3D[RB_INDICES] / 3), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Base Surfs: %i", pc3D[RB_SURFACES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("B Batches:  %i", pc3D[RB_BATCHES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Shdr Chges: %i", pc3D[RB_SHADER_CHANGES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Draw Calls: %i", pc3D[RB_DRAW_CALLS]), qfalse);
	y += ch;

	SCR_DrawString(x, y, cw, ch, va("Lit Verts:  %i", pc3D[RB_LIT_VERTICES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Lit Tris:   %i", pc3D[RB_LIT_INDICES] / 3), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Lit Surfs:  %i", pc3D[RB_LIT_SURFACES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("L Batches:  %i", pc3D[RB_LIT_BATCHES]), qfalse);
	y += ch;

	SCR_DrawString(x, y, cw, ch, va("L LateCullT: %i", pc3D[RB_LIT_VERTICES_LATECULLTEST]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("L LateCullI: %i %i", pc3D[RB_LIT_INDICES_LATECULL_IN], pc3D[RB_LIT_INDICES_LATECULL_OUT]), qfalse);
	y += ch;

	x = 480, y = 240;
	SCR_AdjustFrom640(&x, &y, 0, 0);
	SCR_DrawString(x, y, cw, ch, "2D", qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Vertices: %i", pc2D[RB_VERTICES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Indices:  %i", pc2D[RB_INDICES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Surfaces: %i", pc2D[RB_SURFACES]), qfalse);
	y += ch;
	SCR_DrawString(x, y, cw, ch, va("Batches:  %i", pc2D[RB_BATCHES]), qfalse);
	y += ch;

	x = 0, y = 480 - ch;
	SCR_AdjustFrom640(&x, &y, 0, 0);
	SCR_DrawString(x, y, cw, ch, va("Back-end: %s", Cvar_VariableString("r_backend")), qfalse);
	y -= ch;
	SCR_DrawString(x, y, cw, ch, va("V-Sync  : %s", Sys_V_IsVSynced() ? "ON" : "OFF"), qfalse);
	y -= ch;
}
