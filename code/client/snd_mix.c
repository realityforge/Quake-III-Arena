  ...
  while (ls_paintedtime < endtime) {
    ...
    if (CL_VideoRecording())
      CL_WriteAVIAudioFrame((byte *)snd_out,
                            snd_linear_count
                                << 1); // snd_linear_count * (dma.samplebits/8)
  }
}
  ...
  if (dma.samplebits == 16 && dma.channels == 2) { // optimized case
    ...
    if ((dma.isfloat) && (dma.samplebits == 32)) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (val > 0x7fff)
          val = 0x7fff;
        else if (val < -32767) /* clamp to one less than max to make division
                                  max out at -1.0f. */
          val = -32767;
...
static void S_PaintChannelFrom16(channel_t *ch, const sfx_t *sc, int count,
  ...
  if (com_altivec->integer) {
    ...
    S_PaintChannelFrom16_altivec(paintbuffer, snd_vol, ch, sc, count,
                                 sampleOffset, bufferOffset);
    return;
  }
#endif
