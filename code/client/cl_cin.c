...
static unsigned int yuv_to_rgb24(long y, long u, long v) {
  ...
  return LittleLong((unsigned long)((r) | (g << 8) | (b << 16)) |
                    (255UL << 24));
}
...
e_status CIN_RunCinematic(int handle) {
  ...
  cinTable[currentHandle].tfps =
      (((CL_ScaledMilliseconds() - cinTable[currentHandle].startTime) * 3) /
       100);
  ...
  if (handle >= 0 && handle < MAX_VIDEO_HANDLES) {
    ...
    if (cinTable[handle].dirty &&
      ...
      re.UploadCinematic(cinTable[handle].CIN_WIDTH,
                         cinTable[handle].CIN_HEIGHT, 256, 256, (byte *)buf2,
                         handle, qtrue);
      cinTable[handle].dirty = qfalse;
      Hunk_FreeTempMemory(buf2);
    } else {
      ...
      re.UploadCinematic(cinTable[handle].CIN_WIDTH,
                         ...
                         cinTable[handle].drawY, cinTable[handle].buf, handle,
