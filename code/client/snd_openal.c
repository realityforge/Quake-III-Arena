...
typedef struct alSfx_s {
  ...
  int masterLoopSrc; // All other sources looping this buffer are synced to this
                     // master src
} alSfx_t;
...
static void S_AL_NewLoopMaster(src_t *rmSource, qboolean iskilled) {
  ...
  if (curSfx->loopCnt) {
    if (rmSource->priority == SRCPRI_ENTITY) {
      ...
      // Only if rmSource was the master and if there are still playing loops
      // for this sound will we need to find a new master.
      ...
      if (!curSfx->loopActiveCnt) {
        ...
        if (rmSource->isPlaying) {
          // this was the last not stopped source, save last sample position +
          // time
          S_AL_SaveLoopPos(curSource, rmSource->alSource);
        } else {
          // second case: all loops using this sound have stopped due to
          // listener being of of range, and now the inactive master gets
          // deleted. Just move over the soundpos settings to the new master.
  ...
  for (i = 0; i < srcCount; i++) {
    ...
    if (curSource->isPlaying) {
      if (weakest_isplaying && curSource->priority < priority &&
        ...
        // the last two values are only compared if it's not a looping sound,
        // because we want to prevent two loops (loops are added EVERY frame)
        // fighting for a slot
  ...
  if ((srcActiveCnt > 5 * srcCount / 3) &&
      (DistanceSquared(sorigin, lastListenerOrigin) >=
       (s_alMaxDistance->value + s_alGraceDistance->value) *
           (s_alMaxDistance->value + s_alGraceDistance->value))) {
    // We're getting tight on sources and source is not within hearing distance
    // so don't add it
    return;
  }
    ...
    if (curSource->isLooping) {
      ...
      if (sent->loopAddedThisFrame) {
        ...
        if (sent->startLoopingSound) {
          S_AL_SrcSetup(i, sent->loopSfx, sent->loopPriority, entityNum, -1,
                        curSource->local);
          curSource->isLooping = qtrue;
        ...
        if (!curSource->isPlaying) {
          ...
          if (curSource->priority == SRCPRI_AMBIENT) {
            ...
            if (curSfx->loopActiveCnt) {
              ...
              S_AL_ClearError(qfalse);
              qalGetSourcei(srcList[curSfx->masterLoopSrc].alSource,
              ...
              // This loop sound used to be played, but all sources are stopped.
              // Use last sample position/time to calculate offset so the player
              // thinks the sources continued playing while they were inaudible.

              if (master->lastTimePos >= 0) {
                secofs =
                    master->lastTimePos +
                    (Sys_Milliseconds() - master->lastSampleTime) / 1000.0f;
                secofs = fmodf(secofs,
            ...
            // For unsynced loops (SRCPRI_ENTITY) just carry on playing as if
            // the sound was never stopped

            secofs = curSource->lastTimePos +
                     (Sys_Milliseconds() - curSource->lastSampleTime) / 1000.0f;
            secofs =
...
static void S_AL_RawSamples(int stream, int samples, int rate, int width,
  ...
  if (numBuffers == MAX_STREAM_BUFFERS) {
    Com_DPrintf(
        S_COLOR_RED
        "WARNING: Steam dropping raw samples, reached MAX_STREAM_BUFFERS\n");
    return;
  }
  ...
  if (numBuffers == streamNumBuffers[stream]) {
    ...
    for (i = 0; i < streamNumBuffers[stream]; ++i)
      streamBuffers[stream][i] =
          oldBuffers[(streamBufIndex[stream] + i) % streamNumBuffers[stream]];
...
qboolean S_AL_Init(soundInterface_t *si) {
  ...
  s_alDriver = Cvar_Get("s_alDriver", ALDRIVER_DEFAULT,
                        CVAR_ARCHIVE | CVAR_LATCH | CVAR_PROTECTED);
  ...
  if (enumeration_ext || enumeration_all_ext) {
    ...
    // Generic Software as that one works more reliably with various sound
    // systems. If it's not, use OpenAL's default selection as we don't want to
    // ignore native hardware acceleration.
  ...
  if (!s_alCapture->integer) {
    Com_Printf("OpenAL capture support disabled by user ('+set s_alCapture 1' "
               "to enable)\n");
  }
#if USE_MUMBLE
...
#endif
    {
      ...
      alCaptureDevice = qalcCaptureOpenDevice(
          inputdevice, 48000, AL_FORMAT_MONO16, VOIP_MAX_PACKET_SAMPLES * 4);
      if (!alCaptureDevice && inputdevice) {
        Com_Printf("Failed to open OpenAL Input device '%s', trying default.\n",
                   inputdevice);
        alCaptureDevice = qalcCaptureOpenDevice(NULL, 48000, AL_FORMAT_MONO16,
                                                VOIP_MAX_PACKET_SAMPLES * 4);
      }
