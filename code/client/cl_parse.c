...
void CL_ParseGamestate(msg_t *msg) {
  ...
  // stop recording now so the demo won't have an unnecessary level load at the
  // end.
...
static void CL_PlayVoip(int sender, int samplecnt, const byte *data,
  ...
  if (flags & VOIP_DIRECT) {
    S_RawSamples(sender + 1, samplecnt, 48000, 2, 1, data, clc.voipGain[sender],
                 -1);
  }

  if (flags & VOIP_SPATIAL) {
    S_RawSamples(sender + MAX_CLIENTS + 1, samplecnt, 48000, 2, 1, data, 1.0f,
                 sender);
  }
}
...
static void CL_ParseVoip(msg_t *msg, qboolean ignoreData) {
  static short
      decoded[VOIP_MAX_PACKET_SAMPLES * 4]; // !!! FIXME: don't hard code
  ...
  if (generation != clc.voipIncomingGeneration[sender]) {
    ...
    opus_decoder_ctl(clc.opusDecoder[sender], OPUS_RESET_STATE);
    seqdiff = 0;
  } else if (seqdiff * VOIP_MAX_PACKET_SAMPLES * 2 >=
  ...
  numSamples = opus_decode(clc.opusDecoder[sender], encoded, packetsize,
                           decoded + written, ARRAY_LEN(decoded) - written, 0);
