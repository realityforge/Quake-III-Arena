  ...
  for (i = 0; i < outcount; i++) {
    ...
    for (j = 0; j < channels; j++) {
      if (inwidth == 2) {
        ...
        sample = (unsigned int)((unsigned char)(data[srcsample + j]) - 128)
                 << 8;
      }
...
qboolean S_LoadSound(sfx_t *sfx) {
  ...
  samples =
      Hunk_AllocateTempMemory(info.channels * info.samples * sizeof(short) * 2);
  ...
  if (info.channels == 1 && sfx->soundCompressed == qtrue) {
    ...
    sfx->soundData = NULL;
    sfx->soundLength = ResampleSfx(sfx, info.channels, info.rate, info.width,
                                   info.samples, data + info.dataofs, qfalse);
  }
