...
static void CL_CaptureVoip(void) {
  ...
  if (cl_voip->modified || cl_rate->modified) {
    if ((cl_voip->integer) && (cl_rate->integer < 25000)) {
      ...
      Com_Printf(
          "Set 'Data Rate' to 'LAN/Cable/xDSL' in 'Setup/System/Network'.\n");
  ...
  if ((cl_voipSend->integer) || (finalFrame)) { // user wants to capture audio?
    ...
    const int packetSamples =
        (finalFrame) ? VOIP_MAX_FRAME_SAMPLES : VOIP_MAX_PACKET_SAMPLES;
    ...
    if (samples >= packetSamples) {
      ...
      if (samples != 120 && samples != 240 && samples != 480 &&
      ...
      clc.voipPower =
          (voipPower / (32768.0f * 32768.0f * ((float)samples))) * 100.0f;
...
void CL_Record_f(void) {
  ...
  if (NET_IsLocalAddress(clc.serverAddress) &&
    ...
    Com_Printf(S_COLOR_YELLOW "WARNING: You should set 'g_synchronousClients "
                              "1' for smoother demo recording\n");
  }
...
void CL_RequestMotd(void) {
  ...
  Com_sprintf(cls.updateChallenge, sizeof(cls.updateChallenge), "%i",
              (int)((((unsigned int)rand() << 16) ^ (unsigned int)rand()) ^
                    Com_Milliseconds()));
...
void CL_Connect_f(void) {
  ...
  if (NET_IsLocalAddress(clc.serverAddress))
    ...
    // server.
    clc.challenge = (((unsigned int)rand() << 16) ^ (unsigned int)rand()) ^
                    Com_Milliseconds();
  }
...
void CL_InitDownloads(void) {
  ...
  if (!(cl_allowDownload->integer & DLF_ENABLE)) {
    ...
    if (FS_ComparePaks(missingfiles, sizeof(missingfiles), qfalse)) {
      ...
      //   but at this point while joining the game we don't know wether we will
      //   successfully join or not
      Com_Printf(
          "\nWARNING: You are missing some files referenced by the server:\n%s"
          "You might not be able to join the game\n"
          "Go to the setting menu to turn on autodownload, or get the file "
          "elsewhere\n\n",
          missingfiles);
    }
...
void CL_CheckForResend(void) {
  ...
  switch (clc.state) {
  case CA_CONNECTING:
    // requesting a challenge .. IPv6 users always get in as authorize server
    // supports no ipv6.
#ifndef STANDALONE
    ...
    // server knows we're running the correct game or can reject the client with
...
void CL_CheckTimeout(void) {
  ...
  // check timeout
  //
  if ((!CL_CheckPaused() || !sv_paused->integer) && clc.state >= CA_CONNECTED &&
...
void CL_Frame(int msec) {
  ...
  if (cl_autoRecordDemo->integer) {
    if (clc.state == CA_ACTIVE && !clc.demorecording && !clc.demoplaying) {
      ...
      Com_RealTime(&now);
      nowString =
          va("%04d%02d%02d%02d%02d%02d", 1900 + now.tm_year, 1 + now.tm_mon,
...
void CL_InitRef(void) {
  ...
  if (!(rendererLib = Sys_LoadDll(dllName, qfalse)) &&
    ...
    Com_sprintf(dllName, sizeof(dllName),
                "renderer_opengl2_" ARCH_STRING DLL_EXT);
...
void CL_Init(void) {
  ...
  cl_cURLLib =
      Cvar_Get("cl_cURLLib", DEFAULT_CURL_LIB, CVAR_ARCHIVE | CVAR_PROTECTED);
#endif
  ...
  cl_voipProtocol = Cvar_Get("cl_voipProtocol", cl_voip->integer ? "opus" : "",
...
0-4. ioquake3 2017; made master 0 fetch all master servers and 1-5 request a
