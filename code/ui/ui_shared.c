...
// menu item key horizontal action: -1 = previous value, 1 = next value, 0 = no
// change
...
qboolean Item_Bind_HandleKey(itemDef_t *item, int key, qboolean down) {
  ...
  if (!g_waitingForKey) {
    if (down &&
        ((key == K_MOUSE1 &&
          Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)) ||
         key == K_ENTER || key == K_KP_ENTER || key == K_JOY1 ||
         key == K_JOY2 || key == K_JOY3 || key == K_JOY4)) {
...
static void Item_ApplyHacks(itemDef_t *item) {
  ...
  if (item->type == ITEM_TYPE_EDITFIELD && item->cvar &&
    ...
    if (editField->maxChars < 48) {
      Com_Printf("Extended create favorite address edit field length to hold "
                 "an IPv6 address\n");
