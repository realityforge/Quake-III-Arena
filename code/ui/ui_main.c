...
static const char *netSources[] = {"Local",   "Internet", "Master1",
...
static const char *teamArenaGameTypes[] = {"FFA",      "TOURNAMENT", "SP",
                                           "TEAM DM",  "CTF",        "1FCTF",
                                           "OVERLOAD", "HARVESTER"};
  ...
  if (i >= 0 && i < uiInfo.teamCount) {
    ...
    if (uiInfo.teamList[i].cinematic >= -2) {
      ...
      if (uiInfo.teamList[i].cinematic >= 0) {
        ...
        UI_DrawCinematic(uiInfo.teamList[i].cinematic, rect->x, rect->y,
                         rect->w, rect->h);
      } else {
        trap_R_SetColor(color);
        UI_DrawHandlePic(rect->x, rect->y, rect->w, rect->h,
...
static void UI_DrawMapCinematic(rectDef_t *rect, float scale, vec4_t color,
  ...
  if (uiInfo.mapList[map].cinematic >= -1) {
    ...
    if (uiInfo.mapList[map].cinematic >= 0) {
      ...
      UI_DrawCinematic(uiInfo.mapList[map].cinematic, rect->x, rect->y, rect->w,
                       rect->h);
    } else {
...
static void UI_DrawOpponent(rectDef_t *rect) {
  ...
  if (updateOpponentModel) {
    ...
    Q_strncpyz(model, UI_Cvar_VariableString("ui_opponentModel"),
               sizeof(model));
    Q_strncpyz(headmodel, UI_Cvar_VariableString("ui_opponentModel"),
               sizeof(headmodel));
    team[0] = '\0';
...
*/
static int QDECL UI_ServersQsortCompare(const void *arg1, const void *arg2) {
  return trap_LAN_CompareServers(UI_SourceForLAN(), uiInfo.serverStatus.sortKey,
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (!j) {
      if (protocolLegacy > 0 && uiInfo.demoCount < MAX_DEMOS) {
        ...
        uiInfo.demoCount += trap_FS_GetFileList("demos", demoExt, demolist,
                                                ARRAY_LEN(demolist));
        demoname = demolist;
      } else
        break;
...
static void UI_RunMenuScript(char **args) {
  ...
  if (String_Parse(args, &name)) {
    if (Q_stricmp(name, "StartServer") == 0) {
      ...
      // UpdateFilter is called when server broser menu is opened and when a
      // favorite server is deleted.
      UI_StartServerRefresh(qtrue, qfalse);
      UI_BuildServerDisplayList(qtrue);
      ...
      if (String_Parse(args, &orders)) {
        ...
        if (selectedPlayer < uiInfo.myTeamCount) {
          Com_sprintf(buff, sizeof(buff), orders,
                      uiInfo.teamClientNums[selectedPlayer]);
          trap_Cmd_ExecuteText(EXEC_APPEND, buff);
          trap_Cmd_ExecuteText(EXEC_APPEND, "\n");
        } else {
          int i;
  ...
  for (j = 0; j < TEAM_MEMBERS; j++) {
    for (k = 0; k < uiInfo.aliasCount; k++) {
      if (uiInfo.aliasList[k].name != NULL) {
        if (Q_stricmp(uiInfo.teamList[tIndex].teamMembers[j],
        ...
        if (ping <= 0) {
          ...
          if (ui_netSource.integer == UIAS_LOCAL) {
            ...
            Com_sprintf(hostname, sizeof(hostname), "%s [%s]",
                        Info_ValueForKey(info, "hostname"), netnames[nettype]);
            return hostname;
          } else {
...
static void UI_StartServerRefresh(qboolean full, qboolean force) {
  ...
  // This function is called with force=qfalse when server browser menu opens or
  // net source changes. Automatically update local and favorite servers. Only
  // auto update master server list if there is no server info cache.
  ...
  trap_Cvar_Set(va("ui_lastServerRefresh_%i", ui_netSource.integer),
                va("%s-%i, %i at %i:%02i", MonthAbbrev[q.tm_mon], q.tm_mday,
  ...
  uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 5000;
  if (ui_netSource.integer >= UIAS_GLOBAL0 &&
      ui_netSource.integer <= UIAS_GLOBAL5) {
