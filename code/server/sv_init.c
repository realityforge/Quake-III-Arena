...
static void SV_Startup(void) {
  ...
  // Join the ipv6 multicast group now that a map is running so clients can scan
  // for us on the local network.
  NET_JoinMulticast6();
}
...
void SV_SpawnServer(char *server, qboolean killBots) {
  ...
  sv.checksumFeed = (((unsigned int)rand() << 16) ^ (unsigned int)rand()) ^
                    Com_Milliseconds();
...
void SV_Init(void) {
  ...
  sv_voipProtocol = Cvar_Get("sv_voipProtocol", sv_voip->integer ? "opus" : "",
