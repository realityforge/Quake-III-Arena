...
void SV_GetChallenge(netadr_t from) {
  ...
  // sv_maxping
  challenge->challenge =
      (((unsigned int)rand() << 16) ^ (unsigned int)rand()) ^ svs.time;
  challenge->wasrefused = qfalse;
  challenge->time = svs.time;

#ifndef STANDALONE
  // Drop the authorize stuff if this client is coming in via v6 as the auth
...
void SV_AuthorizeIpPacket(netadr_t from) {
  ...
  if (!Q_stricmp(s, "accept")) {
    NET_OutOfBandPrint(NS_SERVER, challengeptr->adr,
                       "challengeResponse %d %d %d", challengeptr->challenge,
...
void SV_DirectConnect(netadr_t from) {
  ...
  if (!NET_IsLocalAddress(from)) {
    ...
    if (challengeptr->wasrefused) {
      // Return silently, so that error messages written by the server keep
      // being displayed.
      return;
    }
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
  ...
  if (!cl->download) {
    ...
    if (!(sv_allowDownload->integer & DLF_ENABLE) ||
        (sv_allowDownload->integer & DLF_NO_UDP) || idPack || unreferenced ||
        (cl->downloadSize =
...
void SV_UserinfoChanged(client_t *cl) {
  ...
  if (i != cl->snapshotMsec) {
    // Reset last sent snapshot so we avoid desync between server frame time and
    // snapshot send time
    cl->lastSnapshotTime = 0;
    cl->snapshotMsec = i;
  }
