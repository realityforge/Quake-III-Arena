...
void SV_RankBegin(char *gamekey) {
  ...
  if (!strlen(Cvar_VariableString("sv_leagueName"))) {
    status = GRankNewGameAsync(
        s_server_context, SV_RankNewGameCBF, NULL, GR_OPT_LEAGUENAME,
        (void *)(Cvar_VariableString("sv_leagueName")), GR_OPT_END);
  } else {
    status = GRankNewGameAsync(s_server_context, SV_RankNewGameCBF, NULL,
                               GR_OPT_END);
  }
  ...
  memset((void *)s_ranked_players, 0,
         sv_maxclients->value * sizeof(ranked_player_t));
}
...
void SV_RankEnd(void) {
  ...
  status = GRankSendReportsAsync(s_server_context, 0, SV_RankSendReportsCBF,
                                 NULL, GR_OPT_END);
...
void SV_RankUserCreate(int index, char *username, char *password, char *email) {
  ...
  status = GRankUserCreateAsync(s_ranked_players[index].context, username,
                                password, email, SV_RankUserCBF,
...
void SV_RankUserLogin(int index, char *username, char *password) {
  ...
  status = GRankUserLoginAsync(s_ranked_players[index].context, username,
                               password, SV_RankUserCBF,
                               (void *)&s_ranked_players[index], GR_OPT_END);
...
===================
*/
qboolean SV_RankUserValidate(int index, const char *player_id, const char *key,
  ...
  if ((player_id != NULL) && (key != NULL)) {
    ...
    status = GRankPlayerValidate(
        s_server_context, ranked_player->player_id, ranked_player->token,
        token_len, GR_OPT_PLAYERCONTEXT, ranked_player->context, GR_OPT_END);
  } else {
...
void SV_RankUserLogout(int index) {
  ...
  status = GRankSendReportsAsync(s_ranked_players[index].context, 0,
  ...
  if (status == GR_STATUS_PENDING) {
    ...
    cleanup_status =
        GRankCleanupAsync(s_ranked_players[index].context, 0, SV_RankCleanupCBF,
                          (void *)&s_ranked_players[index], GR_OPT_END);
...
void SV_RankReportInt(int index1, int index2, int key, int value,
  ...
  status = GRankReportInt(context, match, user1, user2, key, value, opt_accum,
                          GR_OPT_END);
...
void SV_RankReportStr(int index1, int index2, int key, char *value) {
  ...
  if (user2 != 0) {
    ...
    status =
        GRankReportStr(context, match, user1, user2, key, value, GR_OPT_END);
        ...
        if (s_ranked_players[i].grank_status == QGR_STATUS_ACTIVE) {
          ...
          if (s_ranked_players[i].context) {
            ...
            cleanup_status = GRankCleanupAsync(
                s_ranked_players[i].context, 0, SV_RankCleanupCBF,
...
static void SV_RankUserCBF(GR_LOGIN *gr_login, void *cbf_arg) {
  ...
  case GR_STATUS_OK:
    ...
    join_status = GRankJoinGameAsync(ranked_player->context, s_rankings_game_id,
                                     SV_RankJoinGameCBF, cbf_arg, GR_OPT_END);
  ...
  if (ranked_player->final_status != QGR_STATUS_NEW) {
    ...
    cleanup_status =
        GRankCleanupAsync(ranked_player->context, 0, SV_RankCleanupCBF,
                          (void *)ranked_player, GR_OPT_END);
...
static void SV_RankJoinGameCBF(GR_JOINGAME *gr_joingame, void *cbf_arg) {
  ...
  if (gr_joingame->status == GR_STATUS_OK) {
    ...
    cleanup_status = GRankCleanupAsync(ranked_player->context, 0,
                                       SV_RankCleanupCBF, cbf_arg, GR_OPT_END);
...
static void SV_RankSendReportsCBF(GR_STATUS *status, void *cbf_arg) {
  ...
  if (context == 0) {
    ...
    cleanup_status =
        GRankCleanupAsync(context, 0, SV_RankCleanupCBF, cbf_arg, GR_OPT_END);
