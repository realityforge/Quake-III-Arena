...
void SV_AddServerCommand(client_t* client, const char* cmd)
{
    ...
    strncpyz(client->reliableCommands[index], cmd, sizeof(client->reliableCommands[index]));
}
...
static void SV_MasterHeartbeat(const char* message)
{
    ...
    for (i = 0; i < MAX_MASTER_SERVERS; i++) {
        ...
        // this command should be changed if the server info / status format ever changes incompatibly
