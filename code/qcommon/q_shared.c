...
qboolean Q_IsColorString(const char *p) {
  ...
  // isalnum expects a signed integer in the range -1 (EOF) to 255, or it might
  // assert on undefined behaviour a dereferenced char pointer has the range
  // -128 to 127, so we just need to rangecheck the negative part
  if (p[1] < 0)
    return qfalse;
