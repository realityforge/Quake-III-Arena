...
unsigned int JSON_ArrayGetIndex(const char *json, const char *jsonEnd,
                                const char **indexes, unsigned int numIndexes);

// Get pointer to indexed value from array
// returns NULL if not an array, no index, or out of data
const char *JSON_ArrayGetValue(const char *json, const char *jsonEnd,
                               unsigned int index);
...
const char *JSON_ObjectGetNamedValue(const char *json, const char *jsonEnd,
                                     const char *name);
...
// returns length and copies into string if successful, including terminating
// nul. string values are stripped of enclosing quotes but not escaped
unsigned int JSON_ValueGetString(const char *json, const char *jsonEnd,
                                 char *outString, unsigned int stringLen);
...
static const char *JSON_SkipValueAndSeparators(const char *json,
                                               const char *jsonEnd);
...
// returns 0 if value requires more parsing, 1 if no more data/false/null, 2 if
// true
...
unsigned int JSON_ArrayGetIndex(const char *json, const char *jsonEnd,
  ...
  for (json = JSON_ArrayGetFirstValue(json, jsonEnd); json;
       json = JSON_ArrayGetNextValue(json, jsonEnd)) {
...
const char *JSON_ArrayGetValue(const char *json, const char *jsonEnd,
                               unsigned int index) {
  for (json = JSON_ArrayGetFirstValue(json, jsonEnd); json && index;
       json = JSON_ArrayGetNextValue(json, jsonEnd))
    index--;
