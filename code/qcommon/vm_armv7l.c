...
static void store_item_opstack(instruction_t* ins)
{
    ...
    case OP_LOCAL:
        ...
        break;
    default:
        DROP("incorrect opcode %i", ins->op);
    }
...
static void emitBlockCopy(vm_t* vm, const uint32_t count)
{
    ...
    load_rx_opstack(R0 | FORCED);
    dec_opstack(); // src: r0 = *opstack; opstack -= 4
    load_rx_opstack(R1 | FORCED);
    dec_opstack(); // dst: r1 = *opstack; opstack -= 4
...
static qboolean ConstOptimize(vm_t* vm, instruction_t* ci, instruction_t* ni)
{
    ...
    case OP_BXOR:
        if (can_encode(ci->value)) {
            ...
            switch (ni->op) {
            case OP_ADD:
                emit(ADDi(rx[1], rx[0], ci->value));
                break; // r1 = r0 + x
            case OP_SUB:
                emit(SUBi(rx[1], rx[0], ci->value));
                break; // r1 = r0 - x
            case OP_BAND:
                emit(ANDi(rx[1], rx[0], ci->value));
                break; // r1 = r0 & x
            case OP_BOR:
                emit(ORRi(rx[1], rx[0], ci->value));
                break; // r1 = r0 | x
            case OP_BXOR:
                emit(EORi(rx[1], rx[0], ci->value));
                break; // r1 = r0 ^ x
            }
            if (rx[0] != rx[1]) {
                unmask_rx(rx[0]);
    ...
    case OP_RSHU:
        ...
        switch (ni->op) {
        case OP_LSH:
            emit(LSLi(rx[1], rx[0], ci->value));
            break; // r1 = r0 << x
        case OP_RSHI:
            emit(ASRi(rx[1], rx[0], ci->value));
            break; // r1 = r0 >> x
        case OP_RSHU:
            emit(LSRi(rx[1], rx[0], ci->value));
            break; // r1 = (unsigned)r0 >> x
        }
        if (rx[0] != rx[1]) {
            unmask_rx(rx[0]);
        }
    ...
    case OP_LTI:
        if (can_encode(ci->value)) {
            ...
            rx[0] = load_rx_opstack(R0 | RCONST);
            dec_opstack(); // r0 = *opstack; opstack -= 4
...
qboolean VM_Compile(vm_t* vm, vmHeader_t* header)
{
    ...
    __recompile:
        ...
        while (ip < header->instructionCount) {
            ...
            case OP_GEF: {
                uint32_t comp = get_comp(ci->op);
                sx[0] = load_sx_opstack(S0 | RCONST);
                dec_opstack(); // s0 = *opstack; opstack -= 4
                sx[1] = load_sx_opstack(S1 | RCONST);
                dec_opstack(); // s1 = *opstack; opstack -= 4
                unmask_sx(sx[0]);
                unmask_sx(sx[1]);
            ...
            case OP_LOAD4:
                ...
                if (addr_on_top(&var)) {
                    ...
                    if ((reg = find_rx_var(&rx[0], &var)) != NULL) {
                        ...
                        if ((ci + 1)->op == sign_extend && sign_extend != OP_UNDEF) {
                            ...
                            if (var.addr < 256) {
                                ...
                                switch (ci->op) {
                                case OP_LOAD1:
                                    emit(LDRSBai(rx[0], var.base, var.addr));
                                    set_rx_ext(rx[0], S_EXT8);
                                    break; // r0 = (signed char)var.base[var.addr]
                                case OP_LOAD2:
                                    emit(LDRSHai(rx[0], var.base, var.addr));
                                    set_rx_ext(rx[0], S_EXT16);
                                    break; // r0 = (signed short)var.base[var.addr]
                                }
                            } else {
                                // long offset
                                ...
                                switch (ci->op) {
                                case OP_LOAD1:
                                    emit(LDRSBa(rx[0], var.base, rx[1]));
                                    set_rx_ext(rx[0], S_EXT8);
                                    break; // r0 = (signed char)var.base[r1]
                                case OP_LOAD2:
                                    emit(LDRSHa(rx[0], var.base, rx[1]));
                                    set_rx_ext(rx[0], S_EXT16);
                                    break; // r0 = (signed short)var.base[r1]
                                }
                                unmask_rx(rx[1]);
                            }
                            ...
                            if (var.addr < max_offset) {
                                ...
                                switch (ci->op) {
                                case OP_LOAD1:
                                    emit(LDRBai(rx[0], var.base, var.addr));
                                    set_rx_ext(rx[0], Z_EXT8);
                                    break; // r0 = (unsigned char)var.base[var.addr]
                                case OP_LOAD2:
                                    emit(LDRHai(rx[0], var.base, var.addr));
                                    set_rx_ext(rx[0], Z_EXT16);
                                    break; // r0 = (unsigned short)var.base[var.addr]
                                default:
                                    emit(LDRai(rx[0], var.base, var.addr));
                                    set_rx_ext(rx[0], Z_NONE);
                                    break; // r0 = var.base[var.addr]
                                }
                            } else {
                                // long offset
                                rx[1] = alloc_rx_const(R1, var.addr);
                                switch (ci->op) {
                                case OP_LOAD1:
                                    emit(LDRBa(rx[0], var.base, rx[1]));
                                    set_rx_ext(rx[0], Z_EXT8);
                                    break; // r0 = (unsigned char)var.base[r1]
                                case OP_LOAD2:
                                    emit(LDRHa(rx[0], var.base, rx[1]));
                                    set_rx_ext(rx[0], Z_EXT16);
                                    break; // r0 = (unsigned short)var.base[r1]
                                default:
                                    emit(LDRa(rx[0], var.base, rx[1]));
                                    set_rx_ext(rx[0], Z_NONE);
                                    break; // r0 = var.base[r1]
                                }
                                unmask_rx(rx[1]);
                            }
                        } // load with zero-extension
                    ...
                    if ((ci + 1)->op == sign_extend && sign_extend != OP_UNDEF) {
                        ...
                        switch (ci->op) {
                        case OP_LOAD1:
                            emit(LDRSBa(rx[0], rDATABASE, rx[0]));
                            set_rx_ext(rx[0], S_EXT8);
                            break; // r0 = (signed char)dataBase[r0]
                        case OP_LOAD2:
                            emit(LDRSHa(rx[0], rDATABASE, rx[0]));
                            set_rx_ext(rx[0], S_EXT16);
                            break; // r0 = (signed short)dataBase[r0]
                        }
                        ip += 1; // OP_SEX8/OP_SEX16
                    } else {
                        ...
                        switch (ci->op) {
                        case OP_LOAD1:
                            emit(LDRBa(rx[0], rDATABASE, rx[0]));
                            set_rx_ext(rx[0], Z_EXT8);
                            break; // r0 = (unsigned char)dataBase[r0]
                        case OP_LOAD2:
                            emit(LDRHa(rx[0], rDATABASE, rx[0]));
                            set_rx_ext(rx[0], Z_EXT16);
                            break; // r0 = (unsigned short)dataBase[r0]
                        default:
                            emit(LDRa(rx[0], rDATABASE, rx[0]));
                            set_rx_ext(rx[0], Z_NONE);
                            break; // r0 = dataBase[r0]
                        }
                    }
                }
                ...
                if (scalar_on_top() && ci->op == OP_STORE4) {
                    ...
                    if (addr_on_top(&var)) {
                        ...
                        if (var.addr < max_offset) {
                            ...
                            switch (ci->op) {
                            case OP_STORE1:
                                emit(STRBai(rx[0], var.base, var.addr));
                                break; // (byte*)var.base[var.addr] = r0
                            case OP_STORE2:
                                emit(STRHai(rx[0], var.base, var.addr));
                                break; // (short*)var.base[var.addr] = r0
                            default:
                                emit(STRai(rx[0], var.base, var.addr));
                                break; // var.base[var.addr] = r0
                            }
                        } else {
                            // long offset
                            rx[1] = alloc_rx_const(R1, var.addr);
                            switch (ci->op) {
                            case OP_STORE1:
                                emit(STRBa(rx[0], var.base, rx[1]));
                                break; // (byte*)var.base[r1] = r0
                            case OP_STORE2:
                                emit(STRHa(rx[0], var.base, rx[1]));
                                break; // (short*)var.base[r1] = r0
                            default:
                                emit(STRa(rx[0], var.base, rx[1]));
                                break; // var.base[r1] = r0
                            }
                            unmask_rx(rx[1]);
                        }
                        wipe_var_range(&var);
                        ...
                        switch (ci->op) {
                        case OP_STORE1:
                            emit(STRBa(rx[0], rDATABASE, rx[1]));
                            break; // (byte*)database[r1] = r0
                        case OP_STORE2:
                            emit(STRHa(rx[0], rDATABASE, rx[1]));
                            break; // (short*)database[r1] = r0
                        default:
                            emit(STRa(rx[0], rDATABASE, rx[1]));
                            break; // database[r1] = r0
                        }
                        unmask_rx(rx[1]);
                        wipe_vars(); // unknown/dynamic address, wipe all register mappings
                    }
            ...
            case OP_BCOM:
                ...
                case OP_SEX8:
                    emit(SXTB(rx[1], rx[0]));
                    break; // r1 = sign extend r0
                case OP_SEX16:
                    emit(SXTH(rx[1], rx[0]));
                    break; // r1 = sign extend r0
                case OP_NEGI:
                    emit(RSBi(rx[1], rx[0], 0));
                    break; // r1 = -r0
                case OP_BCOM:
                    emit(MVN(rx[1], rx[0]));
                    break; // r1 = ~r0
            ...
            case OP_DIVU:
                if (CPU_Flags & CPU_IDIVA) {
                    rx[0] = load_rx_opstack(R0);
                    dec_opstack(); // r0 = *opstack
                    ...
                    rx[1] = load_rx_opstack(R1 | FORCED);
                    dec_opstack(); // r1 = *opstack
            ...
            case OP_RSHU:
                ...
                load_rx_opstack2(&rx[2], R0, &rx[0], R2);
                dec_opstack(); // r2 = r0 = *opstack
                rx[1] = load_rx_opstack(R1 | RCONST); // opstack-=4; r1 = *opstack
                switch (ci->op) {
                case OP_ADD:
                    emit(ADD(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 + r0
                case OP_SUB:
                    emit(SUB(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 - r0
                case OP_MULI:
                case OP_MULU:
                    emit(MUL(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 * r0
                case OP_BAND:
                    emit(AND(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 & r0
                case OP_BOR:
                    emit(ORR(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 | r0
                case OP_BXOR:
                    emit(EOR(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 ^ r0
                case OP_LSH:
                    emit(LSL(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 << r0
                case OP_RSHI:
                    emit(ASR(rx[2], rx[1], rx[0]));
                    break; // r2 = r1 >> r0
                case OP_RSHU:
                    emit(LSR(rx[2], rx[1], rx[0]));
                    break; // r2 = (unsigned)r1 >> r0
                }
                if (rx[0] != rx[2]) {
                    unmask_rx(rx[0]);
                }
            ...
            case OP_DIVF:
                ...
                load_sx_opstack2(&sx[2], S0, &sx[0], S2);
                dec_opstack(); // s2 = s0 = *opstack
                sx[1] = load_sx_opstack(S1 | RCONST); // opstack -= 4; s1 = *opstack
                switch (ci->op) {
                case OP_ADDF:
                    emit(VADD_F32(sx[2], sx[1], sx[0]));
                    break; // s2 = s1 + s0
                case OP_SUBF:
                    emit(VSUB_F32(sx[2], sx[1], sx[0]));
                    break; // s2 = s1 - s0
                case OP_MULF:
                    emit(VMUL_F32(sx[2], sx[1], sx[0]));
                    break; // s2 = s1 * s0
                case OP_DIVF:
                    emit(VDIV_F32(sx[2], sx[1], sx[0]));
                    break; // s2 = s1 / s0
                }
                if (sx[0] != sx[2]) {
                    unmask_sx(sx[0]);
            ...
            case OP_CVFI:
                ...
                switch (ci->op) {
                case OP_NEGF:
                    emit(VNEG_F32(sx[0], sx[0]));
                    break; // s0 = -s0
                case OP_CVIF:
                    emit(VCVT_F32_S32(sx[0], sx[0]));
                    break; // s0 = (float)s0
                case OP_CVFI:
                    emit(VCVT_S32_F32(sx[0], sx[0]));
                    break; // s0 = (int)s0
                }
                store_sx_opstack(sx[0]); // *opstack = s0
                break;
