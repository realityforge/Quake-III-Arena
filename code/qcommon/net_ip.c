...
static qboolean Sys_StringToSockaddr(const char *s, struct sockaddr *sadr,
  ...
  if (!retval) {
    ...
    if (search) {
      ...
      Com_Printf("Sys_StringToSockaddr: Error resolving %s: No address of "
                 "required type found.\n",
                 s);
  } else
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr) {
  ...
  if (ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr)) {
    fromlen = sizeof(from);
    ret = recvfrom(ip_socket, (void *)net_message->data, net_message->maxsize,
  ...
  if (ip6_socket != INVALID_SOCKET && FD_ISSET(ip6_socket, fdr)) {
    fromlen = sizeof(from);
    ret = recvfrom(ip6_socket, (void *)net_message->data, net_message->maxsize,
                   0, (struct sockaddr *)&from, &fromlen);
...
static qboolean NET_GetCvars(void) {
   ...
   * will be used if available due to ping */
