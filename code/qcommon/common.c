...
void QDECL Com_Error(int code, const char *fmt, ...) {
  ...
  restartClient =
      com_gameClientRestarting && !(com_cl_running && com_cl_running->integer);
...
//   dedicated, but I don't want to risk it
...
static void Name_PlayerNameCompletion(const char **names, int nameCount,
...
qboolean Com_FieldStringToPlayerName(char *name, int length,
                                     const char *rawname) {
  char hex[5];
  int i;
  int ch;
...
void Field_CompletePlayerName(const char **names, int nameCount) {
  ...
  if (completionString[0] != '\0' &&
      Q_stricmp(shortestMatch, completionString) == 0 && nameCount > 1) {
    int i;
    ...
    for (i = 0; i < nameCount; i++) {
      if (Q_stricmp(names[i], completionString) == 0) {
        ...
        Com_PlayerNameToFieldString(shortestMatch, sizeof(shortestMatch),
                                    names[i]);
        break;
      }
    }
