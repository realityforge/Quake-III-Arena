...
static void VM_CompileFunction(source_instruction_t *const i_first) {
  ...
  while ((i_now = i_now->next)) {
    ...
    case OP_NE:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_EQ ? branchTrue : branchFalse),
               4 * cr7 + eq, 0);
      gpr_pos -= 2;
      break;
    ...
    case OP_GEI:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_LTI ? branchTrue : branchFalse),
               4 * cr7 + lt, 0);
      gpr_pos -= 2;
      break;
    ...
    case OP_LEI:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_GTI ? branchTrue : branchFalse),
               4 * cr7 + gt, 0);
      gpr_pos -= 2;
      break;
    ...
    case OP_GEU:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_LTU ? branchTrue : branchFalse),
               4 * cr7 + lt, 0);
      gpr_pos -= 2;
      break;
    ...
    case OP_LEU:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_GTU ? branchTrue : branchFalse),
               4 * cr7 + gt, 0);
      gpr_pos -= 2;
      break;
    ...
    case OP_NEF:
      ...
      emitJump(i_now->arg.i, (i_now->op == OP_EQF ? branchTrue : branchFalse),
               4 * cr7 + eq, 0);
      fpr_pos -= 2;
      break;
    ...
    case OP_GEF:
      MAYBE_EMIT_CONST();
      in(iFCMPU, cr7, fSECOND, fFIRST);
      emitJump(i_now->arg.i, (i_now->op == OP_LTF ? branchTrue : branchFalse),
               4 * cr7 + lt, 0);
      fpr_pos -= 2;
      break;
    ...
    case OP_LEF:
      MAYBE_EMIT_CONST();
      in(iFCMPU, cr7, fSECOND, fFIRST);
      emitJump(i_now->arg.i, (i_now->op == OP_GTF ? branchTrue : branchFalse),
               4 * cr7 + gt, 0);
      fpr_pos -= 2;
      break;
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
...

  {
    ...
    Com_Printf("VM file %s compiled to %i bytes of code (%p - %p)\n", vm->name,
               vm->codeLength, vm->codeBase, vm->codeBase + vm->codeLength);
...
int VM_CallCompiled(vm_t *vm, int *args) {
  ...
  if (time_diff > 100) {
    printf("App clock: %ld, vm total: %ld, vm this: %ld, vm real: %ld, vm out: "
           "%ld\n"
