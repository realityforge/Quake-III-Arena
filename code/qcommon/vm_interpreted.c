...
void VM_PrepareInterpreter(vm_t *vm, vmHeader_t *header) {
  ...
  // instruction index into an index into codeBase[], which contains opcodes and
  // operands.
  while (instruction < header->instructionCount) {
    op = codeBase[int_pc];
    instruction++;
    ...
    switch (op) {
    // These ops need to translate addresses in jumps from instruction index to
    // int index
    case OP_EQ:
    case OP_NE:
    case OP_LTI:
...
#define r2 codeImage[programCounter]
  ...
  nextInstruction2:
    ...
    case OP_ARG:
      ...
      *(int *)&image[(codeImage[programCounter] + programStack) & dataMask] =
          r0;
      opStackOfs--;
      programCounter += 1;
      goto nextInstruction;
...
//				vm->callLevel = temp;
  ...
  if (opStackOfs != 1 || *opStack != 0xDEADBEEF)
    Com_Error(ERR_DROP, "Interpreter error: opStack[0] = %X, opStackOfs = %d",
