        ...
        while (ip < instructionCount) {
            ...
            case OP_GEF: {
                if (HasSSEFP()) {
                    ...
                    dec_opstack(); // xmm0 = *opstack; opstack -= 4
                    sx[1] = load_sx_opstack(R_XMM1 | RCONST);
                    dec_opstack(); // xmm1 = *opstack; opstack -= 4
            ...
            case OP_BXOR:
                ...
                case OP_SUB:
                    emit_sub_rx(rx[1], rx[0]);
                    break; // sub ecx, eax
                case OP_MULI:
                    emit_mul_rx(rx[1], rx[0]);
                    break; // imul ecx, eax
                case OP_MULU:
                    emit_mul_rx(rx[1], rx[0]);
                    break; // imul ecx, eax
                case OP_BAND:
                    emit_and_rx(rx[1], rx[0]);
                    break; // and ecx, eax
                case OP_BOR:
                    emit_or_rx(rx[1], rx[0]);
                    break; // or ecx, eax
                case OP_BXOR:
                    emit_xor_rx(rx[1], rx[0]);
                    break; // xor ecx, eax
                }
                unmask_rx(rx[0]);
                store_rx_opstack(rx[1]); // *opstack = ecx
            ...
            case OP_MODU:
                ...
                dec_opstack(); // ecx = *opstack; opstack -= 4
            ...
            case OP_DIVF:
                if (HasSSEFP()) {
                    sx[0] = load_sx_opstack(R_XMM0 | RCONST);
                    dec_opstack(); // xmm0 = *opstack
                    sx[1] = load_sx_opstack(R_XMM1); // opstack -= 4; xmm1 = *opstack
                    switch (ci->op) {
                    case OP_ADDF:
                        emit_add_sx(sx[1], sx[0]);
                        break; // xmm1 = xmm1 + xmm0
                    case OP_SUBF:
                        emit_sub_sx(sx[1], sx[0]);
                        break; // xmm1 = xmm1 - xmm0
                    case OP_MULF:
                        emit_mul_sx(sx[1], sx[0]);
                        break; // xmm1 = xmm1 * xmm0
                    case OP_DIVF:
                        emit_div_sx(sx[1], sx[0]);
                        break; // xmm1 = xmm1 / xmm0
                    }
                    unmask_sx(sx[0]);
