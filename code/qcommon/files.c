...
static const unsigned int missionpak_checksums[] = {2430342401u, 511014160u,
...
qboolean FS_PakIsPure(pack_t *pack) {
  ...
  if (fs_numServerPaks) {
    for (i = 0; i < fs_numServerPaks; i++) {
      ...
      //   the client does manips of its file names 'not a bug'
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search,
  ...
  *file = FS_HandleForFile();
  ...
  if (search->pack) {
    ...
    // https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=8 turned out I
    // used FS_FileExists instead
...
long FS_FOpenFileRead(const char *filename, fileHandle_t *file,
  ...
  isLocalConfig =
      !strcmp(filename, "autoexec.cfg") || !strcmp(filename, Q3CONFIG_CFG);
...
================
*/
void FS_GetModDescription(const char *modDir, char *description,
                          int descriptionLen) {
  ...
  Com_sprintf(descPath, sizeof(descPath), "%s%cdescription.txt", modDir,
              PATH_SEP);
...
int FS_GetModList(char *listbuf, int bufsize) {
  ...
  const char *const paths[] = {fs_basepath->string, fs_homepath->string,
                               fs_steampath->string, fs_gogpath->string};
  ...
  *listbuf = 0;
  ...
  for (i = 0; i < ARRAY_LEN(paths); i++) {
    ...
    // Sys_ConcatenateFileLists frees the lists so Sys_FreeFileList isn't
    // required
  ...
  for (i = 0; i < nPotential; i++) {
    ...
    if (bDrop || Q_stricmp(name, com_basegame->string) == 0 ||
        Q_stricmpn(name, ".", 1) == 0) {
      continue;
    }

    // in order to be a valid mod the directory must contain at least one .pk3
    // or .pk3dir we didn't keep the information when we merged the directory
    // names, as to what OS Path it was found under so we will try each of them
    // here
...
void FS_AddGameDirectory(const char *path, const char *dir) {
  ...
  while ((pakfilesi < numfiles) || (pakdirsi < numdirs)) {
    ...
    if (pakwhich) {
      ...
      // But wait, this could be any directory, we're filtering to only ending
      // with ".pk3dir" here.
...
qboolean FS_InvalidGameDir(const char *gamedir) {
  if (!strcmp(gamedir, ".") || !strcmp(gamedir, "..") || strchr(gamedir, '/') ||
      strchr(gamedir, '\\')) {
    return qtrue;
  }
  ...
  for (i = 0; i < fs_numServerReferencedPaks; i++) {
    ...
    if (!havepak && fs_serverReferencedPakNames[i] &&
      ...
      if (dlstring) {
        ...
        // server could overwrite non-pk3 files on clients by writing so much
        // crap into neededpaks that Q_strcat cuts off the .pk3 extension.
        ...
        // this file to the list if that is the case.
...
static void FS_CheckPak0(void) {
  ...
  for (path = fs_searchpaths; path; path = path->next) {
...

    {
      ...
      // Finally check whether this pak's checksum is listed because the user
      // tried to trick us by renaming the file, and set foundPak's highest bit
...
void FS_InitFilesystem(void) {
  ...
  Q_strncpyz(lastValidComBaseGame, com_basegame->string,
             sizeof(lastValidComBaseGame));
  Q_strncpyz(lastValidFsBaseGame, fs_basegame->string,
             sizeof(lastValidFsBaseGame));
