...
template <class T>
T* H_New(ha_pref heap)
{
	return (T*)Hunk_Alloc(sizeof(T), heap);
}
template <class T>
T* H_New(int c, ha_pref heap)
{
	return static_cast<T*>(Hunk_Alloc(sizeof(T) * c, heap));
}
