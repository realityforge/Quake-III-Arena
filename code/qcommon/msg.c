...
void MSG_WriteDeltaUsercmdKey(msg_t* msg, int key, const usercmd_t* from, const usercmd_t* to)
{
    ...
    if (from->angles[0] == to->angles[0] && // HMD PITCH
        ...
        from->forwardmove == to->forwardmove && from->rightmove == to->rightmove && from->upmove == to->upmove && from->buttons == to->buttons && from->weapon == to->weapon
#ifdef USE_VR_QVM
        &&
        ...
        from->right_muzzle_pos[0] == to->right_muzzle_pos[0] && from->right_muzzle_pos[1] == to->right_muzzle_pos[1] && from->right_muzzle_pos[2] == to->right_muzzle_pos[2] &&

        // Right hand origin
        from->right_hand_pos[0] == to->right_hand_pos[0] && from->right_hand_pos[1] == to->right_hand_pos[1] && from->right_hand_pos[2] == to->right_hand_pos[2] &&
        ...
        // Left hand origin
        from->left_hand_pos[0] == to->left_hand_pos[0] && from->left_hand_pos[1] == to->left_hand_pos[1] && from->left_hand_pos[2] == to->left_hand_pos[2] &&
