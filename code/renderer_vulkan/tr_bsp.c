...
static void ParseTriSurf(dsurface_t *ds, drawVert_t *verts, msurface_t *surf,
  ...
  tri = (srfTriangles_t *)ri.Hunk_Alloc(
      sizeof(*tri) + numVerts * sizeof(tri->verts[0]) +
...
static void R_LoadSubmodels(lump_t *l) {
  ...
  s_worldData.bmodels = out =
      (bmodel_t *)ri.Hunk_Alloc(count * sizeof(*out), h_low);
...
void RE_LoadWorldMap(const char *name) {
  ...
  Q_strncpyz(s_worldData.baseName, R_SkipPath(s_worldData.name),
             sizeof(s_worldData.name));
  R_StripExtension(s_worldData.baseName, s_worldData.baseName,
