...
void R_DebugPolygon(int color, int numPoints, float *points) {
  ...
  // that belong to back facing polygons. The code assumes that polygons are
  // convex.
  ...
  vk_shade_geometry(g_stdPipelines.surface_debug_pipeline_solid, VK_FALSE,
                    DEPTH_RANGE_NORMAL, VK_TRUE);

  // Outline.
  memset(tess.svars.colors, tr.identityLightByte,
         numPoints * 2 * sizeof(color4ub_t));
  ...
  updateMVP(backEnd.viewParms.isPortal, backEnd.projection2D,
            getptr_modelview_matrix());
  vk_shade_geometry(g_stdPipelines.surface_debug_pipeline_outline, VK_FALSE,
                    DEPTH_RANGE_ZERO, VK_FALSE);
