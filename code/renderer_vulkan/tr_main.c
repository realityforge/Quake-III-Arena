...
static inline void R_LocalVecToWorld(const float in[3], const float mat[3][3],
                                     float out[3]) {
...
/*
static void R_LocalNormalToWorld (const vec3_t local, const orientationr_t *
const pRT, vec3_t world)
{
        world[0] = local[0] * pRT->axis[0][0] + local[1] * pRT->axis[1][0] +
local[2] * pRT->axis[2][0]; world[1] = local[0] * pRT->axis[0][1] + local[1] *
pRT->axis[1][1] + local[2] * pRT->axis[2][1]; world[2] = local[0] *
pRT->axis[0][2] + local[1] * pRT->axis[1][2] + local[2] * pRT->axis[2][2];
}
*/
...
static inline void R_WorldVectorToLocal(const float in[3],
                                        const float mat[3][3], float out[3]) {
...
static void R_WorldPointToLocal(const vec3_t world,
                                const orientationr_t *const pRT, float out[3]) {
  float delta[3];
...
/*
static void R_MirrorVector (vec3_t in, orientation_t *surface, orientation_t
*camera, vec3_t out)
{
        int		i;
...
static inline void R_MirrorVector(vec3_t in, orientation_t *surface,
...
static void R_MirrorPoint(vec3_t in, orientation_t *surface,
                          orientation_t *camera, vec3_t out) {
...
typedef struct {
    ...
    float		origin[3];		// in world coordinates
        float		viewOrigin[3];	// viewParms->or.origin in local
coordinates } orientationr_t;

=================
*/
void R_RotateForEntity(const trRefEntity_t *const ent,
                       const viewParms_t *const viewParms,
                       orientationr_t *const or) {
  ...
  MatrixMultiply4x4_SSE(glMatrix, viewParms->world.modelMatrix,
                        or->modelMatrix);
...
=================
*/
static void R_RotateForViewer(viewParms_t *const pViewParams,
                              orientationr_t *const pEntityPose) {
  ...
  viewerMatrix[12] =
      -o0 * viewerMatrix[0] - o1 * viewerMatrix[4] - o2 * viewerMatrix[8];
  viewerMatrix[13] =
      -o0 * viewerMatrix[1] - o1 * viewerMatrix[5] - o2 * viewerMatrix[9];
  viewerMatrix[14] =
      -o0 * viewerMatrix[2] - o1 * viewerMatrix[6] - o2 * viewerMatrix[10];
  viewerMatrix[15] = 1;
...
static void R_SetupFrustum(viewParms_t *const pViewParams) {

  {
    ...
    pViewParams->frustum[0].dist =
        DotProduct(pViewParams->or.origin, pViewParams->frustum[0].normal);
    pViewParams->frustum[0].type = PLANE_NON_AXIAL;

    VectorSubtract(temp1, temp2, pViewParams->frustum[1].normal);
    pViewParams->frustum[1].dist =
        DotProduct(pViewParams->or.origin, pViewParams->frustum[1].normal);
...

  {
    ...
    pViewParams->frustum[2].dist =
        DotProduct(pViewParams->or.origin, pViewParams->frustum[2].normal);
    pViewParams->frustum[2].type = PLANE_NON_AXIAL;

    VectorSubtract(temp1, temp2, pViewParams->frustum[3].normal);
    pViewParams->frustum[3].dist =
        DotProduct(pViewParams->or.origin, pViewParams->frustum[3].normal);
...
static qboolean R_GetPortalOrientations(drawSurf_t *drawSurf, int entityNum,
                                        ...
                                        orientation_t *camera, vec3_t pvsOrigin,
                                        qboolean *mirror) {
  int i;
  ...
  for (i = 0; i < tr.refdef.num_entities; i++) {
    ...
    float d =
        DotProduct(e->e.origin, originalPlane.normal) - originalPlane.dist;
    if (d > 64 || d < -64) {
      continue;
    }
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf,
  ...
  for (i = 0; i < tess.numVertexes; i++) {
    ...
    TransformModelToClip_SSE(tess.xyz[i], tr.or.modelMatrix,
...
static void R_SetupProjection(viewParms_t *const pViewParams) {
  ...
  if (tr.refdef.rd.rdflags & RDF_NOWORLDMODEL) {
    ...
    for (i = 0; i < 8; i++) {
      ...
      v[0] = ((i & 1) ? pViewParams->visBounds[0][0]
                      : pViewParams->visBounds[1][0]) -
             o[0];
      v[1] = ((i & 2) ? pViewParams->visBounds[0][1]
                      : pViewParams->visBounds[1][1]) -
             o[1];
      v[2] = ((i & 4) ? pViewParams->visBounds[0][2]
                      : pViewParams->visBounds[1][2]) -
             o[0];
  ...
  // update q3's proj matrix (opengl) to vulkan conventions: z - [0, 1] instead
  // of [-1, 1] and invert y direction
