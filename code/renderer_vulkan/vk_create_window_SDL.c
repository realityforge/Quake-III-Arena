...
static void VKimp_DetectAvailableModes(void) {
  ...
  if (SDL_GetWindowDisplayMode(window_sdl, &windowMode) < 0 ||
      numSDLModes <= 0) {
    ri.Printf(PRINT_ALL,
...
static int VKimp_SetMode(int mode, qboolean fullscreen) {
  ...
  if ((tmp == 0) && (desktopMode.h > 0)) {
    ...
    ri.Printf(PRINT_ALL, " bpp %i\t%s\t%i x %i, refresh_rate: %dHz\n",
              SDL_BITSPERPIXEL(f), SDL_GetPixelFormatName(f), desktopMode.w,
              desktopMode.h, desktopMode.refresh_rate);
  } else if (SDL_GetDisplayMode(r_displayIndex->integer, 0, &desktopMode) !=
             0) {
...
void vk_createWindow(void) {
  ...
  r_displayIndex =
      ri.Cvar_Get("r_displayIndex", "0", CVAR_ARCHIVE | CVAR_LATCH);
  ...
  if (0 == VKimp_SetMode(r_mode->integer, r_fullscreen->integer)) {
    ...
    ri.Printf(PRINT_ALL,
              " Setting r_mode=%d, r_fullscreen=%d failed, falling back on "
              "r_mode=%d\n",
    ...
    if (0 == VKimp_SetMode(3, qfalse)) {
      ...
      ri.Error(ERR_FATAL, "VKimp_Init() - could not load Vulkan subsystem: %s",
               SDL_GetError());
    }
  }
...
void vk_getInstanceProcAddrImpl(void) {
  ...
  if (code) {
    ri.Error(ERR_FATAL, "Failed to load Vulkan library (code %d): %s", code,
             SDL_GetError());
  }
  // Create the window
...
void vk_minimizeWindow(void) {
  ...
  VkBool32 isWinFullscreen =
      (SDL_GetWindowFlags(window_sdl) & SDL_WINDOW_FULLSCREEN);
...
/*
        if( r_fullscreen->modified )
        {
                ...
                int fullscreen = !!( SDL_GetWindowFlags( window_sdl ) &
   SDL_WINDOW_FULLSCREEN );

                if( r_fullscreen->integer && ri.Cvar_VariableIntegerValue(
   "in_nograb" ) )
                {
                        ri.Printf( PRINT_ALL, "Fullscreen not allowed with
   in_nograb 1\n"); ri.Cvar_Set( "r_fullscreen", "0" ); r_fullscreen->modified =
   qfalse;
                }
                ...
                if( needToToggle )
                {
                        sdlToggled = SDL_SetWindowFullscreen( window_sdl,
   r_fullscreen->integer ) >= 0;

                        // SDL_WM_ToggleFullScreen didn't work, so do it the
   slow way if( !sdlToggled ) ri.Cmd_ExecuteText(EXEC_APPEND, "vid_restart\n");
