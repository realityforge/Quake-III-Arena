    ...
    for (w = 0; w < (BytesPerScanline / BytesPerPixel); w++) {
      ...
      for (p = 0; p < BytesPerPixel; p++) {
        ...
        case PNG_FilterType_Average: {
          DecompPtr[p] +=
              ((uint8_t)((((uint16_t)PixelLeft[p]) + ((uint16_t)PixelUp[p])) /
                         2));
...
static qboolean DecodeImageInterlaced(
  ...
  for (a = 0; a < PNG_Adam7_NumPasses; a++) {
    BytesPerScanline[a] =
        (PassWidth[a] * BytesPerPixel + (PixelsPerByte - 1)) / PixelsPerByte;
  }
  ...
  for (a = 0; a < PNG_Adam7_NumPasses; a++) {
    TargetLength +=
        ((BytesPerScanline[a] + (BytesPerScanline[a] ? 1 : 0)) * PassHeight[a]);
  }
        ...
        if (PixelsPerByte > 1) {
          ...
          for (p = 0; p < PixelsPerByte; p++) {
            if (CurrPixel < PassWidth[a]) {
              ...
              OutPtr =
                  OutBuffer + (((((h * HSkip[a]) + HOffset[a]) * IHDR_Width) +
