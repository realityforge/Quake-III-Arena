...
#define WAVEVALUE(table, base, amplitude, phase, freq)                         \
  ((base) +                                                                    \
   table[(int)((((phase) + tess.shaderTime * (freq)) * FUNCTABLE_SIZE)) &      \
         FUNCTABLE_MASK] *                                                     \
       (amplitude))
...
void RB_CalcWaveColor( const waveForm_t *wf, unsigned char *dstColors )
{
    ...
    if ( wf->func == GF_NOISE ) {
                glow = wf->base + R_NoiseGet4f( 0, 0, 0, ( tess.shaderTime +
wf->phase ) * wf->frequency ) * wf->amplitude; } else { glow = EvalWaveForm( wf
) * tr.identityLight;
        }
...
void RB_CalcDiffuseColor(unsigned char (*colors)[4]) {
  ...
  vector unsigned char vSel =
      (vector unsigned char)(0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff,
