...
static VkPipelineShaderStageCreateInfo get_shader_stage_desc(
        VkShaderStageFlagBits stage, VkShaderModule shader_module, const char*
entry)
{
...
// The function will take a buffer with the bytecode and the size of the buffer
// as parameter and craete VkShaderModule from it

static void create_shader_module(const unsigned char *pBytes, const int count,
                                 VkShaderModule *pVkShaderMod) {
  if (count % 4 != 0) {
    ri.Error(ERR_FATAL,
             "Vulkan: SPIR-V binary buffer size is not multiple of 4");
  }
...
void vk_loadShaderModules(void) {
  ...
  create_shader_module(single_texture_clipping_plane_vert_spv,
                       single_texture_clipping_plane_vert_spv_size,
  ...
  create_shader_module(multi_texture_clipping_plane_vert_spv,
                       multi_texture_clipping_plane_vert_spv_size,
  ...
  create_shader_module(multi_texture_mul_frag_spv,
                       multi_texture_mul_frag_spv_size,
  ...
  create_shader_module(multi_texture_add_frag_spv,
                       multi_texture_add_frag_spv_size,
                       &s_gShaderModules.multi_texture_add_fs);
}

void vk_specifyShaderModule(const enum Vk_Shader_Type shader_type,
                            const VkBool32 isClippingPlane, VkShaderModule *vs,
                            VkShaderModule *fs) {
...
void vk_destroyShaderModules(void) {
  ...
  qvkDestroyShaderModule(
      vk.device, s_gShaderModules.single_texture_clipping_plane_vs, NULL);
  ...
  qvkDestroyShaderModule(
      vk.device, s_gShaderModules.multi_texture_clipping_plane_vs, NULL);
  qvkDestroyShaderModule(vk.device, s_gShaderModules.multi_texture_mul_fs,
                         NULL);
  qvkDestroyShaderModule(vk.device, s_gShaderModules.multi_texture_add_fs,
                         NULL);
}
