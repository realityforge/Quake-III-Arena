...
================
*/
void R_LightScaleTexture(unsigned char *dst, unsigned char *in,
                         unsigned int nBytes) {
  unsigned int i;
...
// resolution representation of the same image. The height and width of each
// image, or level, in the mipmap is a power of two smaller than the previous
// level. Mipmaps do not have to be square. They are intended to increase
// rendering speed and reduce aliasing artifacts. A high-resolution mipmap image
// is used for high-density samples, such as for objects close to the camera.
// Lower-resolution images are used as the object appears farther away. This is
// a more efficient way of downfiltering (minifying) a texture than sampling all
// texels in the original texture that would contribute to a screen pixel; it is
// faster to take a constant number of samples from the appropriately
// downfiltered textures. Mipmaps are widely used in 3D computer games.

// The letters "MIP" in the name are an acronym of the Latin phrase multum in
// parvo, meaning "much in little".Since mipmaps, by definition, are
// pre-allocated, additional storage space is required to take advantage of
// them. Mipmap textures are used in 3D scenes to decrease the time required to
...
==================
*/
void R_BlendOverTexture(unsigned char *data, const uint32_t pixelCount,
                        uint32_t l) {
  uint32_t i;
...
================
*/
void R_MipMap(const unsigned char *in, uint32_t width, uint32_t height,
              unsigned char *out) {
...
================
*/
void R_MipMap2(const unsigned char *in, uint32_t inWidth, uint32_t inHeight,
               unsigned char *out) {
  ...
  for (i = 0; i < outHeight; i++) {
    for (j = 0; j < outWidth; j++) {
      ...
      for (k = 0; k < 4; k++) {
        ...
        unsigned int total =
            1 * in[(r0 + c0) * 4 + k] + 2 * in[(r0 + c1) * 4 + k] +
            2 * in[(r0 + c2) * 4 + k] + 1 * in[(r0 + c3) * 4 + k] +

            2 * in[(r1 + c0) * 4 + k] + 4 * in[(r1 + c1) * 4 + k] +
            4 * in[(r1 + c2) * 4 + k] + 2 * in[(r1 + c3) * 4 + k] +

            2 * in[(r2 + c0) * 4 + k] + 4 * in[(r2 + c1) * 4 + k] +
            4 * in[(r2 + c2) * 4 + k] + 2 * in[(r2 + c3) * 4 + k] +

            1 * in[(r3 + c0) * 4 + k] + 2 * in[(r3 + c1) * 4 + k] +
            2 * in[(r3 + c2) * 4 + k] + 1 * in[(r3 + c3) * 4 + k];
...
void ResampleTexture(unsigned char *pOut, const unsigned int inwidth,
                     const unsigned int inheight, const unsigned char *pIn,
  ...
  for (i = 0; i < outheight; i++) {
    const unsigned char *inrow1 =
        pIn + 4 * inwidth * (unsigned int)((i + 0.25) * inheight / outheight);
    const unsigned char *inrow2 =
        pIn + 4 * inwidth * (unsigned int)((i + 0.75) * inheight / outheight);
    // const unsigned char* inrow1 = pIn + inwidth * (unsigned
    // int)((4*i+1)*inheight/outheight); const unsigned char* inrow2 = pIn +
    // inwidth * (unsigned int)((4*i+3)*inheight/outheight);
...
void GetScaledDimension(const unsigned int width, const unsigned int height,
                        unsigned int *const outW, unsigned int *const outH,
                        int isPicMip) {
...
void imsave(char *fileName, unsigned char *buffer2, unsigned int width,
            unsigned int height);
...
void DEBUG_resample(const char *name, unsigned char *data,
                    unsigned char *pBuffer, unsigned int width,
                    unsigned int height, unsigned int scaled_width,
  ...
  ri.Printf(PRINT_ALL,
            "DEBUG_resample, inwidth: %d, inheight: %d, outwidth: %d, "
            "outheight: %d\n",
