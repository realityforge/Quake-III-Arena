...
PFN_vkEnumerateInstanceExtensionProperties
    qvkEnumerateInstanceExtensionProperties;
...
static void vk_createDebugCallback(PFN_vkDebugReportCallbackEXT qvkDebugCB) {
  ...
  VK_CHECK(
      qvkCreateDebugReportCallbackEXT(vk.instance, &desc, NULL, &vk.h_debugCB));
}
...
static void vk_createInstance(void) {
  ...
  // these commands should be queried at runtime as described in Command
  // Function Pointers. Extensions may be provided by layers as well as by a
  ...
  VkExtensionProperties *pInsExt =
      (VkExtensionProperties *)malloc(sizeof(VkExtensionProperties) * nInsExt);
  ...
  const char *const validation_layer_name =
...
static void vk_selectPhysicalDevice(void) {
  ...
  qvkGetPhysicalDeviceMemoryProperties(vk.physical_device,
                                       &vk.devMemProperties);
}
...
static void vk_selectSurfaceFormat(void) {
  ...
  VK_CHECK(qvkGetPhysicalDeviceSurfaceFormatsKHR(vk.physical_device, vk.surface,
                                                 &nSurfmt, NULL));
  assert(nSurfmt > 0);
  ...
  // swapchain
  VK_CHECK(qvkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      vk.physical_device, vk.surface, &vk.surface_caps));

  // VK_IMAGE_USAGE_TRANSFER_DST_BIT is required by image clear operations.
  if ((vk.surface_caps.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) ==
      0)
    ri.Error(ERR_FATAL,
             "VK_IMAGE_USAGE_TRANSFER_DST_BIT is not supported by you GPU.");

  // VK_IMAGE_USAGE_TRANSFER_SRC_BIT is required in order to take screenshots.
  if ((vk.surface_caps.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) ==
      0)
    ri.Error(ERR_FATAL,
             "VK_IMAGE_USAGE_TRANSFER_SRC_BIT is not supported by you GPU.");
  ...
  qvkGetPhysicalDeviceFormatProperties(vk.physical_device,
                                       vk.surface_format.format, &props);
  ...
  if (props.optimalTilingFeatures &
    ...
    qvkGetPhysicalDeviceFormatProperties(vk.physical_device,
                                         VK_FORMAT_D32_SFLOAT_S8_UINT, &props);

    if (props.optimalTilingFeatures &
        VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
      ri.Printf(
          PRINT_ALL,
          " VK_FORMAT_D32_SFLOAT_S8_UINT optimal Tiling feature supported.\n");
...
static void vk_selectQueueFamilyForPresentation(void) {
  ...
  qvkGetPhysicalDeviceQueueFamilyProperties(vk.physical_device, &nSurfmt,
                                            pQueueFamilies);
...
static void vk_createLogicalDevice(void) {
  ...
  qvkEnumerateDeviceExtensionProperties(vk.physical_device, NULL, &nDevExts,
                                        NULL);
