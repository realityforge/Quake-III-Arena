...
int R_ComputeLOD(trRefEntity_t *ent) {
  ...
  if (tr.currentModel->type == MOD_MDR) {
    ...
    mdrFrame_t *mdrframe =
        (mdrFrame_t *)((byte *)mdr + mdr->ofsFrames + frameSize * ent->e.frame);

    radius = RadiusFromBounds(mdrframe->bounds[0], mdrframe->bounds[1]);
  } else {
    md3Frame_t *frame =
        (md3Frame_t *)(((unsigned char *)tr.currentModel->md3[0]) +
  ...
  if (dist > 0) {
    ...
    //  float projected[4];
    //	projected[0] = p[0] * pMatProj[0] + p[1] * pMatProj[4] + p[2] *
    //pMatProj[8] + pMatProj[12];
    //  projected[1] = p[0] * pMatProj[1] - p[1] * pMatProj[5] + p[2] *
    //  pMatProj[9] + pMatProj[13];
    //	projected[2] = p[0] * pMatProj[2] + p[1] * pMatProj[6] + p[2] *
    //pMatProj[10] + pMatProj[14];
    //  projected[3] = p[0] * pMatProj[3] + p[1] * pMatProj[7] + p[2] *
    //  pMatProj[11] + pMatProj[15]; perspective devide pr = projected[1] /
    //  projected[3];

    float p1 = -radius * tr.viewParms.projectionMatrix[5] -
               dist * tr.viewParms.projectionMatrix[9] +
               tr.viewParms.projectionMatrix[13];
    float p3 = radius * tr.viewParms.projectionMatrix[7] -
