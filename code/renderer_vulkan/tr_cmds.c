...
void R_AddDrawSurfCmd(drawSurf_t *drawSurfs, int numDrawSurfs) {
  drawSurfsCommand_t *cmd =
      (drawSurfsCommand_t *)R_GetCommandBuffer(sizeof(drawSurfsCommand_t));
  if (!cmd) {
    return;
  }
...
void RE_SetColor(const float *rgba) {
  ...
  setColorCommand_t *cmd =
      (setColorCommand_t *)R_GetCommandBuffer(sizeof(setColorCommand_t));
  if (!cmd) {
    return;
  }
...
void RE_StretchPic(float x, float y, float w, float h, float s1, float t1,
  ...
  stretchPicCommand_t *cmd =
      (stretchPicCommand_t *)R_GetCommandBuffer(sizeof(stretchPicCommand_t));
  if (!cmd) {
    return;
  }
...
void RE_BeginFrame(stereoFrame_t stereoFrame) {
  ...
  drawBufferCommand_t *cmd =
      (drawBufferCommand_t *)R_GetCommandBuffer(sizeof(drawBufferCommand_t));
  if (!cmd) {
    return;
  }
...
void RE_EndFrame(int *frontEndMsec, int *backEndMsec) {
  ...
  swapBuffersCommand_t *cmd =
      (swapBuffersCommand_t *)R_GetCommandBuffer(sizeof(swapBuffersCommand_t));
  if (!cmd) {
    return;
  }
  ...
  *(int *)(BE_Commands.cmds + BE_Commands.used) = RC_END_OF_LIST;
  ...
  while (1) {
    ...
    case RC_DRAW_BUFFER: {
      ...
      // const drawBufferCommand_t * const cmd = (const drawBufferCommand_t
      // *)data;
