...
static md3Tag_t *R_GetAnimTag(mdrHeader_t *mod, int framenum,
                              const char *tagName, md3Tag_t *dest) {
  int i, j, k;
  int frameSize;
  mdrFrame_t *frame;
  mdrTag_t *tag;
...
static int R_IQMLerpTag(orientation_t *tag, iqmData_t *data, int startFrame,
...
int RE_LerpTag(orientation_t *tag, qhandle_t handle, int startFrame,
               int endFrame, float frac, const char *tagName) {
  ...
  for (i = 0; i < 3; i++) {
    tag->origin[i] =
        start->origin[i] + (end->origin[i] - start->origin[i]) * frac;
    tag->axis[0][i] =
        start->axis[0][i] + (end->axis[0][i] - start->axis[0][i]) * frac;
    tag->axis[1][i] =
        start->axis[1][i] + (end->axis[1][i] - start->axis[1][i]) * frac;
    tag->axis[2][i] =
        start->axis[2][i] + (end->axis[2][i] - start->axis[2][i]) * frac;
  }
