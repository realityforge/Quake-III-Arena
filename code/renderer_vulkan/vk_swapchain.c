...
VK_IMAGE_LAYOUT_PRESENT_SRC_KHR must only be used for presenting a presentable
image for display. A swapchain's image must be transitioned to this layout
before calling vkQueuePresentKHR, and must be transitioned away from this layout
after calling vkAcquireNextImageKHR.

*/

// vulkan does not have the concept of a "default framebuffer", hence it
// requires an infrastruture that will own the buffers we will render to before
// we visualize them on the screen. This infrastructure is known as the swap
// chain and must be created explicity in vulkan. The swap chain is essentially
// a queue of images that are waiting to be presented to the screen. The general
// purpose of the swap chain is to synchronize the presentation of images with
...
void vk_createSwapChain(VkDevice device, VkSurfaceKHR surface,
  ...
  // because it represents the actual conditions for showing images to the
  // screen There four possible modes available in Vulkan:
  ...
  //    to avoid tearing significantly less latency issues than standard
...

  {
    ...
    uint32_t nPM, i;
    qvkGetPhysicalDeviceSurfacePresentModesKHR(vk.physical_device, surface,
                                               &nPM, NULL);
    ...
    ri.Printf(PRINT_ALL,
              "Minimaal mumber ImageCount required: %d, Total %d present mode "
              "supported: \n",
    ...
    ri.Printf(PRINT_ALL,
              ...
              vk.surface_caps.minImageCount, vk.surface_caps.maxImageCount,
              image_count);
...

  {
    ...
    // returned byvkGetPhysicalDeviceSurfaceCapabilitiesKHR for the surface if
    ...
    // render images to a separate image first to perform operations like
    // post-processing
  ...
  //
  {
    ...
    VK_CHECK(qvkGetSwapchainImagesKHR(device, vk.swapchain,
    ...
    for (i = 0; i < vk.swapchain_image_count; i++) {
      ...
      VK_CHECK(qvkCreateImageView(device, &desc, NULL,
