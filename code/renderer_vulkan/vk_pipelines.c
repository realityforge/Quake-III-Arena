...
//
// The tessllation shaders allow you to subdivide geometry based on certain
// rules to increase the mesh quality. This is often make surfaces like brick
...
//
// The fragment shader is invoked for every fragment that servives and
// determines which framebuffer(s) the fragment are written to and with which
// color and depth values. It can do this using the interpolated data from the
// vertex shader, which can include things like texture coordinates and normals
// for lighting.
//
...
void vk_createPipelineLayout(void) {
  ...
  // So we must first create the Descriptor pool.
  {
    ...
    desc.pNext = NULL;
    desc.flags =
        VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT; // used by the
                                                           // cinematic images
    ...
    VK_CHECK(
        qvkCreateDescriptorPool(vk.device, &desc, NULL, &vk.descriptor_pool));
  }
...

  {
    ...
    descriptor_binding.descriptorType =
    ...
    // will be consumed by the set layout and used for the corresponding
    // binding. If pImmutableSamplers is NULL, then the sampler slots are
    // dynamic and sampler handles must be bound into descriptor sets using this
    ...
    VK_CHECK(
        qvkCreateDescriptorSetLayout(vk.device, &desc, NULL, &vk.set_layout));
  }
  ...
  // setLayoutCount: the number of descriptor sets included in the pipeline
  // layout. pSetLayouts: a pointer to an array of VkDescriptorSetLayout
  // objects.
  ...
  VK_CHECK(
      qvkCreatePipelineLayout(vk.device, &desc, NULL, &vk.pipeline_layout));
}

static void vk_create_pipeline(const struct Vk_Pipeline_Def *def,
  ...
  specialization_entries.offset =
      offsetof(struct Specialization_Data, alpha_test_func);
  ...
  vk_specifyShaderModule(def->shader_type, def->clipping_plane,
  ...
  VkVertexInputBindingDescription bindings[4];
  {
    ...
    // The stride parameter specifies the number of bytes from one entry to the
    // next
  ...
  vertex_input_state.vertexBindingDescriptionCount =
      (def->shader_type == ST_SINGLE_TEXTURE) ? 3 : 4;
  ...
  input_assembly_state.topology = def->line_primitives
  ...
  rasterization_state.polygonMode = (def->state_bits & GLS_POLYMODE_LINE)
  ...
  // configure the depth and stencil tests.
  ...
  depth_stencil_state.depthTestEnable =
      (def->state_bits & GLS_DEPTHTEST_DISABLE) ? VK_FALSE : VK_TRUE;
  depth_stencil_state.depthWriteEnable =
      (def->state_bits & GLS_DEPTHMASK_TRUE) ? VK_TRUE : VK_FALSE;
  depth_stencil_state.depthCompareOp = (def->state_bits & GLS_DEPTHFUNC_EQUAL)
                                           ? VK_COMPARE_OP_EQUAL
                                           : VK_COMPARE_OP_LESS_OR_EQUAL;
  depth_stencil_state.depthBoundsTestEnable = VK_FALSE;
  depth_stencil_state.stencilTestEnable =
      (def->shadow_phase != SHADOWS_RENDERING_DISABLED) ? VK_TRUE : VK_FALSE;
  ...
  attachment_blend_state.blendEnable =
      (def->state_bits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ? VK_TRUE
                                                                  : VK_FALSE;
  ...
  if (attachment_blend_state.blendEnable) {
    ...
    case GLS_SRCBLEND_ONE_MINUS_DST_COLOR:
      attachment_blend_state.srcColorBlendFactor =
    ...
    attachment_blend_state.srcAlphaBlendFactor =
        attachment_blend_state.srcColorBlendFactor;
    attachment_blend_state.dstAlphaBlendFactor =
        attachment_blend_state.dstColorBlendFactor;
  ...
  // If we want to do that, we have to fill in a
  ...
  VkDynamicState dynamic_state_array[3] = {VK_DYNAMIC_STATE_VIEWPORT,
  ...
  //
  VK_CHECK(qvkCreateGraphicsPipelines(vk.device, VK_NULL_HANDLE, 1,
                                      &create_info, NULL, pPipeLine));
}
...
void create_standard_pipelines(void) {
  ...
  ri.Printf(PRINT_ALL, " Create surface debug outline pipeline \n");
  {
    ...
    def.state_bits = GLS_POLYMODE_LINE | GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ONE |
                     GLS_DSTBLEND_ONE;
    def.line_primitives = VK_TRUE;
  ...
  ri.Printf(PRINT_ALL, " Create images debug pipeline \n");
  {
    ...
    def.state_bits = GLS_DEPTHTEST_DISABLE | GLS_SRCBLEND_SRC_ALPHA |
...
void vk_destroyGlobalStagePipeline(void) {
  ...
  for (i = 0; i < 2; i++)
    for (j = 0; j < 2; j++) {
      qvkDestroyPipeline(vk.device,
                         g_stdPipelines.shadow_volume_pipelines[i][j], NULL);
    }
  ...
  for (i = 0; i < 2; i++)
    for (j = 0; j < 3; j++)
      for (k = 0; k < 2; k++) {
        qvkDestroyPipeline(vk.device, g_stdPipelines.fog_pipelines[i][j][k],
                           NULL);
        qvkDestroyPipeline(vk.device, g_stdPipelines.dlight_pipelines[i][j][k],
                           NULL);
      }

  qvkDestroyPipeline(vk.device, g_stdPipelines.tris_debug_pipeline, NULL);
  qvkDestroyPipeline(vk.device, g_stdPipelines.tris_mirror_debug_pipeline,
                     NULL);
  qvkDestroyPipeline(vk.device, g_stdPipelines.normals_debug_pipeline, NULL);
  qvkDestroyPipeline(vk.device, g_stdPipelines.surface_debug_pipeline_solid,
                     NULL);
  qvkDestroyPipeline(vk.device, g_stdPipelines.surface_debug_pipeline_outline,
                     NULL);
