...
static const vidmode_t r_vidModes[] = {
    ...
    {"Mode 11: 856x480", 856, 480,
     1}, // Q3 MODES END HERE AND EXTENDED MODES BEGIN
    ...
    {"Mode 17: 1366x768", 1366, 768,
     1}, // yes there are some out there on that extra 6
...
void R_SetWinMode(int mode, unsigned int width, unsigned int height,
                  unsigned int hz) {
  ...
  if (mode == -2) {
    ...
    glConfig.windowAspect =
        (float)r_vidModes[mode].width /
        (r_vidModes[mode].height * r_vidModes[mode].pixelAspect);
    glConfig.displayFrequency = 60;
    glConfig.isFullscreen = 0;
  }
...
void R_InitDisplayResolution(void) {
  ...
  r_customwidth =
      ri.Cvar_Get("r_customwidth", "960", CVAR_ARCHIVE | CVAR_LATCH);
  r_customheight =
      ri.Cvar_Get("r_customheight", "540", CVAR_ARCHIVE | CVAR_LATCH);
  r_customaspect =
      ri.Cvar_Get("r_customaspect", "1.78", CVAR_ARCHIVE | CVAR_LATCH);
}

void R_glConfigClear(void) { memset(&glConfig, 0, sizeof(glConfig)); }

// IN: a pointer to the glConfig struct
void R_GetGlConfig(glconfig_t *const pCfg) { *pCfg = glConfig; }
...
void R_glConfigInit(void) {
  ...
  // Only using SDL_SetWindowBrightness to determine if hardware gamma is
  // supported
...
static void printDeviceExtensions(void) {
  ...
  VK_CHECK(qvkEnumerateDeviceExtensionProperties(vk.physical_device, NULL,
                                                 &nDevExts, NULL));
  ...
  ri.Printf(PRINT_ALL,
            "--------- Total %d Device Extension Supported ---------\n",
            nDevExts);
  uint32_t i;
...
static void printInstanceExtensions(int setting) {
  ...
  if (setting) {
    ...
    for (i = 0; i < nInsExt; ++i) {
      ...
      memcpy(glConfig.extensions_string + indicator, pInsExt[i].extensionName,
             len);
      indicator += len;
  ...
  nDevExts); uint32_t i; for (i=0; i<nDevExts; ++i)
      {
          ri.Printf(PRINT_ALL, " %s \n", pDevExt[i].extensionName);
      }
      ri.Printf(PRINT_ALL, "---- ----------------------------------- ----\n\n");
  ...
  pDevExt[i].extensionName, len); indicator += len;
          glConfig.extensions_string[indicator++] = ' ';
      }
      free(pDevExt);
  */
...
void vulkanInfo_f(void) {
  ...
  strncpy(glConfig.renderer_string, props.deviceName,
