...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  if (strcmp(name + (int)strlen(name) - 5, ".skin")) {
    skin->numSurfaces = 1;
    skin->pSurfaces =
        (skinSurface_t *)ri.Hunk_Alloc(sizeof(skinSurface_t), h_low);
  ...
  while (text_p && *text_p) {
    ...
    //		surf = skin->surfaces[ skin->numSurfaces ] = (skinSurface_t*)
  ...
  memcpy(skin->pSurfaces, parseSurfaces,
...
void R_InitSkins(void) {
  ...
  skin->numSurfaces = 1;
  //	skin->surfaces[0] = (skinSurface_t*) ri.Hunk_Alloc( sizeof(
  //*skin->surfaces ), h_low ); 	skin->surfaces[0]->shader = tr.defaultShader;
...
void R_SkinList_f(void) {
  ...
  for (i = 0; i < tr.numSkins; i++) {
    ...
    for (j = 0; j < skin->numSurfaces; j++) {
      ri.Printf(PRINT_ALL, "       %s = %s\n", skin->pSurfaces[j].name,
                skin->pSurfaces[j].shader->name);
    }
  }
  ri.Printf(PRINT_ALL, "------------------\n");
}
