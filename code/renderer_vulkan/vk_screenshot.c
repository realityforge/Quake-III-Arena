...
VK_IMAGE_TILING_LINEAR may not be supported unless other parameters meetall of
the constraints:
...
VK_IMAGE_USAGE_TRANSFER_DST_BIT Implementations may support additional limits
and capabilities beyond those listed above.
...
static void vk_read_pixels(unsigned char *pBuf, uint32_t W, uint32_t H) {
  ...
  // Create image in host visible memory to serve as a destination for
  // framebuffer pixels.
  ...
  VkDeviceMemory memory;
  {
    ...
    VK_CHECK(
        qvkAllocateMemory(vk.device, &memory_allocate_info, NULL, &memory));
  ...
  // subresource ranges. Memory barriers are used to transfer ownership between
  // queue families, change image layouts, and define availability and
  // visibility operations. They explicitly define the access types and buffer
  // and image subresource ranges that are included in the access scopes of a
  // memory dependency that is created by a synchronization command that
  // includes them.
  //
  // Image memory barriers only apply to memory accesses involving a specific
  // image subresource range. That is, a memory dependency formed from an image
  // memory barrier is scoped to access via the specified image subresource
  // range. Image memory barriers can also be used to define image layout
  // transitions or a queue family ownership transfer for the specified image
  // subresource range.
  ...
  qvkCmdPipelineBarrier(cmdBuf, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                        VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1,
                        &image_barrier);
  qvkCmdCopyImageToBuffer(
      cmdBuf, vk.swapchain_images_array[vk.idx_swapchain_image],
  ...
  unsigned char *data;
  VK_CHECK(
      qvkMapMemory(vk.device, memory, 0, VK_WHOLE_SIZE, 0, (void **)&data));
...
extern void RE_SaveJPG(char *filename, int quality, int image_width,
...
void RB_TakeScreenshot(int width, int height, char *fileName, VkBool32 isJpeg) {
  ...
  if (isJpeg) {
    ...
    // bufSize = RE_SaveJPGToBuffer(out, bufSize, 90, width, height, pImg,
    // padding); ri.FS_WriteFile(filename, out, bufSize);
  } else {
    ...
    unsigned char *const pBuffer =
        (unsigned char *)malloc(imgSize + cnPixels * 4);
...
static void R_TakeScreenshot(int x, int y, int width, int height, char *name,
                             qboolean jpeg) {
  static char fileName[MAX_OSPATH] = {
      0}; // bad things if two screenshots per frame?

  screenshotCommand_t *cmd =
      (screenshotCommand_t *)R_GetCommandBuffer(sizeof(*cmd));
  if (!cmd) {
    return;
  }
...
static void R_LevelShot(int W, int H) {
  ...
  for (y = 0; y < 128; y++) {
    for (x = 0; x < 128; x++) {
      ...
      for (yy = 0; yy < 3; yy++) {
        for (xx = 0; xx < 4; xx++) {
          src = source + 3 * (W * (int)((y * 3 + yy) * yScale) +
                              (int)((x * 4 + xx) * xScale));
          r += src[0];
          g += src[1];
          b += src[2];
...
void R_ScreenShot_f(void) {
  ...
  if (ri.Cmd_Argc() == 2 && !silent) {
    // explicit filename
    snprintf(checkname, sizeof(checkname), "screenshots/%s.tga",
             ri.Cmd_Argv(1));
  } else {
    ...
    for (; lastNumber <= 9999; lastNumber++) {
      ...
      snprintf(checkname, sizeof(checkname), "screenshots/shot%i%i%i%i.tga", a,
               b, c, d);
...
void R_ScreenShotJPEG_f(void) {
  ...
  if (ri.Cmd_Argc() == 2 && !silent) {
    // explicit filename
    snprintf(checkname, sizeof(checkname), "screenshots/%s.jpg",
             ri.Cmd_Argv(1));
  } else {
    // scan for a free filename
    ...
    for (; lastNumber <= 9999; lastNumber++) {
      ...
      snprintf(checkname, sizeof(checkname), "screenshots/shot%i%i%i%i.jpg", a,
               b, c, d);
...
void RB_TakeVideoFrameCmd(const videoFrameCommand_t *const cmd) {
  ...
  unsigned char *const pImg =
      (unsigned char *)malloc(cmd->width * cmd->height * 4);
  ...
  if (cmd->motionJpeg) {
    ...
    memcount = RE_SaveJPGToBuffer(cmd->encodeBuffer, linelen * cmd->height, 90,
...
void RE_TakeVideoFrame(int width, int height, unsigned char *captureBuffer,
                       unsigned char *encodeBuffer, qboolean motionJpeg) {
  if (!tr.registered) {
    return;
  }
