...
// Vk_Instance contains engine-specific vulkan resources that persist entire
// renderer lifetime. This structure is initialized/deinitialized by
// vk_initialize/vk_shutdown functions correspondingly.
struct Vk_Instance {
  VkInstance instance;
  VkPhysicalDevice physical_device;

  // Native platform surface or window objects are abstracted by surface
  // objects, which are represented by VkSurfaceKHR handles. The VK_KHR_surface
  // extension declares the VkSurfaceKHR object, and provides a function for
  // destroying VkSurfaceKHR objects. Separate platform-specific extensions each
  // provide a function for creating a VkSurfaceKHR object for the respective
  // platform.
  ...
  // depth and stencil aspects of a given image subresource must always be in
  // the same layout.
