...
struct ShadingData_t {
  ...
  // This flag is used to decide whether framebuffer's depth attachment should
  // be cleared with vmCmdClearAttachment (dirty_depth_attachment == true), or
  // it have just been cleared by render pass instance clear op
  // (dirty_depth_attachment == false).
...
VkBuffer vk_getIndexBuffer(void) { return shadingDat.index_buffer; }
...
const float *getptr_modelview_matrix() { return s_modelview_matrix; }
...
// memory. Host memory is memory needed by the Vulkan implementation for
...
void vk_createVertexBuffer(void) {
  ...
  qvkGetBufferMemoryRequirements(vk.device, shadingDat.vertex_buffer,
  ...
  ri.Printf(PRINT_ALL,
            " Allocate device memory for Vertex Buffer: %ld bytes. \n",
            alloc_info.allocationSize);

  VK_CHECK(qvkAllocateMemory(vk.device, &alloc_info, NULL,
                             &shadingDat.vertex_buffer_memory));

  qvkBindBufferMemory(vk.device, shadingDat.vertex_buffer,
                      shadingDat.vertex_buffer_memory, 0);

  void *data;
  VK_CHECK(qvkMapMemory(vk.device, shadingDat.vertex_buffer_memory, 0,
...
void vk_createIndexBuffer(void) {
  ...
  qvkGetBufferMemoryRequirements(vk.device, shadingDat.index_buffer,
                                 &ib_memory_requirements);
  ...
  ri.Printf(PRINT_ALL,
            " Allocate device memory for Index Buffer: %ld bytes. \n",
            alloc_info.allocationSize);

  VK_CHECK(qvkAllocateMemory(vk.device, &alloc_info, NULL,
                             &shadingDat.index_buffer_memory));
  qvkBindBufferMemory(vk.device, shadingDat.index_buffer,
                      shadingDat.index_buffer_memory, 0);

  void *data;
  VK_CHECK(qvkMapMemory(vk.device, shadingDat.index_buffer_memory, 0,
                        VK_WHOLE_SIZE, 0, &data));
  shadingDat.index_buffer_ptr = (unsigned char *)data;
}

// Descriptors and Descriptor Sets
// A descriptor is a special opaque shader variable that shaders use to access
// buffer and image resources in an indirect fashion. It can be thought of as a
// "pointer" to a resource.  The Vulkan API allows these variables to be changed
// between draw operations so that the shaders can access different resources
// for each draw.

// A descriptor set is called a "set" because it can refer to an array of
// homogenous resources that can be described with the same layout binding. one
// possible way to use multiple descriptors is to construct a descriptor set
// with two descriptors, with each descriptor referencing a separate texture.
// Both textures are therefore available during a draw. A command in a command
// buffer could then select the texture to use by specifying the index of the
// desired texture. To describe a descriptor set, you use a descriptor set
// layout.
...
void vk_shade_geometry(VkPipeline pipeline, VkBool32 multitexture,
                       enum Vk_Depth_Range depRg, VkBool32 indexed) {
  // configure vertex data stream
  VkBuffer bufs[3] = {shadingDat.vertex_buffer, shadingDat.vertex_buffer,
  ...
  // color
  if ((shadingDat.color_st_elements + tess.numVertexes) * sizeof(color4ub_t) >
      COLOR_SIZE)
  ...
  qvkCmdBindVertexBuffers(vk.command_buffer, 1, multitexture ? 3 : 2, bufs,
                          offs);
  shadingDat.color_st_elements += tess.numVertexes;
  ...
  if (tess.shader->polygonOffset) {
    qvkCmdSetDepthBias(vk.command_buffer, r_offsetUnits->value, 0.0f,
...
void updateMVP(VkBool32 isPortal, VkBool32 is2D, const float mvMat4x4[16]) {
  if (isPortal) {
    ...
    // Eye space transform.
    MatrixMultiply4x4_SSE(mvMat4x4, backEnd.viewParms.projectionMatrix,
                          push_constants);
    ...
    R_TransformPlane(backEnd.viewParms.or.axis, backEnd.viewParms.or.origin,
                     &eye_plane);
    ...
    // As described above in section Pipeline Layouts, the pipeline layout
    // defines shader push constants which are updated via Vulkan commands
    // rather than via writes to memory or copy commands. Push constants
    // represent a high speed path to modify constant data in pipelines that is
    // expected to outperform memory-backed resource updates.
    qvkCmdPushConstants(vk.command_buffer, vk.pipeline_layout,
                        VK_SHADER_STAGE_VERTEX_BIT, 0, 128, push_constants);
  } else {
    // push constants are another way of passing dynamic values to shaders
    // Specify push constants.
    float mvp[16] QALIGN(
        16); // mvp transform + eye transform + clipping plane in eye space
...
//  data transferred from one or more VkBuffer(s) by drawing commands. Vertex
//  shader input variables are bound to buffers via an indirect binding where
//  the vertex shader associates a vertex input attribute number with each
//  variable, vertex input attributes are associated to vertex input bindings on
//  a per-pipeline basis, and vertex input bindings are associated with specific
//  buffers on a per-draw basis via the vkCmdBindVertexBuffers command.
//
...
//  input attributes and VkPhysicalDeviceLimits::maxVertexInputBindings number
//  of vertex input bindings (each referred to by zero-based indices), where
//  there are at least as many vertex input attributes as there are vertex input
//  bindings. Applications can store multiple vertex input attributes
//  interleaved in a single buffer, and use a single vertex input binding to
//  access those attributes.
//
//  In GLSL, vertex shaders associate input variables with a vertex input
//  attribute number using the location layout qualifier. The component layout
//  qualifier associates components of a vertex shader input variable with
//  components of a vertex input attribute.

void vk_UploadXYZI(float (*pXYZ)[4], uint32_t nVertex, uint32_t *pIdx,
                   uint32_t nIndex) {
  // xyz stream
  {
    const VkDeviceSize xyz_offset =
        XYZ_OFFSET + shadingDat.xyz_elements * sizeof(vec4_t);
    ...
    qvkCmdBindVertexBuffers(vk.command_buffer, 0, 1, &shadingDat.vertex_buffer,
                            &xyz_offset);
  ...
  if (nIndex != 0) {
    ...
    unsigned char *iDst =
        shadingDat.index_buffer_ptr + shadingDat.index_buffer_offset;
    memcpy(iDst, pIdx, indexes_size);

    qvkCmdBindIndexBuffer(vk.command_buffer, shadingDat.index_buffer,
...
void vk_destroy_shading_data(void) {
  ri.Printf(PRINT_ALL, " Destroy vertex/index buffer: shadingDat.vertex_buffer "
                       "shadingDat.index_buffer. \n");
  ri.Printf(
      PRINT_ALL,
      " Free device memory: vertex_buffer_memory index_buffer_memory. \n");
...
void vk_clearColorAttachments(const float *color) {
  ...
  /*
          ...
          // It's a HACK to prevent Vulkan validation layer's performance
     warning:
          //		"vkCmdClearAttachments() issued on command buffer object
     XXX prior to any Draw Cmds.
          //		 It is recommended you use RenderPass LOAD_OP_CLEAR on
     Attachments prior to any Draw."
          //
          // NOTE: we don't use LOAD_OP_CLEAR for color attachment when we begin
     renderpass
          // since at that point we don't know whether we need color buffer
     clear (usually we don't). uint32_t h = clear_rect[0].rect.extent.height /
     2; clear_rect[0].rect.extent.height = h; clear_rect[1] = clear_rect[0];
...
static void ProjectDlightTexture(void) {
  ...
  for (l = 0; l < backEnd.refdef.num_dlights; l++) {
    ...
    float floatColor[3] = {backEnd.refdef.dlights[l].color[0] * 255.0f,
    ...
    vk_shade_geometry(
        g_stdPipelines
            .dlight_pipelines[backEnd.refdef.dlights[l].additive > 0 ? 1 : 0]
                             [tess.shader->cullType]
...
static void RB_FogPass(void) {
  ...
  VkPipeline pipeline =
      g_stdPipelines
          .fog_pipelines[tess.shader->fogPass - 1][tess.shader->cullType]
                        [tess.shader->polygonOffset];
...
void RB_StageIteratorGeneric(void) {
  ...
  for (stage = 0; stage < MAX_SHADER_STAGES; ++stage) {
...

    {
      ...
      if (numAnimaImg <= 1) {
        updateCurDescriptor(
            tess.xstages[stage]->bundle[0].image[0]->descriptor_set, 0);
      ...
      int index = (int)(tess.shaderTime *
                        tess.xstages[stage]->bundle[0].imageAnimationSpeed *
                        FUNCTABLE_SIZE) >>
                  FUNCTABLE_SIZE2;
      ...
      updateCurDescriptor(
          tess.xstages[stage]->bundle[0].image[index]->descriptor_set, 0);
  ...
  ENDANIMA:
    ...
    if (multitexture) {
      ...
      if (tess.xstages[stage]->bundle[1].numImageAnimations <= 1) {
        updateCurDescriptor(
            tess.xstages[stage]->bundle[1].image[0]->descriptor_set, 1);
        goto END_ANIMA2;
      }

      // it is necessary to do this messy calc to make sure animations line up
      // exactly with waveforms of the same frequency
      int index2 = (int)(tess.shaderTime *
                         tess.xstages[stage]->bundle[1].imageAnimationSpeed *
                         FUNCTABLE_SIZE) >>
                   FUNCTABLE_SIZE2;
      ...
      updateCurDescriptor(
          tess.xstages[stage]->bundle[1].image[index2]->descriptor_set, 1);
    ...
    END_ANIMA2:
      ...
      // replace diffuse texture with a white one thus effectively render only
      // lightmap
    }
    ...
    if (backEnd.viewParms.isMirror) {
      vk_shade_geometry(tess.xstages[stage]->vk_mirror_pipeline, multitexture,
                        depth_range, VK_TRUE);
    } else if (backEnd.viewParms.isPortal) {
      vk_shade_geometry(tess.xstages[stage]->vk_portal_pipeline, multitexture,
                        depth_range, VK_TRUE);
    } else {
      vk_shade_geometry(tess.xstages[stage]->vk_pipeline, multitexture,
                        depth_range, VK_TRUE);
    }

    // allow skipping out to show just lightmaps during development
    if (r_lightmap->integer && (tess.xstages[stage]->bundle[0].isLightmap ||
                                tess.xstages[stage]->bundle[1].isLightmap)) {
      break;
    }
  }
