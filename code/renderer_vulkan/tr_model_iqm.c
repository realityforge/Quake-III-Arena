...
qboolean R_LoadIQM(model_t *mod, void *buffer, int filesize,
  ...
  if (IQM_CheckRange(header, header->ofs_vertexarrays, header->num_vertexarrays,
  ...
  for (i = 0; i < header->num_meshes; i++, mesh++) {
    ...
    if (mesh->num_vertexes >= SHADER_MAX_VERTEXES) {
      ri.Printf(PRINT_WARNING,
                ...
                SHADER_MAX_VERTEXES - 1, meshName[0] ? meshName : "a surface",
                mesh->num_vertexes);
      return qfalse;
    }
  ...
  size +=
      header->num_poses * header->num_frames * 12 * sizeof(float); // pose mats
  if (header->ofs_bounds)
  ...
  for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
    ...
    switch (vertexarray->type) {
    case IQM_POSITION:
      memcpy(iqmData->positions, (byte *)header + vertexarray->offset,
             n * sizeof(float));
      break;
    case IQM_NORMAL:
      memcpy(iqmData->normals, (byte *)header + vertexarray->offset,
             n * sizeof(float));
      break;
    case IQM_TANGENT:
      memcpy(iqmData->tangents, (byte *)header + vertexarray->offset,
             n * sizeof(float));
      break;
    case IQM_TEXCOORD:
      memcpy(iqmData->texcoords, (byte *)header + vertexarray->offset,
             n * sizeof(float));
      break;
    case IQM_BLENDINDEXES:
      if (blendIndexesType == IQM_INT) {
        ...
        memcpy(iqmData->blendIndexes, (byte *)header + vertexarray->offset,
               n * sizeof(byte));
      }
      break;
    case IQM_BLENDWEIGHTS:
      if (blendWeightsType == IQM_FLOAT) {
        memcpy(iqmData->blendWeights.f, (byte *)header + vertexarray->offset,
               n * sizeof(float));
      } else {
        memcpy(iqmData->blendWeights.b, (byte *)header + vertexarray->offset,
               n * sizeof(byte));
      }
      break;
    case IQM_COLOR:
      memcpy(iqmData->colors, (byte *)header + vertexarray->offset,
             n * sizeof(byte));
      break;
    }
  }
...
void ComputePoseMats(iqmData_t *data, int frame, int oldframe, float backlerp,
                     float *mat) {
  float *mat1, *mat2;
