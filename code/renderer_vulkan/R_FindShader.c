...
void R_ClearShaderHashTable() { memset(hashTable, 0, sizeof(hashTable)); }
...
shader_t *R_FindShader(const char *name, int lightmapIndex,
  ...
  //
  {
    ...
    while (sh) {
      ...
      // shader is created.
      if ((0 == Q_stricmp(sh->name, strippedName)) &&
  ...
  */
  ...
  image_t *image = R_FindImageFile(name, mipRawImage, mipRawImage,
...
static void BuildSingleLargeBuffer(char *buffers[], const int nShaderFiles,
                                   const int sum) {
...
void RE_RemapShader(const char *shaderName, const char *newShaderName,
