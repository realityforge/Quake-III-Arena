...
static void *q3_stbi_malloc(size_t size) { return ri.Malloc((int)size); }
static void q3_stbi_free(void *p) { ri.Free(p); }
...
static void LoadTGA(const char *name, unsigned char **pic, uint32_t *width,
                    uint32_t *height);
static void LoadBMP(const char *name, unsigned char **pic, uint32_t *width,
                    uint32_t *height);
static void LoadJPG(const char *name, unsigned char **pic, uint32_t *width,
                    uint32_t *height);
static void LoadPCX32(const char *filename, unsigned char **pic,
                      uint32_t *width, uint32_t *height);

/*
    Loads any of the supported image types into a cannonical 32 bit format.
*/
void R_LoadImage2(const char *name, unsigned char **pic, uint32_t *width,
                  uint32_t *height) {
...
static void LoadJPG(const char *name, unsigned char **pic, uint32_t *width,
  ...
  int components;
  *pic = stbi_load_from_memory((unsigned char *)fbuffer, len, (int *)width,
                               (int *)height, &components, STBI_rgb_alpha);
  if (*pic == NULL) {
...
static void LoadBMP(const char *name, unsigned char **pic, uint32_t *width,
  ...
  *pic = NULL;
  ...
  if (bmpHeader.fileSize != length) {
    ri.Error(
        ERR_DROP,
        "LoadBMP: header size does not match file size (%ld vs. %d) (%s)\n",
...
static void LoadPCX(const char *filename, unsigned char **pic,
                    unsigned char **palette, uint32_t *width,
                    uint32_t *height) {
  char *raw;
  pcx_t *pcx;
  int x, y;
