  ...
  *pic = NULL;
  ...
  if (targa_header.image_type != 2 && targa_header.image_type != 10 &&
    ...
    ri.Printf(PRINT_WARNING,
              "LoadTGA: '%s' Only type 2 (RGB), 3 (gray), and 10 (RGB) TGA "
              "images supported\n",
              name);
  ...
  else if ((targa_header.pixel_size != 32 && targa_header.pixel_size != 24) &&
           targa_header.image_type != 3) {
    ri.Printf(
        PRINT_WARNING,
        "LoadTGA: '%s' Only 32 or 24 bit images supported (no colormaps)\n",
        name);
    //		ri.Error (ERR_DROP, "LoadTGA: Only 32 or 24 bit images supported (no
    //colormaps)");
    return;
  }
