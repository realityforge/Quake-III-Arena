...
void OpenXR_handle_controllers(void)
{
    ...
    vr_info.weapon_zoomed = vr_weaponScope->integer && vr_info.weapon_stabilised && (cl.snap.ps.weapon == WP_RAILGUN) && (VectorLength(vr_info.weaponoffset) < 0.24f) && cl.snap.ps.stats[STAT_HEALTH] > 0;
...
qboolean OpenXR_InitializeActions(void)
{
    ...
    // Suggest bindings for the Oculus Touch.
    {
        ...
        if (!xr_check(result, "xrStringToPath oculus Failed"))
            return qfalse;
        XrActionSuggestedBinding bindings[2][17] = { { // thumbstick x & y
    ...
    // Suggest bindings for the Vive Controller.
    {
        ...
        if (!xr_check(result, "xrStringToPath htc Failed"))
            return qfalse;
        XrActionSuggestedBinding bindings[2][9] = { { { xr.input.grabAction, triggerValuePath[SideLEFT] },
    ...
    // Suggest bindings for the Valve Index Controller.
    {
        ...
        if (!xr_check(result, "xrStringToPath valve Failed"))
            return qfalse;
        XrActionSuggestedBinding bindings[2][9] = { { { xr.input.grabAction, squeezeForcePath[SideLEFT] },
    ...
    // Suggest bindings for the Microsoft Mixed Reality Motion Controller.
    {
        ...
        if (!xr_check(result, "xrStringToPath microsoft Failed"))
            return qfalse;
        XrActionSuggestedBinding bindings[2][8] = { { { xr.input.grabAction, squeezeClickPath[SideLEFT] },
