...
void CG_ShaderStateChanged()
{
    ...
    while (o && *o) {
        n = strstr(o, "=");
        if (n && *n) {
            strncpyz(originalShader, o, n - o);
            originalShader[n - o] = 0;
            n++;
            t = strstr(n, ":");
            if (t && *t) {
                strncpyz(newShader, n, t - n);
                newShader[t - n] = 0;
            } else {
                break;
            }
...
static int CG_ParseVoiceChats(const char* filename, voiceChatList_t* voiceChatList, int maxVoiceChats)
{
    ...
    if (len >= MAX_VOICEFILESIZE) {
        CG_Printf(S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE);
