...
void CG_NewClientInfo(int clientNum)
{
    ...
    if (cg_forceModel.integer) {
        ...
        if (cgs.gametype >= GT_TEAM) {
            strncpyz(newInfo.headModelName, DEFAULT_TEAM_MODEL, sizeof(newInfo.headModelName));
...
static void CG_PlayerSprites(centity_t* cent)
{
    ...
    if (!(cent->currentState.eFlags & EF_DEAD) && cg.snap->ps.persistent[PERS_TEAM] == team && cgs.gametype >= GT_TEAM) {
