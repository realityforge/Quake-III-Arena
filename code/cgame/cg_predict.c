...
static void CG_TouchItem(centity_t* cent)
{
    ...
    if (cgs.gametype == GT_CTF) {
        if (cg.predictedPlayerState.persistent[PERS_TEAM] == TEAM_RED && item->giType == IT_TEAM && item->giTag == PW_REDFLAG)
            return;
        if (cg.predictedPlayerState.persistent[PERS_TEAM] == TEAM_BLUE && item->giType == IT_TEAM && item->giTag == PW_BLUEFLAG)
            return;
    }
