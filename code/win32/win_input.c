...
============================================================
*/
#define ISWINXP(sys) (sys.dwPlatformId == VER_PLATFORM_WIN32_NT && ((sys.dwMajorVersion == 5 && sys.dwMinorVersion >= 1) || (sys.dwMajorVersion > 5)))
    ...
    *pHotKey = 0;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (code == 0) // no more tokens
            break;
        if (code < 0 || kset[code & 0xFF] || (code != VK_CONTROL && code != VK_LCONTROL && code != VK_RCONTROL && code != VK_MENU && code != VK_LMENU && code != VK_RMENU && code != VK_SHIFT && code != VK_LSHIFT && code != VK_RSHIFT && code != (VK_LWIN | HK_MOD_LWIN) && code != (VK_RWIN | HK_MOD_RWIN) && *pHotKey & 0xFF)) {
