...
void RB_AddQuadStampExt(vec3_t origin, vec3_t left, vec3_t up, uint8_t* color, float s1, float t1, float s2, float t2)
{
    ...
    *(unsigned int*)&tess.vertexColors[ndx] = *(unsigned int*)&tess.vertexColors[ndx + 1] = *(unsigned int*)&tess.vertexColors[ndx + 2] = *(unsigned int*)&tess.vertexColors[ndx + 3] = *(unsigned int*)color;
...
static void RB_SurfaceFlare(UNUSED srfFlare_t* surf)
{
}
