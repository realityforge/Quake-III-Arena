...
#define RANGE_CHECK(Bytes)                                                  \
	if (p + (Bytes) > pEnd) {                                               \
		ri.Printf(PRINT_WARNING, "LoadTGA %s: Truncated file\n", fileName); \
		ri.Free(*pic);                                                      \
		*pic = NULL;                                                        \
		return qfalse;                                                      \
	}
...
size_t jpeg_mem_available(j_common_ptr cinfo, size_t min_bytes_needed, size_t max_bytes_needed, size_t already_allocated)
{
