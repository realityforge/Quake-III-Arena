...
template <class T>
T* AllocSurface(int numVerts, int numIndexes)
{
	T* surf = (T*)RI_New<byte>(sizeof(T) + (numVerts * sizeof(*surf->verts)) + (numIndexes * sizeof(*surf->indexes)));
...
template <typename T>
const T* ReadLump(const lump_t* lump, int* c)
{
