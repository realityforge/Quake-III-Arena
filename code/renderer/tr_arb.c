...
static qboolean FBO_CreateBloom(void)
{
    ...
    for (i = 0; i < r_bloom_passes->integer; i++) {
        ...
        if (!FBO_Create(&frameBuffers[i * 2 + BLOOM_BASE + 0], width, height, i == 0 ? qtrue : qfalse, NULL, NULL) || !FBO_Create(&frameBuffers[i * 2 + BLOOM_BASE + 1], width, height, qfalse, NULL, NULL)) {
            return qfalse;
        }
        width = width / 2;
        height = height / 2;
