...
void RE_RegisterFont(const char* fontName, int pointSize, fontInfo_t* font)
{
    ...
    if (len == sizeof(fontInfo_t)) {
        ...
        for (i = 0; i < GLYPHS_PER_FONT; i++) {
            ...
            strncpyz(font->glyphs[i].shaderName, (const char*)&fdFile[fdOffset], sizeof(font->glyphs[i].shaderName));
        ...
        strncpyz(font->name, name, sizeof(font->name));
    ...
    while (i <= GLYPH_END + 1) {
        ...
        if (xOut == -1 || yOut == -1) {
            ...
            for (j = lastStart; j < i; j++) {
                ...
                strncpyz(font->glyphs[j].shaderName, name, sizeof(font->glyphs[j].shaderName));
            }
            lastStart = i;
            memset(out, 0, 256 * 256);
            xOut = 0;
