...
#define IFR_NEXT(ifr) \
    ((struct ifreq*)((char*)(ifr) + sizeof(*(ifr)) + MAX(0, (int)(ifr)->ifr_addr.sa_len - (int)sizeof((ifr)->ifr_addr))))
    ...
    while ((char*)linkInterface < &ifc.ifc_buf[ifc.ifc_len]) {
        ...
        if (linkInterface->ifr_addr.sa_family == AF_LINK) {
            ...
            while ((char*)inetInterface < &ifc.ifc_buf[ifc.ifc_len]) {
                if (inetInterface->ifr_addr.sa_family == AF_INET && !strncmp(inetInterface->ifr_name, linkInterface->ifr_name, sizeof(linkInterface->ifr_name))) {
                    ...
                    if (sdl->sdl_type != IFT_LOOP) {
                        ...
                        strncpyz(ifr.ifr_name, inetInterface->ifr_name, sizeof(ifr.ifr_name));
